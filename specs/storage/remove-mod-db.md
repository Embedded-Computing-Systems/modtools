# Remove `packages/modtools/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/modtools/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the MOD legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/modtools/src/storage/db.ts` are concentrated in 22 source files:

- `packages/modtools/src/account/repo.ts`
- `packages/modtools/src/cli/cmd/db.ts`
- `packages/modtools/src/cli/cmd/import.ts`
- `packages/modtools/src/cli/cmd/stats.ts`
- `packages/modtools/src/control-plane/workspace.ts`
- `packages/modtools/src/index.ts`
- `packages/modtools/src/node.ts`
- `packages/modtools/src/permission/index.ts`
- `packages/modtools/src/project/project.ts`
- `packages/modtools/src/server/projectors.ts`
- `packages/modtools/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/modtools/src/server/shared/fence.ts`
- `packages/modtools/src/session/message-v2.ts`
- `packages/modtools/src/session/projectors.ts`
- `packages/modtools/src/session/prompt.ts`
- `packages/modtools/src/session/session.ts`
- `packages/modtools/src/session/todo.ts`
- `packages/modtools/src/share/share-next.ts`
- `packages/modtools/src/storage/db.ts`
- `packages/modtools/src/sync/index.ts`
- `packages/modtools/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy MOD database wrapper; `packages/modtools/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the MOD wrapper.
...
Status: Completed. `SyncEvent` and the MOD projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy MOD database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy MOD database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy MOD database wrapper.
...
All migration groups are complete or superseded. `packages/modtools/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No MOD data-migration group remains.

The previous MOD `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate MOD data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/MOD`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/modtools/src`
- `bun typecheck` from `packages/MOD`
- Relevant package tests from `packages/MOD`, not the repo root
