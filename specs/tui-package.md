...
Move the canonical MOD terminal application from
`packages/modtools/src/cli/cmd/tui` into a self-contained workspace package while
...
```text
packages/MOD ---\
                      > @modtools-ai/tui -> @modtools-ai/sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/MOD`,
`packages/cli`, or `@modtools-ai/core`.

The SDK is the TUI's MOD boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/MOD`.
- Do not replace private `packages/modtools` imports with `@modtools-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@modtools-ai/tui`.

## Ownership Boundary

### `@modtools-ai/tui` Owns
...
- MOD domain data displayed by the TUI
...
```text
packages/modtools/src/cli/cmd/tui
```

Its private dependency on `packages/MOD` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/modtools/src/*`.
...
Status: Completed. The private `@modtools-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@modtools-ai/tui`.
...
- The package has no dependency on `MOD`, `@modtools-ai/cli`, or
  `@modtools-ai/core`.
...
primitives now live in `@modtools-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@modtools-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads MOD globals or process state;
...
Replace process-global MOD state with resolved TUI inputs.
...
`@modtools-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/MOD` initially.
...
canonical in `@modtools-ai/tui`. Configured references resolve through the new
...
adapters rather than MOD domain implementations.

Make the SDK the only MOD domain boundary used by the TUI.
...
- Domain-facing TUI code imports MOD data and operations only from
  `@modtools-ai/sdk`.
...
`@modtools-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/MOD`.
...
- The package has no imports from `packages/MOD`, `packages/cli`, or
  `@modtools-ai/core`.
...
public `@modtools-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/MOD`.
- Keep the legacy embedded worker and server startup in `packages/MOD`.
- Change those adapters to load config, create transport inputs, and call the
  public `@modtools-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/MOD`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@modtools-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@modtools-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for MOD domain state.
...
rg '@modtools-ai/core|packages/modtools|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/modtools/src packages/modtools/test
```
...
- Build the current-platform `packages/MOD` binary.
