...
The MOD server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@modtools-ai/llm` tool loop after replacing its
...
- plugins get global "MOD" instance like in that post i showed
- MOD instance has stuff like `MOD.session.prompt()` or
  `MOD.tool.register({...})`
...
- decide whether to preserve deprecated `@modtools-ai/llm` orchestration exports
