...
import mod2ThemeJson from "./themes/mod-2.json"
...
const STORAGE_KEYS = {
  THEME_ID: "modtools-theme-id",
  COLOR_SCHEME: "modtools-color-scheme",
  THEME_CSS_LIGHT: "modtools-theme-css-light",
  THEME_CSS_DARK: "modtools-theme-css-dark",
} as const

const THEME_STYLE_ID = "mod-theme"
...
const names: Record<string, string> = {
  ...
  modtools: "Model-of-Design Tools",
  orng: "Orng",
...
function applyThemeCss(theme: DesktopTheme, themeId: string, mode: "light" | "dark") {
  ...
  document.getElementById("mod-theme-preload")?.remove()
...
export const { use: useTheme, provider: ThemeProvider } = createSimpleContext({
  ...
  init: (props: { defaultTheme?: string; onThemeApplied?: (theme: DesktopTheme, mode: "light" | "dark") => void }) => {
    const themeId = normalize(read(STORAGE_KEYS.THEME_ID) ?? props.defaultTheme) ?? "mod-2"
    ...
    const onStorage = (e: StorageEvent) => {
      if (e.key === STORAGE_KEYS.THEME_ID && e.newValue) {
        ...
        if (!next) return
        if (next !== "mod-2" && !knownThemes().has(next) && !store.themes[next]) return
    ...
    onMount(() => {
      ...
      const savedTheme = normalize(rawTheme ?? props.defaultTheme) ?? "mod-2"
    ...
    const setTheme = (id: string) => {
      ...
      if (next !== "mod-2" && !knownThemes().has(next) && !store.themes[next]) {
        console.warn(`Theme "${id}" not found`)
        return
      }
      setStore("themeId", next)
