...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("modtools-find")
  api.delete("modtools-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("modtools-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("modtools-find", new Highlight(...rest))
    return true
  }
