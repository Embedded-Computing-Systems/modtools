...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve mod route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "mod")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "mod")).toBe("openai")
    expect(statProvider("big-pickle", "", "mod")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "mod"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep mod as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "mod"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "mod"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep mod or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "mod"), country: "US" })).toMatchObject([
