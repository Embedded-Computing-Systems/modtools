...
import { runtime } from "@modtools-ai/stats-core/runtime"
...
const statsCanonicalBaseUrl = "https://modtools.ai/data/"
const statsUnfurlPath = "banner.png"
const statsUnfurlAlt = "Mod Data wordmark on a dark patterned background"
...
export default function StatsModel() {
  ...
  const modelDescription = createMemo(() =>
    stats()
      ? `${modelName()} usage, rank, token mix, cost, geo breakdown, and peer data across Mod.`
      : `${modelName()} model facts, limits, and Mod usage availability.`,
  )
      ...
      <div data-slot="model-hero-grid">
        <div data-slot="model-hero-copy">
          ...
          <Show
            when={props.data}
            fallback={
              <p>Model facts from the shared model index. Mod usage appears once this model has activity.</p>
            }
          >
            {(data) => (
              <p>
                Ranked #{data().rank} across recent Mod token usage with {formatPercent(data().tokenShare)} of
                observed volume.
              </p>
            )}
          </Show>
...
function ModelCatalogCallout(props: { catalog: ModelCatalogEntry | null }) {
  return (
    <div data-component="model-rank-panel">
      ...
      <p>No Mod usage in the current data window.</p>
    </div>
  )
}
...
function ModelOverview(props: { data: StatsModelData | null }) {
  return (
    <section data-section="model-panel">
      ...
      <Show
        when={props.data}
        fallback={<ModelEmptyState title="No usage summary" description="This model has no Mod usage rows yet." />}
      >
        {(data) => (
