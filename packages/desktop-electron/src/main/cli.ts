...
const CLI_INSTALL_DIR = ".modtools/bin"
const CLI_BINARY_NAME = "modtools"
...
export function getSidecarPath() {
  ...
  const path = app.isPackaged
    ? join(process.resourcesPath, `modtools-cli${suffix}`)
    : join(root, "../../resources", `modtools-cli${suffix}`)
...
export async function installCli(): Promise<string> {
  ...
  const tempScript = join(tmpdir(), "modtools-install.sh")
...
export function serve(hostname: string, port: number, password: string) {
  ...
  const env = {
    MOD_SERVER_USERNAME: "modtools",
    MOD_SERVER_PASSWORD: password,
  }
...
export function spawnCommand(args: string, extraEnv: Record<string, string>) {
  ...
  const env = {
    ...base,
    MOD_EXPERIMENTAL_ICON_DISCOVERY: "true",
    MOD_EXPERIMENTAL_FILEWATCHER: "true",
    MOD_CLIENT: "desktop",
...
function buildCommand(args: string, env: Record<string, string>, shell: string | null) {
  if (process.platform === "win32" && isWslEnabled()) {
    ...
    const script = [
      "set -e",
      'BIN="$HOME/.modtools/bin/modtools"',
      'if [ ! -x "$BIN" ]; then',
      `  curl -fsSL https://modtools.ai/install | bash -s -- --version ${shellEscape(version)} --no-modify-path`,
      "fi",
