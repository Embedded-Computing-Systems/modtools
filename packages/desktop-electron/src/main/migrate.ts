...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.modtools.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `modtools.settings.dat` is special: it maps to the `modtools.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"modtools.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // modtools.settings.dat → the electron settings store ("modtools.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "modtools.global.dat").
  const storeName = filename === "modtools.settings.dat" ? "modtools.settings" : filename
