...
const APP_NAMES: Record<string, string> = {
  dev: "Model-of-Design Tools Dev",
  beta: "Model-of-Design Tools Beta",
  prod: "Model-of-Design Tools",
}
...
app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "Model-of-Design Tools Dev")
...
function setupApp() {
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("modtools://"))
    if (urls.length) {
  ...
  void app.whenReady().then(async () => {
    // migrate()
    app.setAsDefaultProtocolClient("modtools")
    setDockIcon()
    setupAutoUpdater()
    syncCli()
    await initialize()
...
async function getSidecarPort() {
  const fromEnv = process.env.MOD_PORT
  if (fromEnv) {
...
function sqliteFileExists() {
  ...
  return existsSync(join(base, "modtools", "modtools.db"))
}
