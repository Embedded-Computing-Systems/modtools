...
import { PromptInput } from "@modtools-ai/schema/prompt-input"
import { Session } from "@modtools-ai/schema/session"
import { Project } from "@modtools-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@modtools-ai/schema/schema"
...
import { Revert } from "@modtools-ai/schema/revert"
import { SessionEvent } from "@modtools-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this MOD process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this MOD process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
