...
async fn initialize(app: AppHandle) {
    ...
    let sqlite_done = needs_migration.then(|| {
        tracing::info!(
            path = %modtools_db_path().expect("failed to get db path").display(),
...
fn get_sidecar_port() -> u32 {
    option_env!("MODTOOLS_PORT")
        .map(|s| s.to_string())
        .or_else(|| std::env::var("MODTOOLS_PORT").ok())
...
fn sqlite_file_exists() -> bool {
    let Ok(path) = modtools_db_path() else {
        return true;
    };
