...
use process_wrap::tokio::TokioCommandWrap as CommandWrap;
#[cfg(unix)]
use process_wrap::tokio::ProcessGroup;
#[cfg(windows)]
use process_wrap::tokio::{JobObject, KillOnDrop, TokioCommandWrapper as CommandWrapper};
...
pub fn spawn_command(
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut script = vec![
                "set -e".to_string(),
                "BIN=\"$HOME/.modtools/bin/mod\"".to_string(),
...
pub fn serve(
    ...
    let envs = [
        ("MOD_SERVER_USERNAME", "modtools".to_string()),
    ...
    .expect("Failed to spawn modtools");
...
mod tests {
    ...
    fn merge_shell_env_keeps_explicit_overrides() {
        ...
        assert_eq!(merged.get("MOD_CLIENT"), Some(&"desktop".to_string()));
    }
