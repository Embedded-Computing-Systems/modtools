...
const SIDECAR_SERVICE_NAME = "MOD server"
...
export function preferAppEnv(userDataPath: string) {
  ...
  Object.assign(process.env, {
    ...
    MODTOOLS_EXPERIMENTAL_ICON_DISCOVERY: "true",
...
export async function checkHealth(url: string, password?: string | null): Promise<boolean> {
  ...
  if (password) {
    const auth = Buffer.from(`MOD:${password}`).toString("base64")
...
function createSidecarEnv(): Record<string, string> {
  ...
  if (!app.isPackaged) env.MODTOOLS_DISABLE_CHANNEL_DB = "1"
  return env
}
