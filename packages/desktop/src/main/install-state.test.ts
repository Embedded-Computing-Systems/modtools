...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier MOD launch", () => {
    expect(hasExistingAppState([file("mod.settings")])).toBe(true)
    expect(hasExistingAppState([file("mod.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("mod")])).toBe(true)
  })
})
