...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.mod.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `mod.settings.dat` is special: it maps to the `mod.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"mod.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // mod.settings.dat → the electron settings store ("mod.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "mod.global.dat").
  const storeName = filename === "mod.settings.dat" ? "mod.settings" : filename
