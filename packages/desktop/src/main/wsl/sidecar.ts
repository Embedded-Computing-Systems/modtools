...
import { type WslCommandLine, resolveWslMod, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const mod = await resolveWslMod(distro)
  if (!mod) throw new Error(nativeT("desktop.wsl.error.modNotInstalled", { distro }))
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export MODTOOLS_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export MODTOOLS_CLIENT=desktop",
    `export MODTOOLS_SERVER_USERNAME=${shellEscape(username)}`,
    `export MODTOOLS_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(mod)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
