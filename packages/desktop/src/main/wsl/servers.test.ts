...
let releaseModResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectModVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectModVersion("1.14.35", "1.16.2")).toThrow(
    "MOD update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating MOD", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.mod/bin/mod",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, modChecks: {} })
})
...
test("ignores stale background MOD checks after removing a WSL server", async () => {
  persistedServers = []
  releaseModResolve = undefined
  ...
  await waitFor(() => !!releaseModResolve)
  await controller.removeServer("wsl:Debian")
  releaseModResolve?.()
  await new Promise((resolve) => setTimeout(resolve, 0))

  expect(controller.getState().servers).toEqual([])
  expect(controller.getState().modChecks).toEqual({})
})

test("ignores stale startup MOD checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking MOD", async () => {
  persistedServers = []
  ...
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveMod: async (distro) => {
      mod.push(distro)
      return "/home/me/.mod/bin/mod"
    },
  })
  ...
  expect(mod).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().modChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check MOD in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const mod: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(mod).toEqual(["Debian"])
  expect(Object.keys(controller.getState().modChecks)).toEqual(["Debian"])
})
