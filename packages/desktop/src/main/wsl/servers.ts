  ...
  return {
    ...
    async installMod(name: string) {
      await runJob({ kind: "install-mod", distro: name, startedAt: Date.now() }, async (abort) => {
        ...
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "MOD installation failed")
        }
...
function modCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "mod is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "mod is installed but could not run",
    }
  }
  return {
    distro,
