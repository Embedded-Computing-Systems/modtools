...
import { expectModVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolveMod?: typeof resolveWslMod
...
export function createWslServersController(
  ...
  const setModCheck = (distro: string, check: WslModCheck) => {
    setState({
      modChecks: {
        ...state.modChecks,
        [distro]: check,
  ...
  const checkMod = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveMod ?? resolveWslMod)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return modCheck(distro, resolved, version, appVersion)
  }

  const refreshModCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setModCheck(distro, await checkMod(distro, opts))
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const modChecks = await Promise.all(
      unique
        .filter((distro) => distroProbeReady(state.distroProbes[distro]))
        .filter((distro) => !state.modChecks[distro])
        .map(async (distro) => [distro, await checkMod(distro, opts)] as const),
    )
    if (modChecks.length) {
      setState({ modChecks: { ...state.modChecks, ...Object.fromEntries(modChecks) } })
    }
  }
  ...
  const refreshModCheckBackground = (id: string, distro: string) => {
    void checkMod(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setModCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl mod check failed", { id, distro, message })
      })
  }

  const refreshModChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkMod(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setModCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl mod check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshModCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async installMod(name: string) {
      await runJob({ kind: "install-mod", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslMod(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || nativeT("desktop.wsl.error.installMod"))
        }
        await refreshModCheck(name, { signal: abort.signal })
        expectModVersion(state.modChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.modChecks, distro) : {}),
      })
    },
...
function modCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: nativeT("desktop.wsl.error.modMissing"),
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: nativeT("desktop.wsl.error.modCannotRun"),
    }
  }
  return {
    distro,
