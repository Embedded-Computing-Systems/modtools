...
通过 Model-of-Design Tools，您可以从多个内置主题中进行选择，使用能自动适配终端主题的主题，或者定义您自己的自定义主题。

默认情况下，Model-of-Design Tools 使用我们自己的 `modtools` 主题。
...
Model-of-Design Tools 自带多个内置主题。
...
- 希望 Model-of-Design Tools 与终端的外观保持一致
- 使用了自定义终端配色方案
- 偏好所有终端应用程序拥有统一的视觉风格
...
Model-of-Design Tools 支持灵活的基于 JSON 的主题系统，让用户可以轻松创建和自定义主题。
...
1. **内置主题** — 嵌入在二进制文件中
2. **用户配置目录** — 定义在 `~/.config/modtools/themes/*.json` 或 `$XDG_CONFIG_HOME/modtools/themes/*.json`
3. **项目根目录** — 定义在 `<project-root>/.modtools/themes/*.json`
4. **当前工作目录** — 定义在 `./.modtools/themes/*.json`
