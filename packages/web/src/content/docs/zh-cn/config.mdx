...
您还可以使用 `~/.config/MOD/agents/` 或 `.mod/agents/` 中的 Markdown 文件定义代理。[在此了解更多](/docs/agents)。
...
您还可以使用 `~/.config/MOD/commands/` 或 `.mod/commands/` 中的 Markdown 文件定义命令。[在此了解更多](/docs/commands)。
...
将插件文件放置在 `.mod/plugins/` 或 `~/.config/MOD/plugins/` 中。您还可以通过 `plugin` 选项从 npm 加载插件。
