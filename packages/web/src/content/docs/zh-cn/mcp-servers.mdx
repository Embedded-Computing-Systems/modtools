...
你可以通过 _Model Context Protocol_（MCP）为 Model-of-Design Tools 添加外部工具。Model-of-Design Tools 同时支持本地和远程服务器。
...
你可以在 [Model-of-Design Tools 配置](https://modtools.ai/docs/config/)的 `mcp` 字段下定义 MCP 服务器。为每个 MCP 指定一个唯一的名称，在提示词中可以通过该名称来引用对应的 MCP。
...
组织可以通过其 `.well-known/modtools` 端点提供默认的 MCP 服务器。这些服务器可能默认处于禁用状态，允许用户按需启用。
...
Model-of-Design Tools 会自动处理远程 MCP 服务器的 OAuth 身份验证。当服务器需要身份验证时，Model-of-Design Tools 将：
...
如果服务器需要身份验证，Model-of-Design Tools 会在你首次使用时提示你进行认证。你也可以使用 `modtools mcp auth <server-name>` [手动触发认证流程](#authenticating)。
...
`mcp auth` 命令会打开浏览器进行授权。授权完成后，Model-of-Design Tools 会将 Token 安全地存储在 `~/.local/share/modtools/mcp-auth.json` 中。
...
你的 MCP 在 Model-of-Design Tools 中作为工具使用，与内置工具并列。因此，你可以像管理其他工具一样，通过 Model-of-Design Tools 配置来管理它们。
...
这会打开浏览器窗口完成 OAuth 流程，将 Model-of-Design Tools 连接到你的 Sentry 账户。
