...
Model-of-Design Tools has a list of keybinds that you can customize through `tui.json`.
...
Model-of-Design Tools uses a `leader` key for most keybinds. This avoids conflicts in your terminal.
...
The Model-of-Design Tools desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `modtools.json`.
