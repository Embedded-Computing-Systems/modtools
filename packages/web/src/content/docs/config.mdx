---
title: Config
description: Using the Mod JSON config.
---

You can configure Mod using a JSON config file.

---

## Format

Mod supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="MOD.jsonc"
{
...
1. **Remote config** (from `.well-known/MOD`) - organizational defaults
2. **Global config** (`~/.config/MOD/MOD.json`) - user preferences
3. **Custom config** (`MODTOOLS_CONFIG` env var) - custom overrides
4. **Project config** (`MOD.json` in project) - project-specific settings
5. **`.MOD` directories** - agents, commands, plugins
6. **Inline config** (`MODTOOLS_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/MOD/` on macOS) - admin-controlled
...
:::note
The `.MOD` and `~/.config/MOD` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/MOD` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/MOD"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global Mod config in `~/.config/MOD/MOD.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/MOD/tui.json`.
...
Add `MOD.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Mod starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `MODTOOLS_CONFIG` environment variable.

```bash
export MODTOOLS_CONFIG=/path/to/my/custom-config.json
MOD run "Hello world"
```
...
Specify a custom config directory using the `MODTOOLS_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.MOD` directory, and should
follow the same structure.

```bash
export MODTOOLS_CONFIG_DIR=/path/to/my/config-directory
MOD run "Hello world"
```

The custom directory is loaded after the global config and `.MOD` directories, so it **can override** their settings.
...
Drop an `MOD.json` or `MOD.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/MOD/` |
| Linux    | `/etc/MOD/`                         |
| Windows  | `%ProgramData%\MOD`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, Mod reads managed preferences from the `ai.MOD.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Mod checks these paths:

1. `/Library/Managed Preferences/<user>/ai.MOD.managed.plist`
2. `/Library/Managed Preferences/ai.MOD.managed.plist`

The plist keys map directly to `MOD.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.MOD.managed` PayloadType. The Mod config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.MOD.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.MOD.config</string>
      <key>PayloadUUID</key>
...
The server/runtime config schema is defined in [**`modtools.ai/config.json`**](https://modtools.ai/config.json).

TUI config uses [**`modtools.ai/tui.json`**](https://modtools.ai/tui.json).
...
Use `MODTOOLS_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `MOD.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `MOD serve` and `MOD web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Mod server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `MOD.local`. Useful for running multiple instances on the same network.
...
If not specified, Mod will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Mod config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Mod tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny Mod actions on configured resources. Currently, policies can control which providers Mod may use.
...
Mod normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, Mod omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/MOD/agents/` or `.modtools/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Mod will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`MOD run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/MOD/commands/` or `.modtools/commands/`. [Learn more here](/docs/commands).
...
Mod uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Mod will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, MOD **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Mod with custom tools, hooks, and integrations.

Place plugin files in `.modtools/plugins/` or `~/.config/MOD/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="MOD.json"
{
  "$schema": "https://modtools.ai/config.json",
  "plugin": ["MOD-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict Mod to only use specific providers rather than disabling them one by one.
