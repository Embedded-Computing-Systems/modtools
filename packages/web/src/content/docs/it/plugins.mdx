---
title: Plugin
description: Scrivi plugin per estendere Mod.
---

I plugin ti permettono di estendere Mod agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Mod.
...
- `~/.config/MOD/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/MOD/node_modules/`.
...
1. Config globale (`~/.config/MOD/MOD.json`)
2. Config di progetto (`MOD.json`)
3. Directory plugin globale (`~/.config/MOD/plugins/`)
...
Mod esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di MOD per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere MOD.
...
:::note
Se usi l'app desktop di Mod, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a MOD di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a MOD:
...
L'helper `tool` crea uno strumento personalizzato che MOD può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in MOD insieme agli strumenti integrati.
