---
title: Plugin
description: Scrivi plugin per estendere Model-of-Design Tools.
---

I plugin ti permettono di estendere Model-of-Design Tools agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Model-of-Design Tools.
...
- `.modtools/plugins/` - plugin a livello progetto
- `~/.config/modtools/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/modtools/node_modules/`.
...
1. Config globale (`~/.config/modtools/modtools.json`)
2. Config di progetto (`modtools.json`)
3. Directory plugin globale (`~/.config/modtools/plugins/`)
4. Directory plugin di progetto (`.modtools/plugins/`)
...
Model-of-Design Tools esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di modtools per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere modtools.
...
:::note
Se usi l'app desktop di Model-of-Design Tools, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a modtools di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a modtools:
...
L'helper `tool` crea uno strumento personalizzato che modtools può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in modtools insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://modtools.ai/docs/sdk) per i dettagli.
