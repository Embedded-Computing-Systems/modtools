...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di modtools. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in modtools ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di modtools.
...
modtools include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.modtools/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `modtools.json`:
...
- Globale: `~/.config/modtools/modes/`
- Progetto: `.modtools/modes/`
...
Se non viene specificata alcuna temperatura, modtools usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di modtools sia per quella specifica del progetto.
...
Crea file di modalita in `.modtools/modes/` per modalita specifiche del progetto oppure in `~/.config/modtools/modes/` per modalita globali:
