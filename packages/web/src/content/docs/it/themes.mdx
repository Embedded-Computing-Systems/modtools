...
Con Model-of-Design Tools puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, Model-of-Design Tools usa il tema `modtools`.
...
Model-of-Design Tools include diversi temi integrati.
...
- Vuole che Model-of-Design Tools corrisponda all'aspetto del terminale
...
Model-of-Design Tools supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/modtools/themes/*.json` o `$XDG_CONFIG_HOME/modtools/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.modtools/themes/*.json`
4. **Directory di lavoro corrente** - in `./.modtools/themes/*.json`
