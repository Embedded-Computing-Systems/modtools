...
In Model-of-Design Tools esistono due tipi di agenti: agenti primari e subagenti.
...
Model-of-Design Tools include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
Model-of-Design Tools include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

Model-of-Design Tools include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `modtools.json`:
...
- Globale: `~/.config/modtools/agents/`
...
Se non specifichi una temperature, Model-of-Design Tools usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di Model-of-Design Tools sia per la config specifica del progetto.
...
L'ID modello nella configurazione Model-of-Design Tools usa il formato `provider/model-id`. Per esempio, se stai usando [Model-of-Design Tools Zen](/docs/zen), useresti `modtools/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `modtools models` per vedere la lista dei modelli disponibili.
:::
