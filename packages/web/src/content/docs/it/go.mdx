...
Mod Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in Mod. Ti abboni a Mod Go e
...
Mod Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

Mod Go funziona come qualsiasi altro provider in Mod.

1. Accedi a **<a href={console}>Mod Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `Mod Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a Mod Go.
:::
...
Mod Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua Mod config
utilizza il formato `MOD-go/<model-id>`. Ad esempio, per Kimi K2.7 Code, useresti
`MOD-go/kimi-k2.7-code` nella tua configurazione.
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con Mod.
