...
Le skill degli agenti permettono a Mod di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Mod cerca in queste posizioni:

- Config di progetto: `.modtools/skills/<name>/SKILL.md`
- Config globale: `~/.config/MOD/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Mod risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
...
Le definizioni globali vengono caricate anche da `~/.config/MOD/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Mod elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `MOD.json`:
...
**Per agenti integrati** (in `MOD.json`):
