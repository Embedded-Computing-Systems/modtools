...
Le skill degli agenti permettono a Model-of-Design Tools di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Model-of-Design Tools cerca in queste posizioni:

- Config di progetto: `.modtools/skills/<name>/SKILL.md`
- Config globale: `~/.config/modtools/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Model-of-Design Tools risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.modtools/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/modtools/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.modtools/skills/git-release/SKILL.md` cosi':
...
Model-of-Design Tools elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `modtools.json`:
...
**Per agenti integrati** (in `modtools.json`):
