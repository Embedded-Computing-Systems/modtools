...
Puoi aggiungere strumenti esterni a Mod usando il _Model Context Protocol_, o MCP. Mod supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Mod Config](https://modtools.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/MOD`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Mod gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Mod:
...
Se il server richiede autenticazione, Mod ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `MOD mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Mod memorizzera' i token in modo sicuro in `~/.local/share/MOD/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Mod insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Mod come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Mod al tuo account Sentry.
