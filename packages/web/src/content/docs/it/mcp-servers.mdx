...
Puoi aggiungere strumenti esterni a Model-of-Design Tools usando il _Model Context Protocol_, o MCP. Model-of-Design Tools supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Model-of-Design Tools Config](https://modtools.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/modtools`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Model-of-Design Tools gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Model-of-Design Tools:
...
Se il server richiede autenticazione, Model-of-Design Tools ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `modtools mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Model-of-Design Tools memorizzera' i token in modo sicuro in `~/.local/share/modtools/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Model-of-Design Tools insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Model-of-Design Tools come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Model-of-Design Tools al tuo account Sentry.
