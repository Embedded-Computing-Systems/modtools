---
title: Server LSP
description: Model-of-Design Tools si integra con i tuoi server LSP.
---

Model-of-Design Tools si integra con il tuo Language Server Protocol (LSP) per aiutare l'LLM a interagire con la tua codebase. Usa le diagnostiche per fornire feedback all'LLM.

---

## Integrati

Model-of-Design Tools include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `MOD_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Puoi personalizzare i server LSP tramite la sezione `lsp` nella configurazione di modtools.
