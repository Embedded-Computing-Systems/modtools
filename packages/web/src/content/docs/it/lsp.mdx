---
title: Server LSP
description: MOD si integra con i tuoi server LSP.
---

MOD può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

MOD include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `MODTOOLS_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP è abilitato e MOD apre un file:
...
Puoi abilitare e personalizzare i server LSP tramite la sezione `lsp` nella configurazione di MOD.
