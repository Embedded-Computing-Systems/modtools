---
title: Provider
description: Usa qualsiasi provider LLM in MOD.
---

import config from "../../../../config.mjs"
export const console = config.console

MOD usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di MOD.
...
MOD Zen e una lista di modelli forniti dal team MOD, testati e
verificati per funzionare bene con MOD. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da MOD Zen.
:::

1. Esegui `/connect` nella TUI, seleziona MOD e vai su [modtools.ai/auth](https://modtools.ai/auth).
...
Funziona come qualsiasi altro provider in MOD ed e totalmente opzionale.
...
Per usare Amazon Bedrock con MOD:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui MOD:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `MOD.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in MOD non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Puoi configurare MOD per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché MOD funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di MOD.
...
salvare il token nello storage di auth di MOD.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
MOD usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare MOD
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `MOD.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di [MOD-gitlab-auth](https://www.npmjs.com/package/MOD-gitlab-auth).

##### Configurazione

Personalizza tramite `MOD.json`:
...
Per usare il tuo abbonamento GitHub Copilot con MOD:
...
Per usare Google Vertex AI con MOD:
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da MOD, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [MOD-helicone-session](https://github.com/Embedded-Computing-Systems/modtools-helicone-session) per loggare automaticamente ogni conversazione di MOD come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di MOD elencata come sessione separata.
...
Puoi configurare MOD per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare MOD per usare modelli locali tramite LM Studio.
...
Puoi configurare MOD per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per MOD. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/MOD) per dettagli.
:::
...
Per usare Ollama Cloud con MOD:
...
3. Copia la chiave API da usare in MOD.
...
6. **Importante**: Prima di usare modelli cloud in MOD, devi scaricare le informazioni del modello localmente:
...
MOD Zen è una lista di modelli testati e verificati forniti dal team MOD. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>MOD Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **MOD Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di MOD.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di MOD.
...
5. Puoi anche personalizzarli tramite la configurazione di MOD. Ecco un esempio di specifica di un provider
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con Mod:
...
Puoi anche personalizzare i modelli tramite la tua configurazione di MOD. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con MOD. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `MOD.json` nella directory del progetto:
...
I campi `limit` permettono a MOD di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `MOD auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di MOD e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config MOD.
