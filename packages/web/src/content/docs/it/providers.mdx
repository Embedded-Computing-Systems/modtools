---
title: Provider
description: Usa qualsiasi provider LLM in Model-of-Design Tools.
---

import config from "../../../../config.mjs"
export const console = config.console

Model-of-Design Tools usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di Model-of-Design Tools.
...
configurazione di Model-of-Design Tools.
...
Model-of-Design Tools Zen e una lista di modelli forniti dal team Model-of-Design Tools, testati e
verificati per funzionare bene con Model-of-Design Tools. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da Model-of-Design Tools Zen.
:::

1. Esegui `/connect` nella TUI, seleziona modtools e vai su [modtools.ai/auth](https://modtools.ai/auth).
...
Funziona come qualsiasi altro provider in Model-of-Design Tools ed e totalmente opzionale.
...
Per usare Amazon Bedrock con Model-of-Design Tools:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui modtools:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `modtools.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in Model-of-Design Tools non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché modtools funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di modtools.
...
2. Scegli il tuo metodo di autenticazione:
   ...
   3. Nome: `Model-of-Design Tools`, Scopes: `api`
...
salvare il token nello storage di auth di modtools.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
Model-of-Design Tools usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare Model-of-Design Tools
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `modtools.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di [modtools-gitlab-auth](https://www.npmjs.com/package/modtools-gitlab-auth).

##### Configurazione

Personalizza tramite `modtools.json`:
...
Per usare il tuo abbonamento GitHub Copilot con modtools:
...
Per usare Google Vertex AI con Model-of-Design Tools:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui modtools.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da modtools, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [modtools-helicone-session](https://github.com/H2Shami/modtools-helicone-session) per loggare automaticamente ogni conversazione di Model-of-Design Tools come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di Model-of-Design Tools elencata come sessione separata.
...
Puoi configurare modtools per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare modtools per usare modelli locali tramite LM Studio.
...
Puoi configurare modtools per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per Model-of-Design Tools. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/modtools) per dettagli.
:::
...
Per usare Ollama Cloud con Model-of-Design Tools:
...
3. Copia la chiave API da usare in Model-of-Design Tools.
...
6. **Importante**: Prima di usare modelli cloud in Model-of-Design Tools, devi scaricare le informazioni del modello localmente:
...
Model-of-Design Tools Zen è una lista di modelli testati e verificati forniti dal team Model-of-Design Tools. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>Model-of-Design Tools Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **Model-of-Design Tools Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di Model-of-Design Tools.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di modtools.
...
5. Puoi anche personalizzarli tramite la configurazione di modtools. Ecco un esempio di specifica di un provider
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con Modtools:
...
Puoi anche personalizzare i modelli tramite la tua configurazione di modtools. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con modtools. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `modtools.json` nella directory del progetto:
...
I campi `limit` permettono a Model-of-Design Tools di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `modtools auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di modtools e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config modtools.
