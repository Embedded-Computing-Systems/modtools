---
title: Server
description: Interagisci con il server modtools via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `modtools serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client modtools.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `modtools.local` |
...
Imposta `MOD_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `modtools`, oppure imposta `MOD_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `modtools serve` sia per `modtools web`.
...
Quando esegui `modtools` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server modtools per interagire con modtools in modo programmatico.
:::

Questa architettura permette a modtools di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `modtools serve` per avviare un server standalone. Se la TUI di modtools e gia in esecuzione, `modtools serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Model-of-Design Tools.
...
Il server modtools espone le seguenti API.
