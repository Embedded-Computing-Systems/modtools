...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. Model-of-Design Tools include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in Model-of-Design Tools.
...
:::note
Questo strumento e' disponibile solo quando `MOD_EXPERIMENTAL_LSP_TOOL=true` (oppure `MOD_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider Model-of-Design Tools o quando la variabile d'ambiente `MOD_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di Model-of-Design Tools:
