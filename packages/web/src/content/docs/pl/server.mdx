---
title: Serwer
description: Interakcja z serwerem modtools za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `modtools serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient modtools.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `modtools.local` |
...
Ustaw `MOD_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `modtools` lub ustaw `MOD_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `modtools serve`, jak i `modtools web`.
...
Kiedy uruchomisz `modtools`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera modtools do programowej interakcji z modtools.
:::

Ta architektura umożliwia obsługę wielu klientów przez modtools i programową interakcję z kodem otwartym.

Możesz uruchomić `modtools serve`, aby uruchomić samodzielny serwer. Jeśli masz
modtools TUI działa, `modtools serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki modtools [IDE](/docs/ide).
...
Serwer modtools udostępnia następujące interfejsy API.
