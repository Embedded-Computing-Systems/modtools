---
title: Serwer
description: Interakcja z serwerem MOD za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `MOD serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient MOD.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `MOD.local` |
...
Ustaw `MODTOOLS_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `MOD` lub ustaw `MODTOOLS_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `MOD serve`, jak i `MOD web`.
...
Kiedy uruchomisz `MOD`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera MOD do programowej interakcji z MOD.
:::

Ta architektura umożliwia obsługę wielu klientów przez MOD i programową interakcję z kodem otwartym.

Możesz uruchomić `MOD serve`, aby uruchomić samodzielny serwer. Jeśli masz
MOD TUI działa, `MOD serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki MOD [IDE](/docs/ide).
...
Serwer MOD udostępnia następujące interfejsy API.
