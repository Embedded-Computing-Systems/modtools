...
Umiejętności agenta pozwalają MOD odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
MOD przeszukuje te lokalizacje:

- Project config: `.modtools/skills/<name>/SKILL.md`
- Global config: `~/.config/MOD/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu MOD przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
...
Ładowane są także definicje globalne z `~/.config/MOD/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
MOD wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `MOD.json`:
...
**Dla agentów wbudowanych** (w `MOD.json`):
