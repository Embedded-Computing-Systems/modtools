...
Umiejętności agenta pozwalają modtools odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
modtools przeszukuje te lokalizacje:
...
W przypadku ścieżek lokalnych projektu modtools przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.modtools/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/modtools/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.modtools/skills/git-release/SKILL.md` w ten sposób:
...
modtools wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `modtools.json`:
...
**Dla agentów wbudowanych** (w `modtools.json`):
