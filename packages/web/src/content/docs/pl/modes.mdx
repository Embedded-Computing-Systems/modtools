...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji modtools.
...
te lub skonfiguruj własne za pomocą konfiguracji modtools.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.modtools/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `modtools.json`:
...
- Globalnie: `~/.config/modtools/modes/`
...
Jeśli nie ma wpływu na temperaturę, modtools stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna modtools, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.modtools/modes/` dla trybów zapisanych dla projektu lub `~/.config/modtools/modes/` dla trybów globalnych:
