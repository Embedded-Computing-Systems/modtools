...
Aby debugować problemy z modtools, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\modtools\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `modtools --log-level DEBUG`.

---

## Przechowywanie danych

modtools przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/modtools/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\modtools`
...
modtools Desktop uruchamia w tle lokalny serwer modtools (sidecar `modtools-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Model-of-Design Tools` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/modtools/modtools.jsonc` (lub `~/.config/modtools/modtools.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/modtools/modtools.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\modtools\modtools.jsonc`
...
modtools może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/modtools/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\modtools\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby modtools mógł ją odbudować.

1. Całkowicie zamknij modtools Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/modtools`
- **Linux**: usuń `~/.cache/modtools` (lub uruchom `rm -rf ~/.cache/modtools`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\modtools`

3. Uruchom ponownie modtools Desktop.

---

### Napraw problemy z połączeniem z serwerem

modtools Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `modtools.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `MOD_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `MOD_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows modtools Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami modtools.

---

### Brak powiadomień

modtools Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla modtools w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie modtools Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji modtools Desktop):

- `modtools.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `modtools.global.dat` i `modtools.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z modtools:
...
3. Upewnij się, że masz najnowszą wersję z `modtools upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `modtools models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\modtools`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. modtools dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\modtools`

2. Uruchom ponownie kod modtools, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to modtools do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
modtools wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
