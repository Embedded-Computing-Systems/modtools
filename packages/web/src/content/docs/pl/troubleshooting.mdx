...
Aby debugować problemy z MOD, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\MOD\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `MOD --log-level DEBUG`.

---

## Przechowywanie danych

MOD przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/MOD/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\MOD`
...
MOD Desktop uruchamia w tle lokalny serwer MOD (sidecar `MOD-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `MOD` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/MOD/MOD.jsonc` (lub `~/.config/MOD/MOD.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/MOD/MOD.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\MOD\MOD.jsonc`
...
MOD może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/MOD/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\MOD\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby MOD mógł ją odbudować.

1. Całkowicie zamknij MOD Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/MOD`
- **Linux**: usuń `~/.cache/MOD` (lub uruchom `rm -rf ~/.cache/MOD`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\MOD`

3. Uruchom ponownie MOD Desktop.

---

### Napraw problemy z połączeniem z serwerem

MOD Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `MOD.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `MODTOOLS_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `MODTOOLS_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows MOD Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami MOD.

---

### Brak powiadomień

MOD Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla MOD w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie MOD Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji MOD Desktop):

- `MOD.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `MOD.global.dat` i `MOD.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `MOD upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `MOD models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\MOD`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. MOD dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\MOD`

2. Uruchom ponownie kod MOD, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to MOD do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
MOD wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
