...
W Model-of-Design Tools obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
Model-of-Design Tools zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
Model-of-Design Tools ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

Model-of-Design Tools ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `modtools.json`:
...
- Globalnie: `~/.config/modtools/agents/`
...
Jeśli nie zostanie skonfigurowana, Model-of-Design Tools używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji Model-of-Design Tools, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji Model-of-Design Tools w `provider/model-id`. Na przykład, [Model-of-Design Tools Zen](/docs/zen), użyj `modtools/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `modtools models`, aby zobaczyć dostępne modele.
:::
