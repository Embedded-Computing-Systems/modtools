...
Dzięki modtools możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie modtools używa naszego własnego motywu `modtools`.
...
modtools ma kilka wbudowanych motywów.
...
- Chcą, aby Model-of-Design Tools pasował do wyglądu ich terminala
...
modtools obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/modtools/themes/*.json` lub `$XDG_CONFIG_HOME/modtools/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.modtools/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.modtools/themes/*.json`
