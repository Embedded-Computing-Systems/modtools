...
Dzięki MOD możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie MOD używa naszego własnego motywu `MOD`.
...
MOD ma kilka wbudowanych motywów.
...
- Chcą, aby Mod pasował do wyglądu ich terminala
...
MOD obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/MOD/themes/*.json` lub `$XDG_CONFIG_HOME/MOD/themes/*.json`
