---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć MOD.
---

Wtyczki umożliwiające wyłączenie MOD poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania MOD.
...
- `~/.config/MOD/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/MOD/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/MOD/MOD.json`)
2. Konfiguracja projektu (`MOD.json`)
3. Globalny katalog wtyczek (`~/.config/MOD/plugins/`)
...
MOD uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z MOD do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do MOD.
...
:::note
Jeśli korzystasz z aplikacji komputerowej MOD, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj MOD czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do MOD:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod MOD. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
