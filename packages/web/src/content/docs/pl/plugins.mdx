---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć modtools.
---

Wtyczki umożliwiające wyłączenie modtools poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania modtools.
...
- `.modtools/plugins/` - Wtyczki na poziomie projektu
- `~/.config/modtools/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/modtools/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/modtools/modtools.json`)
2. Konfiguracja projektu (`modtools.json`)
3. Globalny katalog wtyczek (`~/.config/modtools/plugins/`)
4. Katalog wtyczek projektu (`.modtools/plugins/`)
...
modtools uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z modtools do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do modtools.
...
:::note
Jeśli korzystasz z aplikacji komputerowej modtools, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj modtools czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do modtools:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod modtools. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://modtools.ai/docs/sdk).
