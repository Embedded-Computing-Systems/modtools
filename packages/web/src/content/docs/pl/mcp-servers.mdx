...
Możesz dodać dodatkowe informacje do modtools za pomocą _Model Context Protocol_, w skrócie MCP. modtools obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [modtools Config](https://modtools.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/modtools`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
modtools automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, modtools:
...
Jeśli serwer wymaga uwierzytelnienia, modtools poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `modtools mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji modtools bezpieczne przechowa tokeny w `~/.local/share/modtools/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w modtools, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie modtools, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć modtools z kontem Sentry.
