...
Możesz dodać dodatkowe informacje do MOD za pomocą _Model Context Protocol_, w skrócie MCP. MOD obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [MOD Config](https://modtools.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/MOD`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
MOD automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, MOD:
...
Jeśli serwer wymaga uwierzytelnienia, MOD poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `MOD mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji MOD bezpieczne przechowa tokeny w `~/.local/share/MOD/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w MOD, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie MOD, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć MOD z kontem Sentry.
