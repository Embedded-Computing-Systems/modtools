---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w MOD.
---

import config from "../../../../config.mjs"
export const console = config.console

MOD używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji MOD.
...
w `~/.local/share/MOD/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim MOD
config.
...
MOD Zen to lista modeli dostarczonych przez zespół MOD, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z MOD. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od MOD Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `MOD Zen` i przejdź do [modtools.ai/auth](https://opencode.ai/zen).
...
Działa jak każdy inny dostawca w MOD i jest całkowicie opcjonalny w użyciu.

---

## MOD Go

MOD Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół MOD, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z MOD.

1. Uruchom polecenie `/connect` w TUI, wybierz `MOD Go` i przejdź do [modtools.ai/auth](https://opencode.ai/zen).
...
Aby używać Amazon Bedrock z MOD:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania MOD:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `MOD.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w MOD nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować MOD do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod MOD działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji MOD.
...
MOD używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować MOD
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`MOD.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [MOD-gitlab-auth](https://www.npmjs.com/package/MOD-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem MOD:
...
Aby używać Google Vertex AI z MOD:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania MOD.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą MOD, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [MOD-helicone-session](https://github.com/Embedded-Computing-Systems/modtools-helicone-session), aby automatycznie rejestrować każdą konwersację MOD jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja MOD będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować MOD tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować MOD tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować MOD tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem MOD. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/MOD).
:::
...
Aby korzystać z Ollama Cloud z MOD:
...
3. Skopiuj klucz API do użycia w MOD.
...
6. **Ważne**: Przed użyciem modeli chmurowych w MOD musisz pobrać informacje o modelu lokalnie:
...
MOD Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół MOD. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>MOD Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **MOD Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API MOD.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji MOD.
...
5. Możesz je także dostosować za pomocą konfiguracji MOD. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z MOD:
...
Możesz także dostosować modele za pomocą konfiguracji MOD. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z MOD. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `MOD.json` w katalogu projektu:
...
Pola `limit` pozwalają MOD zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `MOD auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację MOD i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji MOD.
