---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w modtools.
---

import config from "../../../../config.mjs"
export const console = config.console

modtools używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji modtools.
...
w `~/.local/share/modtools/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim modtools
config.
...
Model-of-Design Tools Zen to lista modeli dostarczonych przez zespół modtools, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z modtools. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Model-of-Design Tools Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Model-of-Design Tools Zen` i przejdź do [modtools.ai/auth](https://modtools.ai/zen).
...
Działa jak każdy inny dostawca w modtools i jest całkowicie opcjonalny w użyciu.

---

## Model-of-Design Tools Go

Model-of-Design Tools Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół modtools, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z modtools.

1. Uruchom polecenie `/connect` w TUI, wybierz `Model-of-Design Tools Go` i przejdź do [modtools.ai/auth](https://modtools.ai/zen).
...
Aby używać Amazon Bedrock z modtools:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania modtools:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `modtools.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w modtools nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod modtools działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji modtools.
...
modtools używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować modtools
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`modtools.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [modtools-gitlab-auth](https://www.npmjs.com/package/modtools-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem modtools:
...
Aby używać Google Vertex AI z modtools:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania modtools.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą modtools, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [modtools-helicone-session](https://github.com/H2Shami/modtools-helicone-session), aby automatycznie rejestrować każdą konwersację modtools jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja modtools będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować modtools tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować modtools tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować modtools tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem modtools. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/modtools).
:::
...
Aby korzystać z Ollama Cloud z modtools:
...
3. Skopiuj klucz API do użycia w modtools.
...
6. **Ważne**: Przed użyciem modeli chmurowych w modtools musisz pobrać informacje o modelu lokalnie:
...
Model-of-Design Tools Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół modtools. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Model-of-Design Tools Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Model-of-Design Tools Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API modtools.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji modtools.
...
5. Możesz je także dostosować za pomocą konfiguracji modtools. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z modtools:
...
Możesz także dostosować modele za pomocą konfiguracji modtools. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z modtools. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `modtools.json` w katalogu projektu:
...
Pola `limit` pozwalają modtools zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `modtools auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację modtools i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji modtools.
