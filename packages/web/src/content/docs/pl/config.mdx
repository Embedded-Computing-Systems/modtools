---
title: Konfiguracja
description: Ustawienia Model-of-Design Tools w pliku JSON.
---

Możesz dostosować Model-of-Design Tools za pomocą pliku konfiguracyjnego JSON.

---

## Format

Model-of-Design Tools obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "modtools"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/modtools`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/modtools/modtools.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `MOD_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`modtools.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.modtools`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `MOD_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.modtools` i `~/.config/modtools` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/modtools`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację Model-of-Design Tools w `~/.config/modtools/modtools.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/modtools/tui.json`.
...
Dodaj `modtools.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy Model-of-Design Tools się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `MOD_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `MOD_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.modtools` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.modtools`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`modtools.ai/config.json`**](https://modtools.ai/config.json).
...
Użyj `MOD_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `modtools.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `modtools serve` i `modtools web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer Model-of-Design Tools.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `modtools.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie Model-of-Design Tools wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/modtools/agents/` lub `.modtools/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, Model-of-Design Tools powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`modtools run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/modtools/commands/` lub `.modtools/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
Model-of-Design Tools może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy Model-of-Design Tools nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie Model-of-Design Tools **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają Model-of-Design Tools o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.modtools/plugins/` lub `~/.config/modtools/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie Model-of-Design Tools tylko do określonych dostawców.
