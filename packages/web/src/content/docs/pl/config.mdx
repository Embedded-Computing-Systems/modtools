...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/MOD/agents/` lub `.mod/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/MOD/commands/` lub `.mod/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
Umieść pliki wtyczek w `.mod/plugins/` lub `~/.config/MOD/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
