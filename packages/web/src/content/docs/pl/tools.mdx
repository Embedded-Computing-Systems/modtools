...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. MOD posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w MOD.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `MODTOOLS_EXPERIMENTAL_LSP_TOOL=true` (lub `MODTOOLS_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy MOD lub gdy zmienna środowiskowa `MODTOOLS_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu MOD:
