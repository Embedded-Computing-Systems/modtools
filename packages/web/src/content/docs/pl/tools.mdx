...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Model-of-Design Tools posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Model-of-Design Tools.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `MOD_EXPERIMENTAL_LSP_TOOL=true` (lub `MOD_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Model-of-Design Tools lub gdy zmienna środowiskowa `MOD_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Model-of-Design Tools:
