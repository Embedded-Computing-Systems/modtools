...
With Mod you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Mod uses our own `MOD` theme.
...
Mod comes with several built-in themes.
...
- Want Mod to match their terminal's appearance
...
Mod supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/MOD/themes/*.json` or `$XDG_CONFIG_HOME/MOD/themes/*.json`
