...
With Model-of-Design Tools you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Model-of-Design Tools uses our own `modtools` theme.
...
Model-of-Design Tools comes with several built-in themes.
...
- Want Model-of-Design Tools to match their terminal's appearance
...
Model-of-Design Tools supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/modtools/themes/*.json` or `$XDG_CONFIG_HOME/modtools/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.modtools/themes/*.json`
4. **Current working directory** - Defined in `./.modtools/themes/*.json`
