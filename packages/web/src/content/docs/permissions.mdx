...
Model-of-Design Tools uses the `permission` config to decide whether a given action should run automatically, prompt you, or be blocked.
...
Use `external_directory` to allow tool calls that touch paths outside the working directory where Model-of-Design Tools was started. This applies to any tool that takes a path as input (for example `read`, `edit`, `list`, `glob`, `grep`, and many `bash` commands).
...
Model-of-Design Tools permissions are keyed by tool name, plus a couple of safety guards:
...
If you don’t specify anything, Model-of-Design Tools starts from permissive defaults:
...
When Model-of-Design Tools prompts for approval, the UI offers three outcomes:

- `once` — approve just this request
- `always` — approve future requests matching the suggested patterns (for the rest of the current Model-of-Design Tools session)
