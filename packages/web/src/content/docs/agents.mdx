...
There are two types of agents in Model-of-Design Tools; primary agents and subagents.
...
Model-of-Design Tools comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Model-of-Design Tools comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

Model-of-Design Tools comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `modtools.json` config file:
...
If no temperature is specified, Model-of-Design Tools uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Model-of-Design Tools config and the project specific config.
...
The model ID in your Model-of-Design Tools config uses the format `provider/model-id`. For example, if you're using [Model-of-Design Tools Zen](/docs/zen), you would use `modtools/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `modtools models` to see a list of the available models.
:::
