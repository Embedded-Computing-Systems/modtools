...
modtools ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\modtools\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `modtools --log-level DEBUG`.

---

## Depolama

modtools, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/modtools/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\modtools` yapistirin
...
modtools Desktop arka planda yerel bir modtools sunucusu (`modtools-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `modtools` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/modtools/modtools.jsonc` (veya `~/.config/modtools/modtools.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/modtools/modtools.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\modtools\modtools.jsonc` yapistirin
...
modtools diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/modtools/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\modtools\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), modtools'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. modtools Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/modtools`
- **Linux**: `~/.cache/modtools` dizinini silin (veya `rm -rf ~/.cache/modtools` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\modtools` yapistirin

3. modtools Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

modtools Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`modtools.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `MOD_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `MOD_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta modtools Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, modtools ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

modtools Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda modtools icin bildirimler etkinse
...
1. modtools Desktop'u kapatin
2. Su dosyalari bulun ve silin (modtools Desktop uygulama veri dizininde yer alirlar):

- `modtools.settings.dat` (desktop varsayilan sunucu URL'si)
- `modtools.global.dat` ve `modtools.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
modtools ile ilgili bir sorun yasiyorsaniz:
...
3. `modtools upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `modtools models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\modtools`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. modtools, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\modtools`

2. En guncel provider paketlerini yeniden kurmak icin modtools'u yeniden baslatin

Bu, modtools'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
modtools, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
