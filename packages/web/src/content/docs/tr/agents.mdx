...
modtools'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
modtools, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
modtools, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

modtools iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `modtools.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse modtools modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global modtools yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
modtools yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [Model-of-Design Tools Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `modtools/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `modtools models` komutunu çalıştırın.
:::
