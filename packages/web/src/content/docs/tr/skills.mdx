...
Ajan becerileri, MOD'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
MOD su konumlari tarar:

- Proje konfigurasyonu: `.modtools/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/MOD/skills/<name>/SKILL.md`
...
Proje yerel yollarinda MOD, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
...
Genel tanimlar da `~/.config/MOD/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
MOD, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`MOD.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`MOD.json` icinde):
