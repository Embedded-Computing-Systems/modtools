---
title: Sunucu
description: modtools sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`modtools serve` komutu, modtools istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `modtools.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `MOD_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `modtools` değeridir; değiştirmek isterseniz `MOD_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `modtools serve` hem de `modtools web` için geçerlidir.
...
`modtools` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
modtools ile programatik etkileşim için modtools sunucusunu kullanın.
:::

Bu mimari, modtools'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`modtools serve` ile bağımsız bir sunucu başlatabilirsiniz. modtools TUI açıksa, `modtools serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum modtools [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
modtools sunucusu aşağıdaki API'leri sunar.
