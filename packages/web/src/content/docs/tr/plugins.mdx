---
title: Eklentiler
description: Model-of-Design Tools'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Model-of-Design Tools'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Model-of-Design Tools'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.modtools/plugins/` - Proje düzeyinde eklentiler
- `~/.config/modtools/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/modtools/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/modtools/modtools.json`)
2. Project config (`modtools.json`)
3. Global eklenti dizini (`~/.config/modtools/plugins/`)
4. Proje eklenti dizini (`.modtools/plugins/`)
...
Model-of-Design Tools bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Model-of-Design Tools SDK istemcisi.
...
Model-of-Design Tools'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Model-of-Design Tools masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Model-of-Design Tools'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Model-of-Design Tools'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Model-of-Design Tools'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Model-of-Design Tools'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://modtools.ai/docs/sdk) bakın.
