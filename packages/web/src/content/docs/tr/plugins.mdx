---
title: Eklentiler
description: Mod'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Mod'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Mod'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `~/.config/MOD/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/MOD/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/MOD/MOD.json`)
2. Project config (`MOD.json`)
3. Global eklenti dizini (`~/.config/MOD/plugins/`)
...
Mod bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Mod SDK istemcisi.
...
Mod'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Mod masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Mod'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Mod'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Mod'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Mod'a da hazır olacaktır.
