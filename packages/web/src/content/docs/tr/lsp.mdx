...
description: MOD, LSP sunucularınızla bütünleşir.
---

MOD, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

MOD, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`MODTOOLS_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve MOD bir dosyayı açtığında:
...
LSP sunucularını, MOD yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
