...
:::caution
Modlar artık modtools yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

modtools'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya modtools yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
modtools iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.modtools/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`modtools.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, modtools modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global modtools yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.modtools/modes/` veya genel modlar için `~/.config/modtools/modes/`'de mod dosyaları oluşturun:
