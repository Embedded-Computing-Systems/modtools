...
Model-of-Design Tools Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

:::note
Model-of-Design Tools Go şu anda beta olarak mevcut.
:::

Go, Model-of-Design Tools'daki diğer sağlayıcılar gibi çalışır. Model-of-Design Tools Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Model-of-Design Tools'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Model-of-Design Tools Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Model-of-Design Tools Go, Model-of-Design Tools'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Model-of-Design Tools Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Model-of-Design Tools Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Model-of-Design Tools Go'ya abone olabilir.
:::
...
Model-of-Design Tools Go aşağıdaki limitleri içerir:
...
Model-of-Design Tools yapılandırmanızdaki [model id](/docs/config/#models) formatı `modtools-go/<model-id>` şeklindedir. Örneğin, Kimi K2.5 için yapılandırmanızda `modtools-go/kimi-k2.5` kullanmalısınız.
...
Model-of-Design Tools Go'yu şu amaçlarla oluşturduk:
...
4. Model-of-Design Tools ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
