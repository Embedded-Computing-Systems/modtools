...
Mod Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, Mod'daki diğer sağlayıcılar gibi çalışır. Mod Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Mod'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Mod Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Mod Go, Mod'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Mod Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Mod Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Mod Go'ya abone olabilir.
:::
...
Mod Go aşağıdaki limitleri içerir:
...
Mod yapılandırmanızdaki [model id](/docs/config/#models) formatı `MOD-go/<model-id>` şeklindedir. Örneğin, Kimi K2.7 Code için yapılandırmanızda `MOD-go/kimi-k2.7-code` kullanmalısınız.
...
Mod Go'yu şu amaçlarla oluşturduk:
...
4. Mod ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
