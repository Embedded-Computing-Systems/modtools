---
title: Sağlayıcılar
description: MOD'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

MOD, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı MOD yapılandırmanızda yapılandırın.
...
`~/.local/share/MOD/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları MOD'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
MOD Zen, MOD ekibi tarafından sağlanan modellerin bir listesidir.
MOD ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz MOD Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, MOD'u seçin ve [modtools.ai/auth](https://modtools.ai/auth)'ye gidin.
...
MOD'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## MOD Go

MOD Go, MOD ile iyi çalıştığı test edilmiş ve doğrulanmış, MOD ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `MOD Go`'yu seçin ve [modtools.ai/auth](https://opencode.ai/zen) adresine gidin.
...
Amazon Bedrock'u MOD ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   MOD'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `MOD.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi MOD'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
MOD'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   MOD'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Mod yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
MOD, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. MOD'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`MOD.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [MOD-gitlab-auth](https://www.npmjs.com/package/MOD-gitlab-auth) ana sayfasında.

##### Yapılandırma

`MOD.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi MOD'la kullanmak için:
...
Google Vertex AI'yi MOD ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları MOD çalıştırılırken ayarlayın.
...
Helicone'un MOD aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her MOD görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [MOD-helicone-session](https://github.com/Embedded-Computing-Systems/modtools-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her MOD görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
MOD'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla MOD'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla MOD'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini MOD için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/MOD)'a bakın.
:::
...
Ollama Cloud'u MOD ile kullanmak için:
...
3. MOD'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: MOD'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
MOD Zen, MOD ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>MOD Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **MOD Zen**'i arayın.

   ```txt
   /connect
   ```

3. MOD API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   MOD yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca MOD yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Mod ile kullanmak için:
...
Modelleri MOD yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı MOD ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `MOD.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları MOD'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `MOD auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için MOD yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, MOD yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
