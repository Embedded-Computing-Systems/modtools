---
title: Sağlayıcılar
description: modtools'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

modtools, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı modtools yapılandırmanızda yapılandırın.
...
`~/.local/share/modtools/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları modtools'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Model-of-Design Tools Zen, modtools ekibi tarafından sağlanan modellerin bir listesidir.
modtools ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Model-of-Design Tools Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, modtools'u seçin ve [modtools.ai/auth](https://modtools.ai/auth)'ye gidin.
...
modtools'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Model-of-Design Tools Go

Model-of-Design Tools Go, modtools ile iyi çalıştığı test edilmiş ve doğrulanmış, modtools ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Model-of-Design Tools Go`'yu seçin ve [modtools.ai/auth](https://modtools.ai/zen) adresine gidin.
...
Amazon Bedrock'u modtools ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   modtools'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `modtools.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi modtools'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   modtools'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Modtools yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
modtools, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. modtools'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`modtools.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [modtools-gitlab-auth](https://www.npmjs.com/package/modtools-gitlab-auth) ana sayfasında.

##### Yapılandırma

`modtools.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi modtools'la kullanmak için:
...
Google Vertex AI'yi modtools ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları modtools çalıştırılırken ayarlayın.
...
Helicone'un modtools aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her modtools görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [modtools-helicone-session](https://github.com/H2Shami/modtools-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her modtools görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
modtools'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla modtools'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla modtools'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini modtools için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/modtools)'a bakın.
:::
...
Ollama Cloud'u modtools ile kullanmak için:
...
3. modtools'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: modtools'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Model-of-Design Tools Zen, modtools ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Model-of-Design Tools Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Model-of-Design Tools Zen**'i arayın.

   ```txt
   /connect
   ```

3. modtools API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   modtools yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca modtools yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Modtools ile kullanmak için:
...
Modelleri modtools yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı modtools ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `modtools.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları modtools'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `modtools auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için modtools yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, modtools yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
