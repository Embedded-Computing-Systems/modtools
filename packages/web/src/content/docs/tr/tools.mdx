...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. modtools yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
modtools içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `MOD_EXPERIMENTAL_LSP_TOOL=true` (veya `MOD_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca modtools provider kullanılırken veya `MOD_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

modtools başlatırken etkinleştirmek için:
