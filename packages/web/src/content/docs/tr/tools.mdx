...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. MOD yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
MOD içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `MODTOOLS_EXPERIMENTAL_LSP_TOOL=true` (veya `MODTOOLS_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca MOD provider kullanılırken veya `MODTOOLS_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

MOD başlatırken etkinleştirmek için:
