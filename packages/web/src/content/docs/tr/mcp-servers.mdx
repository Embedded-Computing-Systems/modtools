...
_Model Bağlam Protokolü_ veya MCP kullanarak MOD'a harici araçlar ekleyebilirsiniz. MOD hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [MOD Config](https://modtools.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/MOD` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
MOD, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde MOD şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa MOD, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `MOD mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından MOD, belirteçleri `~/.local/share/MOD/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra MOD'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi MOD yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve MOD'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
