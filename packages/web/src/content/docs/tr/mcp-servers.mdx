...
_Model Bağlam Protokolü_ veya MCP kullanarak modtools'a harici araçlar ekleyebilirsiniz. modtools hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [modtools Config](https://modtools.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/modtools` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
modtools, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde modtools şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa modtools, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `modtools mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından modtools, belirteçleri `~/.local/share/modtools/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra modtools'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi modtools yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve modtools'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
