...
description: modtools dile özgü biçimlendiriciler kullanır.
---

modtools, dosyaları dile özgü formatlayıcılar kullanılarak yazıldıktan veya düzenlendikten sonra otomatik olarak formatlar. Bu, oluşturulan kodun projenizin kod stillerine uymasını sağlar.

---

## Yerleşik

modtools, popüler diller ve çerçeveler için çeşitli yerleşik biçimlendiricilerle birlikte gelir. Aşağıda ihtiyaç duyduğu biçimlendiricilerin, desteklenen dosya uzantılarının ve komutların veya yapılandırma seçeneklerinin bir listesi bulunmaktadır.
...
Yani eğer projenizin `package.json` dosyasında `prettier` varsa, modtools bunu otomatik olarak kullanacaktır.

---

## Nasıl çalışır?

modtools bir dosyayı yazdığında veya düzenlediğinde:
...
Biçimlendiricileri modtools yapılandırmanızdaki `formatter` bölümü aracılığıyla özelleştirebilirsiniz.
