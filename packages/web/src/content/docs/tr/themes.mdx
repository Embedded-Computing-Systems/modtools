...
MOD ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak MOD kendi `MOD` temasını kullanır.
...
MOD birden fazla yerleşik temayla gelir.
...
- MOD'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
MOD, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/MOD/themes/*.json` veya `$XDG_CONFIG_HOME/MOD/themes/*.json`
