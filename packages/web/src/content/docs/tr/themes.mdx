...
modtools ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak modtools kendi `modtools` temasını kullanır.
...
modtools birden fazla yerleşik temayla gelir.
...
- modtools'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
modtools, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/modtools/themes/*.json` veya `$XDG_CONFIG_HOME/modtools/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.modtools/themes/*.json`
4. **Geçerli çalışma dizini** - `./.modtools/themes/*.json`
