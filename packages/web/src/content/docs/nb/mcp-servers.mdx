...
Du kan legge til eksterne verktøy i Model-of-Design Tools ved å bruke _Model Context Protocol_ eller MCP. Model-of-Design Tools støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Model-of-Design Tools Config](https://modtools.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="modtools.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/modtools`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Model-of-Design Tools håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Model-of-Design Tools:
...
Hvis serveren krever autentisering, vil Model-of-Design Tools be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `modtools mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Model-of-Design Tools lagre tokens sikkert i `~/.local/share/modtools/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i Model-of-Design Tools, sammen med innebygde verktøy. Så du kan administrere dem gjennom Model-of-Design Tools-konfigurasjonen som alle andre verktøy.
...
```json title="modtools.json" {11, 14-18}
{
  "$schema": "https://modtools.ai/config.json",
  "mcp": {
    "my-mcp": {
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Model-of-Design Tools til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Model-of-Design Tools-samtale oppført som en separat økt.
