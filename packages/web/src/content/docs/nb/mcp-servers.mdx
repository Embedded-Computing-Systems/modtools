...
Du kan legge til eksterne verktøy i MOD ved å bruke _Model Context Protocol_ eller MCP. MOD støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [MOD Config](https://modtools.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="MOD.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/MOD`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
MOD håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil MOD:
...
Hvis serveren krever autentisering, vil MOD be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `MOD mcp auth <server-name>`.
...
```bash
MOD mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil MOD lagre tokens sikkert i `~/.local/share/MOD/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i MOD, sammen med innebygde verktøy. Så du kan administrere dem gjennom MOD-konfigurasjonen som alle andre verktøy.
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble MOD til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver MOD-samtale oppført som en separat økt.
