---
title: Konfigurasjon
description: Ved å bruke modtools JSON-konfigurasjonen.
---

Du kan konfigurere Model-of-Design Tools ved å bruke en JSON konfigurasjonsfil.

---

## Format

Model-of-Design Tools støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis din globale konfigurasjon setter `theme: "modtools"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/modtools`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/modtools/modtools.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`MOD_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`modtools.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.modtools` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`MOD_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.modtools` og `~/.config/modtools` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/modtools`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale Model-of-Design Tools-konfigurasjon i `~/.config/modtools/modtools.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `modtools.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når Model-of-Design Tools starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `MOD_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `MOD_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.modtools` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.modtools`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`modtools.ai/config.json`**](https://modtools.ai/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `modtools serve` og `modtools web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage Model-of-Design Tools-serveren din.
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `modtools.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan konfigurere leverandørene og modellene du vil bruke i Model-of-Design Tools-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver Model-of-Design Tools å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/modtools/agents/` eller `.modtools/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil Model-of-Design Tools falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`modtools run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/modtools/commands/` eller `.modtools/commands/`. [Les mer her](/docs/commands).
...
Model-of-Design Tools vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater Model-of-Design Tools alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider Model-of-Design Tools med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.modtools/plugins/` eller `~/.config/modtools/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense Model-of-Design Tools til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
