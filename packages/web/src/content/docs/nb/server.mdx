---
title: Tjener
description: Samhandle med Model-of-Design Tools-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `modtools serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Model-of-Design Tools-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `modtools.local` |
...
Sett `MOD_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `modtools` som standard, eller sett `MOD_SERVER_USERNAME` for å overstyre det. Dette gjelder både `modtools serve` og `modtools web`.
...
Når du kjører `modtools` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Model-of-Design Tools-serveren til å samhandle med Model-of-Design Tools programmatisk.
:::

Denne arkitekturen lar Model-of-Design Tools støtte flere klienter og lar deg samhandle med Model-of-Design Tools programmatisk.

Du kan kjøre `modtools serve` for å starte en frittstående server. Hvis du har
Model-of-Design Tools TUI kjører, vil `modtools serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Model-of-Design Tools [IDE](/docs/ide) plugins.
...
modtools-serveren viser følgende APIer.
