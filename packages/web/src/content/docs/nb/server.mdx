---
title: Tjener
description: Samhandle med Mod-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `MOD serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Mod-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `MOD.local` |
...
Sett `MODTOOLS_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `MOD` som standard, eller sett `MODTOOLS_SERVER_USERNAME` for å overstyre det. Dette gjelder både `MOD serve` og `MOD web`.
...
Når du kjører `MOD` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Mod-serveren til å samhandle med Mod programmatisk.
:::

Denne arkitekturen lar Mod støtte flere klienter og lar deg samhandle med Mod programmatisk.

Du kan kjøre `MOD serve` for å starte en frittstående server. Hvis du har
Mod TUI kjører, vil `MOD serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Mod [IDE](/docs/ide) plugins.
...
MOD-serveren viser følgende APIer.
