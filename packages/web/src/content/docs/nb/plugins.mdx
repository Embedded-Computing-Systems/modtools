...
description: Skriv dine egne programtillegg for å utvide Model-of-Design Tools.
---

Plugins lar deg utvide Model-of-Design Tools ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Model-of-Design Tools.
...
- `.modtools/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/modtools/node_modules/`.
...
1. Global konfigurasjon (`~/.config/modtools/modtools.json`)
2. Prosjektkonfigurasjon (`modtools.json`)
3. Global plugin-katalog (`~/.config/modtools/plugins/`)
4. Prosjektpluginkatalog (`.modtools/plugins/`)
...
Model-of-Design Tools kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Model-of-Design Tools SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Model-of-Design Tools.
...
:::note
Hvis du bruker Model-of-Design Tools-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Model-of-Design Tools fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Model-of-Design Tools:
...
`tool`-hjelperen lager et tilpasset verktøy som Model-of-Design Tools kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Model-of-Design Tools sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://modtools.ai/docs/sdk) for detaljer.
