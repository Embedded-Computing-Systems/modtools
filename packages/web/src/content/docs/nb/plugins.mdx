...
description: Skriv dine egne programtillegg for å utvide Mod.
---

Plugins lar deg utvide Mod ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Mod.
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/MOD/node_modules/`.
...
1. Global konfigurasjon (`~/.config/MOD/MOD.json`)
2. Prosjektkonfigurasjon (`MOD.json`)
3. Global plugin-katalog (`~/.config/MOD/plugins/`)
...
Mod kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Mod SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Mod.
...
:::note
Hvis du bruker Mod-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Mod fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Mod:
...
`tool`-hjelperen lager et tilpasset verktøy som Mod kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Mod sammen med innebygde verktøy.
