...
De fleste populære leverandørene er forhåndslastet som standard. Hvis du har lagt til legitimasjonen for en leverandør gjennom kommandoen `/connect`, vil den være tilgjengelig når du starter Model-of-Design Tools.
...
Her er flere modeller som fungerer godt med Model-of-Design Tools, uten spesiell rekkefølge. (Dette er ikke en uttømmende liste og er heller ikke nødvendigvis oppdatert):
...
Model-of-Design Tools-konfigurasjon.
...
Her er hele ID `provider_id/model_id`. For eksempel, hvis du bruker [Model-of-Design Tools Zen](/docs/zen), vil du bruke `modtools/gpt-5.1-codex` for GPT 5.1 Codex.
...
Mange modeller støtter flere varianter med forskjellige konfigurasjoner. modtools leveres med innebygde standardvarianter for populære leverandører.

### Innebygde varianter

Model-of-Design Tools leveres med standardvarianter for mange leverandører:
...
Når Model-of-Design Tools starter opp, ser den etter modeller i følgende prioriterte rekkefølge:

1. Kommandolinjeflagget `--model` eller `-m`. Formatet er det samme som i konfigurasjonsfilen: `provider_id/model_id`.

2. Modelllisten i Model-of-Design Tools-konfigurasjonen.
