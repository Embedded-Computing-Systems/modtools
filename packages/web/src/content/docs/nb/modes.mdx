...
:::caution
Moduser konfigureres nå gjennom alternativet `agent` i Model-of-Design Tools-konfigurasjonen.
Alternativet `mode` er nå utdatert. [Finn ut mer](/docs/agents).
:::

Moduser i Model-of-Design Tools lar deg tilpasse oppførselen, verktøyene og prompter for ulike brukstilfeller.

Den kommer med to innebygde moduser: **bygg** og **plan**. Du kan tilpasse
disse eller konfigurer din egen gjennom Model-of-Design Tools-konfigurasjonen.
...
Model-of-Design Tools kommer med to innebygde moduser.
...
- `edit` - Kan ikke endre eksisterende filer, bortsett fra filer som ligger på `.modtools/plans/*.md` for å detaljere selve planen
...
Konfigurer moduser i din `modtools.json` konfigurasjonsfil:
...
- Globalt: `~/.config/modtools/modes/`
- Prosjekt: `.modtools/modes/`
...
Hvis ingen temperatur er spesifisert, bruker Model-of-Design Tools modellspesifikke standardinnstillinger (vanligvis 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
Denne banen er relativ til der konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Model-of-Design Tools-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Opprett modusfiler i `.modtools/modes/` for prosjektspesifikke moduser eller `~/.config/modtools/modes/` for globale moduser:
