---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Model-of-Design Tools.
---

import config from "../../../../config.mjs"
export const console = config.console

Model-of-Design Tools bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Model-of-Design Tools-konfigurasjonen.
...
i `~/.local/share/modtools/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Model-of-Design Tools
konfig.
...
Model-of-Design Tools Zen er en liste over modeller levert av Model-of-Design Tools-teamet som har vært
testet og verifisert for å fungere godt med Model-of-Design Tools. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Model-of-Design Tools Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Model-of-Design Tools Zen` og gå til [modtools.ai/auth](https://modtools.ai/zen).
...
Det fungerer som alle andre leverandører i Model-of-Design Tools og er helt valgfritt å bruke.

---

## Model-of-Design Tools Go

Model-of-Design Tools Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Model-of-Design Tools-teamet som har vært
testet og verifisert for å fungere godt med Model-of-Design Tools.

1. Kjør kommandoen `/connect` i TUI, velg `Model-of-Design Tools Go`, og gå til [modtools.ai/auth](https://modtools.ai/zen).
...
Slik bruker du Amazon Bedrock med Model-of-Design Tools:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Model-of-Design Tools:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `modtools.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i modtools støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Model-of-Design Tools skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Model-of-Design Tools-konfigurasjonen.
...
2. Velg autentiseringsmetoden din:
   ...
   3. Navn: `Model-of-Design Tools`, omfang: `api`
...
for å lagre token i Model-of-Design Tools auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Model-of-Design Tools bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Model-of-Design Tools
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`modtools.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [modtools-gitlab-auth](https://www.npmjs.com/package/modtools-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `modtools.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Model-of-Design Tools:
...
Slik bruker du Google Vertex AI med Model-of-Design Tools:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Model-of-Design Tools.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Model-of-Design Tools, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [modtools-helicone-session](https://github.com/H2Shami/modtools-helicone-session) plugin for automatisk å logge hver Model-of-Design Tools-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Model-of-Design Tools-samtale oppført som en separat økt.
...
Du kan konfigurere Model-of-Design Tools for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Model-of-Design Tools for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Model-of-Design Tools for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Model-of-Design Tools. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/modtools) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Model-of-Design Tools:
...
3. Kopier API-nøkkelen for bruk i Model-of-Design Tools.
...
6. **Viktig**: Før du bruker skymodeller i Model-of-Design Tools, må du hente modellinformasjonen lokalt:
...
Model-of-Design Tools Zen er en liste over testede og verifiserte modeller levert av Model-of-Design Tools-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Model-of-Design Tools Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Model-of-Design Tools Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Model-of-Design Tools API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Model-of-Design Tools-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Model-of-Design Tools-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Model-of-Design Tools:
...
Du kan også tilpasse modeller gjennom Model-of-Design Tools-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Model-of-Design Tools. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `modtools.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Model-of-Design Tools forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `modtools auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Model-of-Design Tools-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Model-of-Design Tools-konfigurasjonen.
