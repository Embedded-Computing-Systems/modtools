---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i MOD.
---

import config from "../../../../config.mjs"
export const console = config.console

MOD bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i MOD-konfigurasjonen.
...
i `~/.local/share/MOD/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i MOD
konfig.
...
MOD Zen er en liste over modeller levert av MOD-teamet som har vært
testet og verifisert for å fungere godt med MOD. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med MOD Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `MOD Zen` og gå til [modtools.ai/auth](https://opencode.ai/zen).
...
Det fungerer som alle andre leverandører i MOD og er helt valgfritt å bruke.

---

## MOD Go

MOD Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av MOD-teamet som har vært
testet og verifisert for å fungere godt med MOD.

1. Kjør kommandoen `/connect` i TUI, velg `MOD Go`, og gå til [modtools.ai/auth](https://opencode.ai/zen).
...
Slik bruker du Amazon Bedrock med MOD:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører MOD:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `MOD.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i MOD støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere MOD til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at MOD skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom MOD-konfigurasjonen.
...
for å lagre token i MOD auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
MOD bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse MOD
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`MOD.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [MOD-gitlab-auth](https://www.npmjs.com/package/MOD-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `MOD.json`:
...
Slik bruker du GitHub Copilot-abonnementet med MOD:
...
Slik bruker du Google Vertex AI med MOD:
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom MOD, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [MOD-helicone-session](https://github.com/Embedded-Computing-Systems/modtools-helicone-session) plugin for automatisk å logge hver MOD-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver MOD-samtale oppført som en separat økt.
...
Du kan konfigurere MOD for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere MOD for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere MOD for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for MOD. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/MOD) for detaljer.
:::
...
Slik bruker du Ollama Cloud med MOD:
...
3. Kopier API-nøkkelen for bruk i MOD.
...
6. **Viktig**: Før du bruker skymodeller i MOD, må du hente modellinformasjonen lokalt:
...
MOD Zen er en liste over testede og verifiserte modeller levert av MOD-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>MOD Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **MOD Zen**.

   ```txt
   /connect
   ```

3. Skriv inn MOD API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom MOD-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom MOD-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med MOD:
...
Du kan også tilpasse modeller gjennom MOD-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med MOD. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `MOD.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar MOD forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `MOD auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk MOD-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i MOD-konfigurasjonen.
