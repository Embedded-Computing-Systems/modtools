---
title: TUI
description: 使用 Model-of-Design Tools 終端機使用者介面。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Model-of-Design Tools 提供了一個互動式終端機介面（TUI），用於配合 LLM 處理您的專案。

執行 Model-of-Design Tools 即可啟動當前目錄的 TUI。
...
使用 Model-of-Design Tools TUI 時，您可以輸入 `/` 後跟指令名稱來快速執行操作。例如：
...
將供應商新增到 Model-of-Design Tools。允許您從可用的供應商中選擇並新增其 API 金鑰。
...
退出 Model-of-Design Tools。_別名_：`/quit`、`/q`
...
這與設定伺服器/執行時行為的 `modtools.json` 是分開的。
...
使用 `MOD_TUI_CONFIG` 載入自訂 TUI 設定路徑。
