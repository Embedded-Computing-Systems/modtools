---
title: 提供商
description: 在 Model-of-Design Tools 中使用任意 LLM 提供商。
---

import config from "../../../../config.mjs"
export const console = config.console

Model-of-Design Tools 使用 [AI SDK](https://ai-sdk.dev/) 和 [Models.dev](https://models.dev)，支援 **75+ LLM 提供商**，同時也支援執行本地模型。

要新增提供商，您需要：

1. 使用 `/connect` 指令新增提供商的 API 金鑰。
2. 在 Model-of-Design Tools 設定中設定該提供商。
...
`~/.local/share/modtools/auth.json` 中。

---

### 設定

您可以透過 Model-of-Design Tools 設定中的 `provider` 部分來自訂提供商。
...
Model-of-Design Tools Zen 是由 Model-of-Design Tools 團隊提供的模型列表，這些模型已經過測試和驗證，能夠與 Model-of-Design Tools 良好配合使用。[了解更多](/docs/zen)。

:::tip
如果您是新使用者，我們建議從 Model-of-Design Tools Zen 開始。
:::

1. 在 TUI 中執行 `/connect` 指令，選擇 `Model-of-Design Tools Zen`，然後前往 [modtools.ai/auth](https://modtools.ai/zen)。
...
它的使用方式與 Model-of-Design Tools 中的其他提供商完全相同，且完全可選。

---

## Model-of-Design Tools Go

Model-of-Design Tools Go 是一個低成本的訂閱計畫，提供對 Model-of-Design Tools 團隊提供的流行開放編碼模型的可靠存取，這些模型已經過測試和驗證，能夠與 Model-of-Design Tools 良好配合使用。

1. 在 TUI 中執行 `/connect` 指令，選擇 `Model-of-Design Tools Go`，然後前往 [modtools.ai/auth](https://modtools.ai/zen)。
...
它的使用方式與 Model-of-Design Tools 中的任何其他提供商相同，且完全可選。
...
要在 Model-of-Design Tools 中使用 Amazon Bedrock：
...
2. 使用以下方法之一**設定身分驗證**：
   ...
   如需專案級別或持久化的設定，請使用 `modtools.json`：
...
:::info
在 Model-of-Design Tools 中使用 Claude Pro/Max 訂閱不是 [Anthropic](https://anthropic.com) 官方支援的用法。
:::
...
2. 前往 [Azure AI Foundry](https://ai.azure.com/) 並部署一個模型。
   ...
   :::note
   部署名稱必須與模型名稱一致，Model-of-Design Tools 才能正常運作。
   :::
...
5. 執行 `/models` 指令選擇模型。
   ...
   您也可以透過 Model-of-Design Tools 設定新增模型。
...
2. 選擇您的身分驗證方式：
   ...
   2. 點擊 **Add new token**
   3. 名稱填寫 `Model-of-Design Tools`，範圍選擇 `api`
   4. 複製權杖（以 `glpat-` 開頭）
   5. 在終端機中輸入該權杖
...
:::note
您也可以透過指定 `GITLAB_TOKEN` 環境變數來避免將權杖儲存在 Model-of-Design Tools 的認證儲存中。
:::

##### 自架 GitLab

:::note[合規說明]
Model-of-Design Tools 會使用一個小模型來執行部分 AI 任務，例如產生工作階段標題。預設情況下使用由 Zen 託管的 gpt-5-nano。如果您需要讓 Model-of-Design Tools 僅使用您自己的 GitLab 託管實例，請在 `modtools.json` 檔案中新增以下內容。同時建議停用工作階段分享。
...
更多文件請參閱 [modtools-gitlab-auth](https://www.npmjs.com/package/modtools-gitlab-auth) 首頁。

##### 設定

透過 `modtools.json` 進行自訂設定：
...
要在 Model-of-Design Tools 中使用您的 GitHub Copilot 訂閱：
...
要在 Model-of-Design Tools 中使用 Google Vertex AI：
...
如果 Helicone 的某些功能或模型未透過 Model-of-Design Tools 自動設定，您隨時可以手動設定。
...
Helicone 的 [Sessions](https://docs.helicone.ai/features/sessions) 功能允許您將相關的 LLM 請求歸為一組。使用 [modtools-helicone-session](https://github.com/H2Shami/modtools-helicone-session) 外掛可以自動將每個 Model-of-Design Tools 對話記錄為 Helicone 中的一個工作階段。
...
該外掛會在您的請求中注入 `Helicone-Session-Id` 和 `Helicone-Session-Name` 請求標頭。在 Helicone 的 Sessions 頁面中，您可以看到每個 Model-of-Design Tools 對話都作為獨立的工作階段列出。
...
您可以透過 [llama.cpp](https://github.com/ggml-org/llama.cpp) 的 llama-server 工具設定 Model-of-Design Tools 使用本地模型。
...
您可以透過 LM Studio 設定 Model-of-Design Tools 使用本地模型。
...
您可以透過 Ollama 設定 Model-of-Design Tools 使用本地模型。

:::tip
Ollama 可以自動為 Model-of-Design Tools 進行設定。詳見 [Ollama 整合文件](https://docs.ollama.com/integrations/modtools)。
:::
...
6. **重要**：在 Model-of-Design Tools 中使用雲端模型之前，必須先將模型資訊拉取到本地：
...
Model-of-Design Tools Zen 是由 Model-of-Design Tools 團隊提供的經過測試和驗證的模型列表。[了解更多](/docs/zen)。

1. 登入 **<a href={console}>Model-of-Design Tools Zen</a>** 並點擊 **Create API Key**。

2. 執行 `/connect` 指令並搜尋 **Model-of-Design Tools Zen**。

   ```txt
   /connect
   ```

3. 輸入您的 Model-of-Design Tools API 金鑰。
...
4. 預設已預先載入了許多 OpenRouter 模型，執行 `/models` 指令選擇您想要的模型。
   ...
   您也可以透過 Model-of-Design Tools 設定新增更多模型。
...
5. 您還可以透過 Model-of-Design Tools 設定自訂模型。以下是指定提供商的範例：
...
要在 Model-of-Design Tools 中使用 [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)：
...
您也可以透過 Model-of-Design Tools 設定自訂模型。以下是指定提供商路由順序的範例。
...
:::tip
您可以在 Model-of-Design Tools 中使用任何 OpenAI 相容的提供商。大多數現代 AI 提供商都提供 OpenAI 相容的 API。
:::
...
4. 在專案目錄中建立或更新 `modtools.json` 檔案：
...
`limit` 欄位讓 Model-of-Design Tools 了解您還剩餘多少上下文空間。標準提供商會自動從 models.dev 拉取這些資訊。
...
1. **檢查認證設定**：執行 `modtools auth list` 查看該提供商的憑證是否已新增到設定中。

   這不適用於 Amazon Bedrock 等依賴環境變數進行認證的提供商。

2. 對於自訂提供商，請檢查 Model-of-Design Tools 設定並確認：
   - `/connect` 指令中使用的提供商 ID 與 Model-of-Design Tools 設定中的 ID 一致。
