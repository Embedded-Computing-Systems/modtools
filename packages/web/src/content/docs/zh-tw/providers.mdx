...
1. 在 TUI 中執行 `/connect` 指令，選擇 `Mod Zen`，然後前往 [modtools.ai/auth](https://modtools.ai/zen)。
...
1. 在 TUI 中執行 `/connect` 指令，選擇 `Mod Go`，然後前往 [modtools.ai/auth](https://modtools.ai/zen)。
...
Helicone 的 [Sessions](https://docs.helicone.ai/features/sessions) 功能允許您將相關的 LLM 請求歸為一組。使用 [MOD-helicone-session](https://github.com/H2Shami/MOD-helicone-session) 外掛可以自動將每個 Mod 對話記錄為 Helicone 中的一個工作階段。
