...
工具允許 LLM 在您的程式碼庫中執行操作。Model-of-Design Tools 自帶一組內建工具，您也可以透過[自訂工具](/docs/custom-tools)或 [MCP 伺服器](/docs/mcp-servers)來擴充它。
...
以下是 Model-of-Design Tools 中所有可用的內建工具。
...
:::note
該工具僅在設定 `MOD_EXPERIMENTAL_LSP_TOOL=true`（或 `MOD_EXPERIMENTAL=true`）時可用。
:::
...
:::note
該工具僅在使用 Model-of-Design Tools 供應商時，或當 `MOD_ENABLE_EXA` 環境變數設定為任意真值（例如 `true` 或 `1`）時可用。

在啟動 Model-of-Design Tools 時啟用：
