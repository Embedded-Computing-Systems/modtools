...
Model-of-Design Tools 中有兩種類型的代理：主代理和子代理。
...
Model-of-Design Tools 內建了兩個主代理：**Build** 和 **Plan**。我們將在下面介紹它們。
...
Model-of-Design Tools 內建了兩個子代理：**General** 和 **Explore**。我們將在下面介紹它們。

---

## 內建代理

Model-of-Design Tools 內建了兩個主代理和兩個子代理。
...
如果未指定溫度，Model-of-Design Tools 將使用模型特定的預設值；大多數模型通常為 0，Qwen 模型為 0.55。
...
此路徑相對於設定檔所在位置。因此它同時適用於全域 Model-of-Design Tools 設定和專案級設定。
...
Model-of-Design Tools 設定中的模型 ID 使用 `provider/model-id` 格式。例如，如果您使用 [Model-of-Design Tools Zen](/docs/zen)，則可以使用 `modtools/gpt-5.1-codex` 來表示 GPT 5.1 Codex。
...
:::tip
您有想要分享的代理嗎？[提交 PR](https://github.com/Embedded-Computing-Systems/modtools)。
:::
