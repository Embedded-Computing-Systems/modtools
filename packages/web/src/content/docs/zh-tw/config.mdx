...
您還可以使用 `~/.config/MOD/agents/` 或 `.mod/agents/` 中的 Markdown 檔案定義代理。[在此了解更多](/docs/agents)。
...
您還可以使用 `~/.config/MOD/commands/` 或 `.mod/commands/` 中的 Markdown 檔案定義指令。[在此了解更多](/docs/commands)。
...
將外掛程式檔案放置在 `.mod/plugins/` 或 `~/.config/MOD/plugins/` 中。您還可以透過 `plugin` 選項從 npm 載入外掛程式。
