---
title: Proveedores
description: Usando cualquier proveedor LLM en Model-of-Design Tools.
---

import config from "../../../../config.mjs"
export const console = config.console

Model-of-Design Tools usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración Model-of-Design Tools.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu Model-of-Design Tools
configuración.
...
Model-of-Design Tools Zen es una lista de modelos proporcionados por el equipo Model-of-Design Tools que han sido
probado y verificado para funcionar bien con Model-of-Design Tools. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con Model-of-Design Tools Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione modtools y diríjase a [modtools.ai/auth](https://modtools.ai/auth).
...
Funciona como cualquier otro proveedor en Model-of-Design Tools y su uso es completamente opcional.

---

## Model-of-Design Tools Go

Model-of-Design Tools Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de Model-of-Design Tools que han sido
probado y verificado para funcionar bien con Model-of-Design Tools.

1. Ejecute el comando `/connect` en TUI, seleccione `Model-of-Design Tools Go` y diríjase a [modtools.ai/auth](https://modtools.ai/zen).
...
Para usar Amazon Bedrock con Model-of-Design Tools:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta modtools:
   ...
   Para una configuración persistente o específica del proyecto, utilice `modtools.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en Model-of-Design Tools no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que modtools funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración modtools.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `Model-of-Design Tools`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación modtools.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
Model-of-Design Tools utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear Model-of-Design Tools
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `modtools.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [modtools-gitlab-auth](https://www.npmjs.com/package/modtools-gitlab-auth).

##### Configuración

Personalizar a través de `modtools.json`:
...
Para utilizar su suscripción GitHub Copilot con modtools:
...
Para utilizar Google Vertex AI con Model-of-Design Tools:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta modtools.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de modtools, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [modtools-helicone-session](https://github.com/H2Shami/modtools-helicone-session) para registrar automáticamente cada conversación Model-of-Design Tools como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación Model-of-Design Tools enumerada como una sesión separada.
...
Puede configurar modtools para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar modtools para usar modelos locales a través de LM Studio.
...
Puedes configurar modtools para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para Model-of-Design Tools. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/modtools) para obtener más detalles.
:::
...
Para usar Ollama Cloud con Model-of-Design Tools:
...
3. Copie la clave API para usarla en Model-of-Design Tools.
...
6. **Importante**: Antes de usar modelos en la nube en Model-of-Design Tools, debe obtener la información del modelo localmente:
...
Model-of-Design Tools Zen es una lista de modelos probados y verificados proporcionada por el equipo Model-of-Design Tools. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>Model-of-Design Tools Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **Model-of-Design Tools Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave Model-of-Design Tools API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración modtools.
...
5. También puedes personalizarlos a través de tu configuración modtools. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con Model-of-Design Tools:
...
También puede personalizar modelos a través de su configuración modtools. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con modtools. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `modtools.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a Model-of-Design Tools comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `modtools auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración modtools y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración modtools.
