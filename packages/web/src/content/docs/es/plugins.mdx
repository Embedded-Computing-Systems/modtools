---
title: Complementos
description: Escriba sus propios complementos para extender Mod.
---

Los complementos le permiten extender Mod al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Mod.
...
- `~/.config/MOD/plugins/` - Complementos globales
...
```json title="MOD.json"
{
  ...
  "plugin": ["MOD-helicone-session", "MOD-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/MOD/node_modules/`.
...
1. Configuración global (`~/.config/MOD/MOD.json`)
2. Configuración del proyecto (`MOD.json`)
3. Directorio global de complementos (`~/.config/MOD/plugins/`)
...
Mod ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK MOD para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar MOD.
  ...
  return {
    event: async ({ event }) => {
      ...
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "MOD"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Mod, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que MOD lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a MOD:
...
El ayudante `tool` crea una herramienta personalizada a la que MOD puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para MOD junto con las herramientas integradas.
