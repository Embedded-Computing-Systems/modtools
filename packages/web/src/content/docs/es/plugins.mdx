---
title: Complementos
description: Escriba sus propios complementos para extender Model-of-Design Tools.
---

Los complementos le permiten extender Model-of-Design Tools al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Model-of-Design Tools.
...
- `.modtools/plugins/` - Complementos a nivel de proyecto
- `~/.config/modtools/plugins/` - Complementos globales
...
```json title="modtools.json"
{
  ...
  "plugin": ["modtools-helicone-session", "modtools-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/modtools/node_modules/`.
...
1. Configuración global (`~/.config/modtools/modtools.json`)
2. Configuración del proyecto (`modtools.json`)
3. Directorio global de complementos (`~/.config/modtools/plugins/`)
4. Directorio de complementos del proyecto (`.modtools/plugins/`)
...
```json title=".modtools/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Model-of-Design Tools ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".modtools/plugins/my-plugin.ts"
...
```js title=".modtools/plugins/example.js"
...
- `client`: Un cliente SDK modtools para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar modtools.
...
```js title=".modtools/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "modtools"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Model-of-Design Tools, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que modtools lea archivos `.env`:

```javascript title=".modtools/plugins/env-protection.js"
...
```javascript title=".modtools/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a modtools:

```ts title=".modtools/plugins/custom-tools.ts"
import { type Plugin, tool } from "@modtools-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que modtools puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para modtools junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://modtools.ai/docs/sdk) para obtener más detalles.
...
```ts title=".modtools/plugins/compaction.ts"
import type { Plugin } from "@modtools-ai/plugin"
...
```ts title=".modtools/plugins/custom-compaction.ts"
import type { Plugin } from "@modtools-ai/plugin"
