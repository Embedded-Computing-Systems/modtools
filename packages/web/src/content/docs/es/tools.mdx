...
Las herramientas permiten que LLM realice acciones en su código base. Model-of-Design Tools viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Model-of-Design Tools.
...
:::note
Esta herramienta solo está disponible cuando `MOD_EXPERIMENTAL_LSP_TOOL=true` (o `MOD_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Model-of-Design Tools o cuando la variable de entorno `MOD_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Model-of-Design Tools:

```bash
MOD_ENABLE_EXA=1 modtools
```
