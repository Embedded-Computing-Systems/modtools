...
Las herramientas permiten que LLM realice acciones en su código base. MOD viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en MOD.
...
:::note
Esta herramienta solo está disponible cuando `MODTOOLS_EXPERIMENTAL_LSP_TOOL=true` (o `MODTOOLS_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor MOD o cuando la variable de entorno `MODTOOLS_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar MOD:

```bash
MODTOOLS_ENABLE_EXA=1 MOD
```
