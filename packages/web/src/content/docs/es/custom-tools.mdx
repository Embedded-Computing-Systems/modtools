...
- Localmente colocándolos en el directorio `.mod/tools/` de tu proyecto.
...
```ts title=".mod/tools/database.ts" {1}
...
```ts title=".mod/tools/project.ts" {8}
import { tool } from "@modtools-ai/plugin"
...
```ts title=".mod/tools/python-add.ts" {10}
import { tool } from "@modtools-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".mod/tools/add.py")
