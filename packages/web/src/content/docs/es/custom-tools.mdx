...
description: Cree herramientas que LLM pueda llamar en modtools.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de modtools como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.modtools/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/modtools/tools/`.
...
```ts title=".modtools/tools/database.ts" {1}
...
```ts title=".modtools/tools/math.ts"
import { tool } from "@modtools-ai/plugin"
...
```ts title=".modtools/tools/bash.ts"
import { tool } from "@modtools-ai/plugin"
...
```ts title=".modtools/tools/project.ts" {8}
import { tool } from "@modtools-ai/plugin"
...
```python title=".modtools/tools/add.py"
import sys
...
```ts title=".modtools/tools/python-add.ts" {10}
import { tool } from "@modtools-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".modtools/tools/add.py")
