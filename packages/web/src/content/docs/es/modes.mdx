...
:::caution
Los modos ahora se configuran a través de la opción `agent` en la configuración modtools. El
La opción `mode` ahora está en desuso. [Más información](/docs/agents).
:::

Los modos en modtools le permiten personalizar el comportamiento, las herramientas y las indicaciones para diferentes casos de uso.

Viene con dos modos integrados: **construir** y **planificar**. Puedes personalizar
estos o configure el suyo propio a través de la configuración modtools.
...
modtools viene con dos modos integrados.
...
- `edit` - No se pueden modificar archivos existentes, excepto los archivos ubicados en `.modtools/plans/*.md` para detallar el plan en sí.
...
Configure los modos en su archivo de configuración `modtools.json`:
...
Si no se especifica ninguna temperatura, modtools utiliza valores predeterminados específicos del modelo (normalmente 0 para la mayoría de los modelos, 0,55 para los modelos Qwen).
...
tanto la configuración global modtools como la configuración específica del proyecto.
...
Cree archivos de modo en `.modtools/modes/` para modos específicos del proyecto o `~/.config/modtools/modes/` para modos globales:
