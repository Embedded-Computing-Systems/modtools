---
title: Servidor
description: Interactuar con el servidor modtools a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `modtools serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente modtools puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `modtools.local` |
...
Configure `MOD_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `modtools`, o configure `MOD_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `modtools serve` como a `modtools web`.
...
Cuando ejecuta `modtools`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor modtools para interactuar con modtools mediante programación.
:::

Esta arquitectura permite que modtools admita múltiples clientes y le permite interactuar con modtools mediante programación.

Puede ejecutar `modtools serve` para iniciar un servidor independiente. Si tienes el
modtools TUI ejecutándose, `modtools serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Model-of-Design Tools [IDE](/docs/ide).
...
El servidor modtools expone las siguientes API.
