---
title: Servidor
description: Interactuar con el servidor MOD a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `MOD serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente MOD puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `MOD.local` |
...
Configure `MODTOOLS_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `MOD`, o configure `MODTOOLS_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `MOD serve` como a `MOD web`.
...
Cuando ejecuta `MOD`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor MOD para interactuar con MOD mediante programación.
:::

Esta arquitectura permite que MOD admita múltiples clientes y le permite interactuar con MOD mediante programación.

Puede ejecutar `MOD serve` para iniciar un servidor independiente. Si tienes el
MOD TUI ejecutándose, `MOD serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Mod [IDE](/docs/ide).
...
El servidor MOD expone las siguientes API.
