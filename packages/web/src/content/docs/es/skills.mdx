...
Las habilidades del agente permiten a Mod descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Mod busca estas ubicaciones:

- Configuración del proyecto: `.modtools/skills/<name>/SKILL.md`
- Configuración global: `~/.config/MOD/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Mod sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
...
Las definiciones globales también se cargan desde `~/.config/MOD/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Mod enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `MOD.json`:
...
**Para agentes integrados** (en `MOD.json`):
