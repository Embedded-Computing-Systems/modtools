...
Para depurar problemas con Model-of-Design Tools, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\modtools\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `modtools --log-level DEBUG`.

---

## Almacenamiento

modtools almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/modtools/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\modtools`
...
Model-of-Design Tools Desktop ejecuta un servidor Model-of-Design Tools local (el sidecar `modtools-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Model-of-Design Tools` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/modtools/modtools.jsonc` (o `~/.config/modtools/modtools.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/modtools/modtools.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\modtools\modtools.jsonc`
...
Model-of-Design Tools también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/modtools/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\modtools\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Model-of-Design Tools pueda reconstruirlo.

1. Salga de Model-of-Design Tools Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/modtools`
- **Linux**: eliminar `~/.cache/modtools` (o ejecutar `rm -rf ~/.cache/modtools`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\modtools`

3. Reinicie el escritorio Model-of-Design Tools.

---

### Solucionar problemas de conexión del servidor

Model-of-Design Tools Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `modtools.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `MOD_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `MOD_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio Model-of-Design Tools requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Model-of-Design Tools.

---

### Notificaciones que no se muestran

Model-of-Design Tools Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Model-of-Design Tools en la configuración de su sistema operativo, y
...
1. Salga del escritorio Model-of-Design Tools.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Model-of-Design Tools):

- `modtools.settings.dat` (URL del servidor predeterminado de escritorio)
- `modtools.global.dat` y `modtools.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con Model-of-Design Tools:
...
### Model-of-Design Tools no se inicia
...
3. Asegúrese de tener la última versión con `modtools upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `modtools models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\modtools`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. modtools instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\modtools`

2. Reinicie modtools para reinstalar los paquetes del proveedor más recientes.

Esto obligará a modtools a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
modtools detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
