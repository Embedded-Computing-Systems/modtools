...
description: Erstellen Sie Tools, die der LLM in Model-of-Design Tools aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von Model-of-Design Tools wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.modtools/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/modtools/tools/` platzieren.
