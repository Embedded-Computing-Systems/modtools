...
Model-of-Design Tools bringt eingebaute Tools mit und laesst sich ueber [Custom Tools](/docs/custom-tools) oder [MCP-Server](/docs/mcp-servers) erweitern.
...
Hier sind alle in Model-of-Design Tools verfuegbaren eingebauten Tools.
...
:::note
Dieses Tool ist nur verfuegbar, wenn `MOD_EXPERIMENTAL_LSP_TOOL=true` (oder `MOD_EXPERIMENTAL=true`) gesetzt ist.
:::
...
:::note
Dieses Tool ist nur verfuegbar, wenn der Model-of-Design Tools-Provider genutzt wird oder die Umgebungsvariable `MOD_ENABLE_EXA` auf einen 'truthy' Wert (z. B. `true` oder `1`) gesetzt ist.

Zum Aktivieren beim Start von Model-of-Design Tools:
