...
Sie können externe Tools zu Model-of-Design Tools hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Model-of-Design Tools unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Model-of-Design Tools Config](https://modtools.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/modtools`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
Model-of-Design Tools übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Model-of-Design Tools Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Model-of-Design Tools beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `modtools mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Model-of-Design Tools die Token sicher in `~/.local/share/modtools/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Model-of-Design Tools verfügbar. Sie können sie also wie jedes andere Tool über die Model-of-Design Tools-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Model-of-Design Tools mit Ihrem Sentry-Konto zu verbinden.
