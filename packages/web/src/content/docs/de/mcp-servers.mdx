...
Sie können externe Tools zu MOD hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. MOD unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [MOD Config](https://modtools.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/MOD`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
MOD übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird MOD Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von MOD beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `MOD mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert MOD die Token sicher in `~/.local/share/MOD/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in MOD verfügbar. Sie können sie also wie jedes andere Tool über die MOD-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und MOD mit Ihrem Sentry-Konto zu verbinden.
