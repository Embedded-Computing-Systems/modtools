...
:::caution
Modi werden jetzt über die Option `agent` in der Model-of-Design Tools-Konfiguration konfiguriert. Der
Die Option `mode` ist jetzt veraltet. [Learn more](/docs/agents).
:::

Mit den Modi in Model-of-Design Tools können Sie das Verhalten, die Tools und die Eingabeaufforderungen für verschiedene Anwendungsfälle anpassen.

Es verfügt über zwei integrierte Modi: **Build** und **Plan**. Sie können anpassen
diese oder konfigurieren Sie Ihre eigenen über die Model-of-Design Tools-Konfiguration.
...
Model-of-Design Tools verfügt über zwei integrierte Modi.
...
- `edit` – Vorhandene Dateien können nicht geändert werden, mit Ausnahme der Dateien unter `.modtools/plans/*.md`, um den Plan selbst detailliert darzustellen
...
Konfigurieren Sie Modi in Ihrer `modtools.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet Model-of-Design Tools modellspezifische Standardwerte (normalerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle).
...
sowohl die globale Model-of-Design Tools-Konfiguration als auch die projektspezifische Konfiguration.
...
Erstellen Sie Modusdateien in `.modtools/modes/` für projektspezifische Modi oder `~/.config/modtools/modes/` für globale Modi:
