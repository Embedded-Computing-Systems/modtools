---
title: Regeln
description: Lege eigene Anweisungen fuer modtools fest.
---

Du kannst modtools ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in modtools den Befehl `/init` aus.
...
So kann modtools sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht modtools sie zu erweitern.
...
modtools kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/modtools/AGENTS.md` speichern.
Diese gelten in allen modtools-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Model-of-Design Tools dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/modtools/AGENTS.md` existiert)
...
Beim Start sucht modtools in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/modtools/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `modtools.json` oder in `~/.config/modtools/modtools.json` angeben.
...
modtools parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit modtools.json

Empfohlen ist das `instructions`-Feld in der `modtools.json`:
...
Du kannst modtools explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `modtools.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
