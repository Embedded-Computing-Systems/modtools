---
title: Regeln
description: Lege eigene Anweisungen fuer MOD fest.
---

Du kannst MOD ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in MOD den Befehl `/init` aus.
...
So kann MOD sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht MOD sie zu erweitern.
...
MOD kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/MOD/AGENTS.md` speichern.
Diese gelten in allen MOD-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt MOD dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/MOD/AGENTS.md` existiert)
...
Beim Start sucht MOD in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/MOD/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `MOD.json` oder in `~/.config/MOD/MOD.json` angeben.
...
MOD parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit MOD.json

Empfohlen ist das `instructions`-Feld in der `MOD.json`:
...
Du kannst MOD explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `MOD.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
