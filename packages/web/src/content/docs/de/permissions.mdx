...
Model-of-Design Tools verwendet die `permission`-Konfiguration, um zu entscheiden, ob eine bestimmte Aktion automatisch ausgeführt werden soll, Sie dazu auffordern oder blockiert werden soll.
...
Verwenden Sie `external_directory`, um Toolaufrufe zuzulassen, die Pfade außerhalb des Arbeitsverzeichnisses berühren, in dem Model-of-Design Tools gestartet wurde. Dies gilt für jedes Werkzeug, das einen Pfad als Eingabe verwendet (z. B. `read`, `edit`, `list`, `glob`, `grep` und viele `bash`-Befehle).
...
Model-of-Design Tools-Berechtigungen basieren auf Tool-Namen sowie einigen Sicherheitsvorkehrungen:
...
Wenn Sie nichts angeben, beginnt Model-of-Design Tools mit zulässigen Standardeinstellungen:
...
Wenn Model-of-Design Tools zur Genehmigung auffordert, bietet UI drei Ergebnisse:

- `once` – nur diese Anfrage genehmigen
- `always` – zukünftige Anfragen genehmigen, die den vorgeschlagenen Mustern entsprechen (für den Rest der aktuellen Model-of-Design Tools-Sitzung)
