---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Model-of-Design Tools.
---

import config from "../../../../config.mjs"
export const console = config.console

Model-of-Design Tools verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer Model-of-Design Tools-Konfiguration.
...
in `~/.local/share/modtools/auth.json`.

---

### Konfiguration

Sie können die Anbieter über den Abschnitt `provider` in Ihrem Model-of-Design Tools anpassen
config.
...
Model-of-Design Tools Zen ist eine Liste von Modellen, die vom Model-of-Design Tools-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Model-of-Design Tools funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit Model-of-Design Tools Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie Model-of-Design Tools aus und gehen Sie zu [modtools.ai/auth](https://modtools.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in Model-of-Design Tools und ist völlig optional.

---

## Model-of-Design Tools Go

Model-of-Design Tools Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom Model-of-Design Tools-Team getestet und verifiziert wurden, dass sie gut mit Model-of-Design Tools funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `Model-of-Design Tools Go` und gehen Sie zu [modtools.ai/auth](https://modtools.ai/zen).
...
So verwenden Sie Amazon Bedrock mit Model-of-Design Tools:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Model-of-Design Tools ausführen:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY modtools

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile modtools

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX modtools
   ```
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `modtools.json`:

   ```json title="modtools.json"
   {
     "$schema": "https://modtools.ai/config.json",
     "provider": {
       "amazon-bedrock": {
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Model-of-Design Tools wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Model-of-Design Tools ordnungsgemäß funktioniert.
   :::
...
5. Legen Sie Ihren Ressourcennamen als Umgebungsvariable fest:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX modtools
   ```
...
5. Legen Sie Ihren Ressourcennamen als Umgebungsvariable fest:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX modtools
   ```
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Model-of-Design Tools-Konfiguration hinzufügen.
...
2. Wählen Sie Ihre Authentifizierungsmethode:
   ...
   3. Name: `Model-of-Design Tools`, Bereiche: `api`
...
um das Token im Model-of-Design Tools-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
Model-of-Design Tools verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Model-of-Design Tools
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`modtools.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [modtools-gitlab-auth](https://www.npmjs.com/package/modtools-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `modtools.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Model-of-Design Tools:
...
So verwenden Sie Google Vertex AI mit Model-of-Design Tools:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Model-of-Design Tools ausführen.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id modtools
   ```
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Model-of-Design Tools konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
```jsonc title="~/.config/modtools/modtools.jsonc"
{
  "$schema": "https://modtools.ai/config.json",
  "provider": {
    "helicone": {
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [modtools-helicone-session](https://github.com/H2Shami/modtools-helicone-session)-Plugin, um jede Model-of-Design Tools-Konversation automatisch als Sitzung in Helicone zu protokollieren.

```bash
npm install -g modtools-helicone-session
```
...
```json title="modtools.json"
{
  "plugin": ["modtools-helicone-session"]
}
```

Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede Model-of-Design Tools-Konversation als separate Sitzung aufgeführt.
...
Sie können Model-of-Design Tools für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren

```json title="modtools.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://modtools.ai/config.json",
  "provider": {
    "llama.cpp": {
...
Sie können Model-of-Design Tools über LM Studio für die Verwendung lokaler Modelle konfigurieren.

```json title="modtools.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://modtools.ai/config.json",
  "provider": {
    "lmstudio": {
...
Sie können Model-of-Design Tools über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Model-of-Design Tools konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/modtools).
:::

```json title="modtools.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://modtools.ai/config.json",
  "provider": {
    "ollama": {
...
So verwenden Sie Ollama Cloud mit Model-of-Design Tools:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Model-of-Design Tools.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Model-of-Design Tools verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
Model-of-Design Tools Zen ist eine Liste getesteter und verifizierter Modelle, die vom Model-of-Design Tools-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>Model-of-Design Tools Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **Model-of-Design Tools Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren Model-of-Design Tools API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Model-of-Design Tools-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Model-of-Design Tools-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
3. Geben Sie Ihren JSON-Service-Account-Key ein.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' modtools
   ```
...
4. Legen Sie optional die Bereitstellung ID und die Ressourcengruppe fest:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group modtools
   ```
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Model-of-Design Tools:
...
Sie können Modelle auch über Ihre Model-of-Design Tools-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Model-of-Design Tools verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
3. Geben Sie Ihren API-Schlüssel für den Anbieter ein.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in modtools.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Erstellen oder aktualisieren Sie Ihre `modtools.json`-Datei in Ihrem Projektverzeichnis:

   ```json title="modtools.json" ""myprovider"" {5-15}
   {
     "$schema": "https://modtools.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="modtools.json" {9,11,17-20}
{
  "$schema": "https://modtools.ai/config.json",
  "provider": {
    "myprovider": {
...
Mithilfe der `limit`-Felder kann Model-of-Design Tools erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `modtools auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Model-of-Design Tools-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Model-of-Design Tools-Konfiguration übereinstimmt.
