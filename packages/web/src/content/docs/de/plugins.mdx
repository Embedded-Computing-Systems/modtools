---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Model-of-Design Tools zu erweitern.
---

Mit Plugins können Sie Model-of-Design Tools erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Model-of-Design Tools zu ändern.
...
- `.modtools/plugins/` – Plugins auf Projektebene
- `~/.config/modtools/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/modtools/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/modtools/modtools.json`)
2. Projektkonfiguration (`modtools.json`)
3. Globales Plugin-Verzeichnis (`~/.config/modtools/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.modtools/plugins/`)
...
Model-of-Design Tools führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Model-of-Design Tools-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Model-of-Design Tools erweitern können.
...
:::note
Wenn Sie die Model-of-Design Tools-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Model-of-Design Tools `.env`-Dateien liest:
...
Plugins können Model-of-Design Tools auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Model-of-Design Tools aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Model-of-Design Tools zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://modtools.ai/docs/sdk).
