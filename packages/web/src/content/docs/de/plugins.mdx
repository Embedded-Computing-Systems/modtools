---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Mod zu erweitern.
---

Mit Plugins können Sie Mod erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Mod zu ändern.
...
- `~/.config/MOD/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/MOD/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/MOD/MOD.json`)
2. Projektkonfiguration (`MOD.json`)
3. Globales Plugin-Verzeichnis (`~/.config/MOD/plugins/`)
...
Mod führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Mod-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Mod erweitern können.
...
:::note
Wenn Sie die Mod-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Mod `.env`-Dateien liest:
...
Plugins können Mod auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Mod aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Mod zur Verfügung.
