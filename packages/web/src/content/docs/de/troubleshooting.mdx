...
Wenn Model-of-Design Tools Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\modtools\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `modtools --log-level DEBUG`.

---

## Speicher

modtools speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/modtools/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\modtools`
...
Model-of-Design Tools Desktop startet im Hintergrund einen lokalen Model-of-Design Tools-Server (`modtools-cli`-Sidecar).
...
- Nur macOS: `Model-of-Design Tools`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/modtools/modtools.jsonc` (oder `~/.config/modtools/modtools.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/modtools/modtools.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\modtools\modtools.jsonc`
...
Model-of-Design Tools kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/modtools/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\modtools\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Model-of-Design Tools ihn neu aufbauen kann.

1. Beende Model-of-Design Tools Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/modtools`
- **Linux**: loesche `~/.cache/modtools` (oder fuehre aus: `rm -rf ~/.cache/modtools`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\modtools`

3. Starte Model-of-Design Tools Desktop neu.

---

### Server-Verbindungsprobleme beheben

Model-of-Design Tools Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `modtools.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `MOD_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `MOD_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Model-of-Design Tools Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Model-of-Design Tools-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Model-of-Design Tools Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Model-of-Design Tools in den OS-Einstellungen aktiviert sind, und
...
1. Beende Model-of-Design Tools Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Model-of-Design Tools Desktop):

- `modtools.settings.dat` (desktop default server URL)
- `modtools.global.dat` und `modtools.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Model-of-Design Tools hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `modtools models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\modtools`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. modtools installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\modtools`

2. Starte modtools neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt modtools, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
modtools erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
