...
Wenn MOD Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\MOD\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `MOD --log-level DEBUG`.

---

## Speicher

MOD speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/MOD/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\MOD`
...
MOD Desktop startet im Hintergrund einen lokalen MOD-Server (`MOD-cli`-Sidecar).
...
- Nur macOS: `MOD`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/MOD/MOD.jsonc` (oder `~/.config/MOD/MOD.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/MOD/MOD.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\MOD\MOD.jsonc`
...
MOD kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/MOD/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\MOD\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit MOD ihn neu aufbauen kann.

1. Beende MOD Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/MOD`
- **Linux**: loesche `~/.cache/MOD` (oder fuehre aus: `rm -rf ~/.cache/MOD`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\MOD`

3. Starte MOD Desktop neu.

---

### Server-Verbindungsprobleme beheben

MOD Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `MOD.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `MODTOOLS_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `MODTOOLS_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt MOD Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit MOD-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

MOD Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer MOD in den OS-Einstellungen aktiviert sind, und
...
1. Beende MOD Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von MOD Desktop):

- `MOD.settings.dat` (desktop default server URL)
- `MOD.global.dat` und `MOD.workspace.*.dat` (UI state like recent servers/projects)
...
3. Stelle sicher, dass du die neueste Version hast: `MOD upgrade`
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `MOD models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\MOD`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. MOD installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\MOD`

2. Starte MOD neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt MOD, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
MOD erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
