---
title: Server
description: Nutze den modtools-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `modtools serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein modtools-Client nutzen kann.
...
Setze `MOD_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `modtools`, kann aber mit `MOD_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `modtools serve` und `modtools web`.
...
Wenn du `modtools` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den modtools-Server, um modtools programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `modtools serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `modtools serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Model-of-Design Tools-[IDE](/docs/ide)-Plugins.
...
Der modtools-Server stellt folgende APIs bereit.
