...
Die meisten gängigen Anbieter sind standardmäßig vorinstalliert. Wenn Sie die Anmeldeinformationen für einen Anbieter über den Befehl `/connect` hinzugefügt haben, sind diese verfügbar, wenn Sie Model-of-Design Tools starten.
...
Hier sind mehrere Modelle, die gut mit Model-of-Design Tools funktionieren, in keiner bestimmten Reihenfolge. (Dies ist weder eine vollständige noch eine unbedingt aktuelle Liste):
...
Hier ist der vollständige ID `provider_id/model_id`. Wenn Sie beispielsweise [Model-of-Design Tools Zen](/docs/zen) verwenden, würden Sie `modtools/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
Viele Modelle unterstützen mehrere Varianten mit unterschiedlichen Konfigurationen. Model-of-Design Tools wird mit integrierten Standardvarianten für beliebte Anbieter ausgeliefert.

### Integrierte Varianten

Model-of-Design Tools wird für viele Anbieter mit Standardvarianten ausgeliefert:
...
Wenn Model-of-Design Tools startet, sucht es nach Modellen in der folgenden Prioritätsreihenfolge:

1. Das Befehlszeilenflag `--model` oder `-m`. Das Format ist das gleiche wie in der Konfigurationsdatei: `provider_id/model_id`.

2. Die Modellliste in der Model-of-Design Tools-Konfiguration.
