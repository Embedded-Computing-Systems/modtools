...
Agent Skills erlauben Mod, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Bei projektlokalen Pfaden laeuft Mod vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
...
Globale Definitionen kommen zusaetzlich aus `~/.config/MOD/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Mod listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `MOD.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `MOD.json`):
