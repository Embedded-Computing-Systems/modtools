...
Agent Skills erlauben Model-of-Design Tools, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Model-of-Design Tools sucht in folgenden Pfaden:

- Project config: `.modtools/skills/<name>/SKILL.md`
- Global config: `~/.config/modtools/skills/<name>/SKILL.md`
...
Bei projektlokalen Pfaden laeuft Model-of-Design Tools vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.modtools/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/modtools/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.modtools/skills/git-release/SKILL.md` zum Beispiel so:
...
Model-of-Design Tools listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `modtools.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `modtools.json`):
