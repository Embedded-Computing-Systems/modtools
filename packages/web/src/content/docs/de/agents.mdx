...
Es gibt zwei Arten von Agenten in Mod; Primäragenten und Subagenten.
...
Mod verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `MOD.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet Mod modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale Mod-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer Mod-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [Mod Zen](/docs/zen) verwenden, würden Sie `MOD/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `MOD models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
