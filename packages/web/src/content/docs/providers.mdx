---
title: Providers
description: Using any LLM provider in Model-of-Design Tools.
---

import config from "../../../config.mjs"
export const console = config.console

Model-of-Design Tools uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Model-of-Design Tools config.
...
You can customize the providers through the `provider` section in your Model-of-Design Tools
config.
...
Model-of-Design Tools Zen is a list of models provided by the Model-of-Design Tools team that have been
tested and verified to work well with Model-of-Design Tools. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Model-of-Design Tools Zen.
:::

1. Run the `/connect` command in the TUI, select `Model-of-Design Tools Zen`, and head to [modtools.ai/auth](https://modtools.ai/zen).
...
It works like any other provider in Model-of-Design Tools and is completely optional to use.

---

## Model-of-Design Tools Go

Model-of-Design Tools Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Model-of-Design Tools team that have been
tested and verified to work well with Model-of-Design Tools.

1. Run the `/connect` command in the TUI, select `Model-of-Design Tools Go`, and head to [modtools.ai/auth](https://modtools.ai/zen).
...
To use Amazon Bedrock with Model-of-Design Tools:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running modtools:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `modtools.json`:
...
Model-of-Design Tools. Anthropic explicitly prohibits this.

Previous versions of Model-of-Design Tools came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Model-of-Design Tools with zero setup:
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for modtools to work properly.
   :::
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your modtools config.
...
GitLab Duo support in Model-of-Design Tools is experimental. Features, configuration, and
behavior may change in future releases.
:::

Model-of-Design Tools integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
2. Choose your authentication method:
   ...
   3. Name: `Model-of-Design Tools`, Scopes: `api`
...
to store token in modtools auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
Model-of-Design Tools uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Model-of-Design Tools
to only use your own GitLab-hosted instance, add the following to your
`modtools.json` file. It is also recommended to disable session sharing.
...
More documentation on [modtools-gitlab-auth](https://www.npmjs.com/package/modtools-gitlab-auth) homepage.

##### Configuration

Customize through `modtools.json`:
...
When a `duo-workflow-*` model is selected, Model-of-Design Tools will:
...
4. Route tool execution requests through Model-of-Design Tools's permission-gated tool system
...
To use your GitHub Copilot subscription with modtools:
...
To use Google Vertex AI with Model-of-Design Tools:
...
2. Set the required environment variables:
   ...
   Set them while running modtools.
...
In the event you see a feature or model from Helicone that isn't configured automatically through modtools, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [modtools-helicone-session](https://github.com/H2Shami/modtools-helicone-session) plugin to automatically log each Model-of-Design Tools conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Model-of-Design Tools conversation listed as a separate session.
...
You can configure modtools to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure modtools to use local models through LM Studio.
...
You can configure modtools to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Model-of-Design Tools. See the [Ollama integration docs](https://docs.ollama.com/integrations/modtools) for details.
:::
...
To use Ollama Cloud with Model-of-Design Tools:
...
3. Copy the API key for use in Model-of-Design Tools.
...
6. **Important**: Before using cloud models in Model-of-Design Tools, you must pull the model information locally:
...
Model-of-Design Tools Zen is a list of tested and verified models provided by the Model-of-Design Tools team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Model-of-Design Tools Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Model-of-Design Tools Zen**.

   ```txt
   /connect
   ```

3. Enter your Model-of-Design Tools API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your modtools config.
...
5. You can also customize them through your modtools config. Here's an example of specifying a provider
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Modtools:
...
You can also customize models through your modtools config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with modtools. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `modtools.json` file in your project directory:
...
The `limit` fields allow Model-of-Design Tools to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `modtools auth list` to see if the credentials
...
2. For custom providers, check the modtools config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your modtools config.
