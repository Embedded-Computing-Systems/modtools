...
Les compétences d'agent permettent à Mod de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
Mod recherche ces emplacements :

- Configuration du projet : `.modtools/skills/<name>/SKILL.md`
- Configuration globale : `~/.config/MOD/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, Mod remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
...
Les définitions globales sont également chargées à partir de `~/.config/MOD/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` et `~/.agents/skills/*/SKILL.md`.
...
Mod répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `MOD.json` :
...
**Pour les agents intégrés** (dans `MOD.json`) :
