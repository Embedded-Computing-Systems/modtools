...
Les compétences d'agent permettent à Model-of-Design Tools de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
Model-of-Design Tools recherche ces emplacements :

- Configuration du projet : `.modtools/skills/<name>/SKILL.md`
- Configuration globale : `~/.config/modtools/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, Model-of-Design Tools remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
Il charge tout `skills/*/SKILL.md` correspondant dans `.modtools/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.

Les définitions globales sont également chargées à partir de `~/.config/modtools/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` et `~/.agents/skills/*/SKILL.md`.
...
Créez `.modtools/skills/git-release/SKILL.md` comme ceci :
...
Model-of-Design Tools répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `modtools.json` :
...
**Pour les agents intégrés** (dans `modtools.json`) :
