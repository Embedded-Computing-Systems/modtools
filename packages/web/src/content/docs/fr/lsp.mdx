---
title: Serveurs LSP
description: Model-of-Design Tools s'intègre à vos serveurs LSP.
---

Model-of-Design Tools s'intègre via le protocole LSP (Language Server Protocol) pour aider le LLM à interagir avec votre base de code. Il utilise des diagnostics pour fournir des commentaires au LLM.

---

## Serveurs intégrés

Model-of-Design Tools est livré avec plusieurs serveurs LSP intégrés pour les langages populaires :
...
:::note
Vous pouvez désactiver les téléchargements automatiques du serveur LSP en définissant la variable d'environnement `MOD_DISABLE_LSP_DOWNLOAD` sur `true`.
:::
...
Lorsque modtools ouvre un fichier, il :
...
Vous pouvez personnaliser les serveurs LSP via la section `lsp` de votre configuration modtools.
