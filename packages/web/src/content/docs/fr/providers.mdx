---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en Model-of-Design Tools.
---

import config from "../../../../config.mjs"
export const console = config.console

Model-of-Design Tools utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration Model-of-Design Tools.
...
en `~/.local/share/modtools/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre Model-of-Design Tools
configuration.
...
Model-of-Design Tools Zen est une liste de modèles fournis par l'équipe Model-of-Design Tools qui ont été
testé et vérifié pour fonctionner correctement avec Model-of-Design Tools. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par Model-of-Design Tools Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez modtools et dirigez-vous vers [modtools.ai/auth](https://modtools.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans Model-of-Design Tools et son utilisation est totalement facultative.

---

## Model-of-Design Tools Go

Model-of-Design Tools Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe Model-of-Design Tools qui ont été
testé et vérifié pour fonctionner correctement avec Model-of-Design Tools.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `Model-of-Design Tools Go` et rendez-vous sur [modtools.ai/auth](https://modtools.ai/zen).
...
Pour utiliser Amazon Bedrock avec Model-of-Design Tools :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de modtools :
...
Pour une configuration spécifique au projet ou persistante, utilisez `modtools.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans Model-of-Design Tools n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que modtools fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration modtools.
...
3.  Nom : `Model-of-Design Tools`, Portées : `api`
...
pour stocker le jeton dans le stockage d'authentification modtools.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
Model-of-Design Tools utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller Model-of-Design Tools
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `modtools.json`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil [modtools-gitlab-auth](https://www.npmjs.com/package/modtools-gitlab-auth).

##### Configuration

Personnalisez via `modtools.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec modtools :
...
Pour utiliser Google Vertex AI avec Model-of-Design Tools :
...
Définissez-les lors de l'exécution de modtools.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via modtools, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [modtools-helicone-session](https://github.com/H2Shami/modtools-helicone-session) pour enregistrer automatiquement chaque conversation Model-of-Design Tools en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation Model-of-Design Tools répertoriée comme une session distincte.
...
Vous pouvez configurer modtools pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer modtools pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer modtools pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour Model-of-Design Tools. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/modtools) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec Model-of-Design Tools :
...
3. Copiez la clé API à utiliser dans Model-of-Design Tools.
...
6. **Important** : Avant d'utiliser des modèles cloud dans Model-of-Design Tools, vous devez extraire les informations du modèle localement :
...
Model-of-Design Tools Zen est une liste de modèles testés et vérifiés fournie par l'équipe Model-of-Design Tools. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>Model-of-Design Tools Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **Model-of-Design Tools Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé Model-of-Design Tools API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration modtools.
...
5. Vous pouvez également les personnaliser via votre configuration modtools. Voici un exemple de spécification d'un fournisseur
...
Pour utiliser [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) avec Modtools :
...
Vous pouvez également personnaliser les modèles via votre configuration modtools. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec modtools. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
4. Créez ou mettez à jour votre fichier `modtools.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à Model-of-Design Tools de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `modtools auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration modtools et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration modtools.
