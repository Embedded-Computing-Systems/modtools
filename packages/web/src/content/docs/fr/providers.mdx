...
1. Exécutez la commande `/connect` dans le TUI, sélectionnez `Mod Go` et rendez-vous sur [modtools.ai/auth](https://modtools.ai/zen).
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [MOD-helicone-session](https://github.com/H2Shami/MOD-helicone-session) pour enregistrer automatiquement chaque conversation Mod en tant que session dans Helicone.
