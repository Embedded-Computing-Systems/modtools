...
Avec Mod, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Mod utilise notre propre thème `MOD`.
...
Mod est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Mod corresponde à l'apparence de leur terminal
...
Mod prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/MOD/themes/*.json` ou `$XDG_CONFIG_HOME/MOD/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/MOD/themes
vim ~/.config/MOD/themes/my-theme.json
```
