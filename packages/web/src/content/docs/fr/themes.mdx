...
Avec Model-of-Design Tools, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Model-of-Design Tools utilise notre propre thème `modtools`.
...
Model-of-Design Tools est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Model-of-Design Tools corresponde à l'apparence de leur terminal
...
Model-of-Design Tools prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/modtools/themes/*.json` ou `$XDG_CONFIG_HOME/modtools/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.modtools/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.modtools/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/modtools/themes
vim ~/.config/modtools/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .modtools/themes
vim .modtools/themes/my-theme.json
```
...
{
  "$schema": "https://modtools.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
