...
Vous pouvez ajouter des outils externes à Mod à l'aide du _Model Context Protocol_ ou MCP. Mod prend en charge les serveurs locaux et distants.
...
Vous pouvez définir des serveurs MCP dans votre [Mod Config](https://modtools.ai/docs/config/) sous `mcp`. Ajoutez chaque MCP avec un nom unique. Vous pouvez faire référence à ce MCP par son nom lorsque vous demandez le LLM.
...
Les organisations peuvent fournir des serveurs MCP par défaut via leur point de terminaison `.well-known/MOD`. Ces serveurs peuvent être désactivés par défaut, permettant aux utilisateurs de choisir ceux dont ils ont besoin.
...
Mod gère automatiquement l'authentification OAuth pour les serveurs MCP distants. Lorsqu'un serveur nécessite une authentification, Mod :
...
Si le serveur nécessite une authentification, Mod vous demandera de vous authentifier lorsque vous essayez de l'utiliser pour la première fois. Sinon, vous pouvez [déclencher manuellement le flux](#authenticating) avec `MOD mcp auth <server-name>`.
...
La commande `mcp auth` ouvrira votre navigateur pour autorisation. Après votre autorisation, Mod stockera les jetons en toute sécurité dans `~/.local/share/MOD/mcp-auth.json`.
...
Vos MCP sont disponibles sous forme d'outils dans Mod, aux côtés des outils intégrés. Vous pouvez donc les gérer via la configuration Mod comme n'importe quel autre outil.
...
Cela ouvrira une fenêtre de navigateur pour terminer le flux OAuth et connecter Mod à votre compte Sentry.
