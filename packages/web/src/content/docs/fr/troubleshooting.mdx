...
Pour déboguer les problèmes avec Model-of-Design Tools, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\modtools\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `modtools --log-level DEBUG`.

---

## Stockage

modtools stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/modtools/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\modtools`
...
Model-of-Design Tools Desktop exécute un serveur Model-of-Design Tools local (le side-car `modtools-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `Model-of-Design Tools` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** : `~/.config/modtools/modtools.jsonc` (ou `~/.config/modtools/modtools.json`)
- **macOS/Linux** (anciennes installations) : `~/.local/share/modtools/modtools.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\modtools\modtools.jsonc`
...
Model-of-Design Tools peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/modtools/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\modtools\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que Model-of-Design Tools puisse le reconstruire.

1. Quittez complètement Model-of-Design Tools Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/modtools`
- **Linux** : supprimez `~/.cache/modtools` (ou exécutez `rm -rf ~/.cache/modtools`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\modtools`

3. Redémarrez le bureau Model-of-Design Tools.

---

### Résoudre les problèmes de connexion au serveur

Model-of-Design Tools Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `modtools.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `MOD_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `MOD_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, Model-of-Design Tools Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de Model-of-Design Tools.

---

### Les notifications ne s'affichent pas

Model-of-Design Tools Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour Model-of-Design Tools dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau Model-of-Design Tools.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application Model-of-Design Tools Desktop) :

- `modtools.settings.dat` (serveur de bureau par défaut URL)
- `modtools.global.dat` et `modtools.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec Model-of-Design Tools :
...
### Model-of-Design Tools ne démarre pas
...
3. Assurez-vous d'avoir la dernière version avec `modtools upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `modtools models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\modtools`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. modtools installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\modtools`

2. Redémarrez modtools pour réinstaller les derniers packages du fournisseur

Cela forcera modtools à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
modtools détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
