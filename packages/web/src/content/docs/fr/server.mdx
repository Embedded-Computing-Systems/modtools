---
title: Serveur
description: Interagissez avec le serveur MOD via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `MOD serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client MOD peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `MOD.local` |
...
Définissez `MODTOOLS_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `MOD`, ou définissez `MODTOOLS_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `MOD serve` et à `MOD web`.
...
Lorsque vous exécutez `MOD`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur MOD pour interagir avec MOD par programmation.
:::

Cette architecture permet à MOD de prendre en charge plusieurs clients et vous permet d'interagir avec MOD par programme.

Vous pouvez exécuter `MOD serve` pour démarrer un serveur autonome. Si vous avez le
MOD TUI en cours d'exécution, `MOD serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Mod [IDE](/docs/ide).
...
Le serveur MOD expose les API suivantes.
