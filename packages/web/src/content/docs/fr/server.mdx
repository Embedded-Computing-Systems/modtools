---
title: Serveur
description: Interagissez avec le serveur modtools via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `modtools serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client modtools peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `modtools.local` |
...
Définissez `MOD_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `modtools`, ou définissez `MOD_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `modtools serve` et à `modtools web`.
...
Lorsque vous exécutez `modtools`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur modtools pour interagir avec modtools par programmation.
:::

Cette architecture permet à modtools de prendre en charge plusieurs clients et vous permet d'interagir avec modtools par programme.

Vous pouvez exécuter `modtools serve` pour démarrer un serveur autonome. Si vous avez le
modtools TUI en cours d'exécution, `modtools serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Model-of-Design Tools [IDE](/docs/ide).
...
Le serveur modtools expose les API suivantes.
