...
Il existe deux types d'agents dans Model-of-Design Tools ; agents primaires et sous-agents.
...
Model-of-Design Tools est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
Model-of-Design Tools est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

Model-of-Design Tools est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `modtools.json` :
...
- Global : `~/.config/modtools/agents/`
- Par projet : `.modtools/agents/`

```markdown title="~/.config/modtools/agents/review.md"
---
...
Si aucune température n'est spécifiée, Model-of-Design Tools utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale Model-of-Design Tools et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration Model-of-Design Tools utilise le format `provider/model-id`. Par exemple, si vous utilisez [Model-of-Design Tools Zen](/docs/zen), vous utiliserez `modtools/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="modtools.json" {3-6,9-12}
{
  "$schema": "https://modtools.ai/config.json",
  "tools": {
    "write": true,
...
```json title="modtools.json" {3-5,8-10}
{
  "$schema": "https://modtools.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Exécutez `modtools models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/Embedded-Computing-Systems/modtools).
:::
...
```markdown title="~/.config/modtools/agents/docs-writer.md"
---
...
```markdown title="~/.config/modtools/agents/security-auditor.md"
---
