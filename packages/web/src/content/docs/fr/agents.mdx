...
Il existe deux types d'agents dans Mod ; agents primaires et sous-agents.
...
Mod est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
Mod est livré avec trois sous-agents intégrés, **General**, **Explore** et **Scout**. Nous les verrons ci-dessous.

---

## Agents intégrés

Mod est livré avec deux agents primaires intégrés et trois sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `MOD.json` :
...
Si aucune température n'est spécifiée, Mod utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale Mod et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration Mod utilise le format `provider/model-id`. Par exemple, si vous utilisez [Mod Zen](/docs/zen), vous utiliserez `MOD/gpt-5.1-codex` pour GPT 5.1 Codex.
...
:::tip
Exécutez `MOD models` pour voir une liste des modèles disponibles.
:::
