...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez Model-of-Design Tools.
...
Voici plusieurs modèles qui fonctionnent bien avec Model-of-Design Tools, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration Model-of-Design Tools.

```json title="modtools.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [Model-of-Design Tools Zen](/docs/zen), vous utiliserez `modtools/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="modtools.jsonc" {7-12,19-24}
{
  "$schema": "https://modtools.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="modtools.jsonc" {6-21}
{
  "$schema": "https://modtools.ai/config.json",
  "provider": {
    "modtools": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. Model-of-Design Tools est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

Model-of-Design Tools est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="modtools.jsonc" {7-18}
{
  "$schema": "https://modtools.ai/config.json",
  "provider": {
    "openai": {
...
Lorsque Model-of-Design Tools démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration Model-of-Design Tools.
