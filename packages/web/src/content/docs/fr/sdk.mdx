---
title: SDK
description: Client JS de type sécurisé pour le serveur MOD.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK MOD JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler MOD par programme.
...
Créez une instance de MOD :

```javascript
import { createMod } from "@modtools-ai/sdk"

const { client } = await createMod()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `MOD.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${MOD.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de MOD, vous pouvez créer une instance client pour vous y connecter :
