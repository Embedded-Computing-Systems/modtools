...
description: Créez des outils que le LLM peut appeler dans MOD.
---

Les outils personnalisés sont des fonctions que vous créez et que le LLM peut appeler pendant les conversations. Ils fonctionnent avec les [outils intégrés](/docs/tools) de MOD comme `read`, `write` et `bash`.
...
- Ou globalement, en les plaçant dans `~/.config/MOD/tools/`.
