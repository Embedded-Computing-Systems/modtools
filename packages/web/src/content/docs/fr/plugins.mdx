---
title: Plugins
description: Écrivez vos propres plugins pour étendre Mod.
---

Les plugins vous permettent d'étendre Mod en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Mod.
...
- `~/.config/MOD/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/MOD/node_modules/`.
...
1. Configuration globale (`~/.config/MOD/MOD.json`)
2. Configuration du projet (`MOD.json`)
3. Répertoire global des plugins (`~/.config/MOD/plugins/`)
...
Mod exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK MOD pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre MOD.
...
:::note
Si vous utilisez l'application de bureau Mod, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez MOD de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à MOD :
...
L'assistant `tool` crée un outil personnalisé que MOD peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour MOD aux côtés des outils intégrés.
