---
title: Plugins
description: Écrivez vos propres plugins pour étendre Model-of-Design Tools.
---

Les plugins vous permettent d'étendre Model-of-Design Tools en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Model-of-Design Tools.
...
- `.modtools/plugins/` - Plugins au niveau du projet
- `~/.config/modtools/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/modtools/node_modules/`.
...
1. Configuration globale (`~/.config/modtools/modtools.json`)
2. Configuration du projet (`modtools.json`)
3. Répertoire global des plugins (`~/.config/modtools/plugins/`)
4. Répertoire des plugins du projet (`.modtools/plugins/`)
...
Model-of-Design Tools exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK modtools pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre modtools.
...
:::note
Si vous utilisez l'application de bureau Model-of-Design Tools, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez modtools de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à modtools :
...
L'assistant `tool` crée un outil personnalisé que modtools peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour modtools aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://modtools.ai/docs/sdk) pour plus de détails.
