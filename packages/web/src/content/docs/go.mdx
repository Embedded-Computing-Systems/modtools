...
Model-of-Design Tools Go is a low cost subscription — **$5 for your first month**, then **$10/month** — that gives you reliable access to popular open coding models.

:::note
Model-of-Design Tools Go is currently in beta.
:::

Go works like any other provider in Model-of-Design Tools. You subscribe to Model-of-Design Tools Go and
get your API key. It's **completely optional** and you don't need to use it to
use Model-of-Design Tools.
...
Model-of-Design Tools Go gives you access to these models for **$5 for your first month**, then **$10/month**.

---

## How it works

Model-of-Design Tools Go works like any other provider in Model-of-Design Tools.

1. You sign in to **<a href={console}>Model-of-Design Tools Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `Model-of-Design Tools Go`, and paste
   your API key.
3. Run `/models` in the TUI to see the list of models available through Go.

:::note
Only one member per workspace can subscribe to Model-of-Design Tools Go.
:::
...
Model-of-Design Tools Go includes the following limits:
...
uses the format `modtools-go/<model-id>`. For example, for Kimi K2.5, you would
use `modtools-go/kimi-k2.5` in your config.
...
We created Model-of-Design Tools Go to:
...
4. Have **no lock-in** by allowing you to use any other provider with Model-of-Design Tools as well.
