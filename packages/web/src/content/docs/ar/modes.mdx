...
:::caution
يتم الآن ضبط الأوضاع عبر خيار `agent` في إعدادات modtools. أصبح خيار
`mode` مُهمَلًا الآن. [اعرف المزيد](/docs/agents).
:::

تتيح لك الأوضاع في modtools تخصيص السلوك والأدوات والمطالبات لحالات استخدام مختلفة.

يأتي مع وضعين مدمجين: **build** و **plan**. يمكنك تخصيصهما أو إعداد
أوضاعك الخاصة عبر إعدادات modtools.
...
- `edit` - لا يمكن تعديل الملفات الموجودة، باستثناء الملفات الموجودة في `.modtools/plans/*.md` لتفصيل الخطة نفسها
...
قم بإعداد الأوضاع في ملف الإعدادات `modtools.json` لديك:
...
- عالمي: `~/.config/modtools/modes/`
- للمشروع: `.modtools/modes/`
...
إذا لم يتم تحديد درجة الحرارة، يستخدم modtools القيم الافتراضية الخاصة بكل نموذج (عادةً 0 لمعظم النماذج و 0.55 لنماذج Qwen).
...
إعدادات modtools العالمية وكذلك إعدادات المشروع.
...
أنشئ ملفات الأوضاع في `.modtools/modes/` لأوضاع خاصة بالمشروع أو في `~/.config/modtools/modes/` لأوضاع عالمية:
