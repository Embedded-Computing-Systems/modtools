...
يمكنك إضافة أدوات خارجية إلى Mod باستخدام _Model Context Protocol_، أو MCP. يدعم Mod خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Mod](https://modtools.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/MOD` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Mod تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Mod بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Mod المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `MOD mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Mod الرموز بشكل آمن في `~/.local/share/MOD/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Mod إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Mod مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Mod بحساب Sentry الخاص بك.
