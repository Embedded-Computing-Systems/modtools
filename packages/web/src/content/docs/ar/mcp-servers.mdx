...
يمكنك إضافة أدوات خارجية إلى Model-of-Design Tools باستخدام _Model Context Protocol_، أو MCP. يدعم Model-of-Design Tools خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Model-of-Design Tools](https://modtools.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/modtools` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Model-of-Design Tools تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Model-of-Design Tools بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Model-of-Design Tools المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `modtools mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Model-of-Design Tools الرموز بشكل آمن في `~/.local/share/modtools/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Model-of-Design Tools إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Model-of-Design Tools مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Model-of-Design Tools بحساب Sentry الخاص بك.
