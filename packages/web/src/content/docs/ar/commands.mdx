...
يمكنك إضافة أوامر مخصصة عبر إعدادات Mod أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Mod:
...
- على مستوى النظام: `~/.config/MOD/commands/`
...
يتضمن MOD عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
