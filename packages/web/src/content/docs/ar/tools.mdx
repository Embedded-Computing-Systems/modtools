...
تتيح الأدوات لـ LLM تنفيذ إجراءات داخل قاعدة الشفرة الخاصة بك. يأتي Model-of-Design Tools مع مجموعة من الأدوات المدمجة، لكن يمكنك توسيعه عبر [أدوات مخصصة](/docs/custom-tools) أو [خوادم MCP](/docs/mcp-servers).
...
فيما يلي جميع الأدوات المدمجة المتاحة في Model-of-Design Tools.
...
:::note
هذه الأداة متاحة فقط عند ضبط `MOD_EXPERIMENTAL_LSP_TOOL=true` (أو `MOD_EXPERIMENTAL=true`).
:::
...
:::note
هذه الأداة متاحة فقط عند استخدام مزود Model-of-Design Tools أو عند ضبط متغير البيئة `MOD_ENABLE_EXA` على أي قيمة منطقية صحيحة (مثل `true` أو `1`).

للتفعيل عند تشغيل Model-of-Design Tools:
