---
title: المزوّدون
description: استخدام أي موفّر LLM في Model-of-Design Tools.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Model-of-Design Tools ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Model-of-Design Tools.
...
في `~/.local/share/modtools/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Model-of-Design Tools.
...
Model-of-Design Tools Zen هي قائمة نماذج يوفّرها فريق Model-of-Design Tools وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Model-of-Design Tools. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Model-of-Design Tools Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر modtools، ثم انتقل إلى [modtools.ai/auth](https://modtools.ai/auth).
...
يعمل مثل أي مزوّد آخر في Model-of-Design Tools واستخدامه اختياري بالكامل.

---

## Model-of-Design Tools Go

Model-of-Design Tools Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Model-of-Design Tools، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Model-of-Design Tools.

1. شغّل الأمر `/connect` في TUI، واختر `Model-of-Design Tools Go`، ثم انتقل إلى [modtools.ai/auth](https://modtools.ai/zen).
...
لاستخدام Amazon Bedrock مع Model-of-Design Tools:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل modtools:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `modtools.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Model-of-Design Tools غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل modtools بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات modtools.
...
تخزين الرمز في مساحة تخزين مصادقة modtools.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Model-of-Design Tools نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Model-of-Design Tools
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`modtools.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [modtools-gitlab-auth](https://www.npmjs.com/package/modtools-gitlab-auth).

##### التهيئة

خصّص عبر `modtools.json`:
...
لاستخدام اشتراك GitHub Copilot مع modtools:
...
لاستخدام Google Vertex AI مع Model-of-Design Tools:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر modtools، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [modtools-helicone-session](https://github.com/H2Shami/modtools-helicone-session) لتسجيل كل محادثة Model-of-Design Tools تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Model-of-Design Tools مدرجة كجلسة مستقلة.
...
يمكنك ضبط modtools لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط modtools لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط modtools لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Model-of-Design Tools. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/modtools) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Model-of-Design Tools:
...
3. انسخ مفتاح API لاستخدامه في Model-of-Design Tools.
...
6. **مهم**: قبل استخدام نماذج السحابة في Model-of-Design Tools، يجب سحب معلومات النموذج محليا:
...
Model-of-Design Tools Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Model-of-Design Tools. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Model-of-Design Tools Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Model-of-Design Tools Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Model-of-Design Tools.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات modtools.
...
5. يمكنك أيضا تخصيصها عبر إعدادات modtools. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Modtools:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات modtools. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع modtools. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `modtools.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Model-of-Design Tools بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `modtools auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات modtools وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات modtools.
