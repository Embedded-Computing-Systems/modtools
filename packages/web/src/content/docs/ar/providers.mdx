---
title: المزوّدون
description: استخدام أي موفّر LLM في MOD.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم MOD ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات MOD.
...
في `~/.local/share/MOD/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات MOD.
...
MOD Zen هي قائمة نماذج يوفّرها فريق MOD وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع MOD. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع MOD Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر MOD، ثم انتقل إلى [modtools.ai/auth](https://modtools.ai/auth).
...
يعمل مثل أي مزوّد آخر في MOD واستخدامه اختياري بالكامل.

---

## MOD Go

MOD Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق MOD، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع MOD.

1. شغّل الأمر `/connect` في TUI، واختر `MOD Go`، ثم انتقل إلى [modtools.ai/auth](https://opencode.ai/zen).
...
لاستخدام Amazon Bedrock مع MOD:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل MOD:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `MOD.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل MOD غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين MOD لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل MOD بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات MOD.
...
تخزين الرمز في مساحة تخزين مصادقة MOD.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم MOD نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر MOD
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`MOD.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [MOD-gitlab-auth](https://www.npmjs.com/package/MOD-gitlab-auth).
...
لاستخدام اشتراك GitHub Copilot مع MOD:
...
لاستخدام Google Vertex AI مع MOD:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر MOD، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [MOD-helicone-session](https://github.com/Embedded-Computing-Systems/modtools-helicone-session) لتسجيل كل محادثة MOD تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة MOD مدرجة كجلسة مستقلة.
...
يمكنك ضبط MOD لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط MOD لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط MOD لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع MOD. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/MOD) للتفاصيل.
:::
...
3. انسخ مفتاح API لاستخدامه في MOD.
...
6. **مهم**: قبل استخدام نماذج السحابة في MOD، يجب سحب معلومات النموذج محليا:
...
MOD Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق MOD. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>MOD Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **MOD Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ MOD.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات MOD.
...
5. يمكنك أيضا تخصيصها عبر إعدادات MOD. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Mod:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات MOD. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع MOD. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `MOD.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ MOD بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `MOD auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات MOD وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات MOD.
