---
title: الخادم
description: تفاعل مع خادم MOD عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `MOD serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل MOD استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `MOD.local` |
...
عيّن `MODTOOLS_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `MOD`، أو عيّن `MODTOOLS_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `MOD serve` و `MOD web`.
...
عند تشغيل `MOD` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم MOD للتفاعل مع MOD برمجيا.
:::

تتيح هذه البنية لـ MOD دعم عدة عملاء وتمكّنك من التفاعل مع MOD برمجيا.

يمكنك تشغيل `MOD serve` لبدء خادم مستقل. إذا كانت واجهة MOD في terminal (TUI)
قيد التشغيل، فسيبدأ `MOD serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات MOD لـ [IDE](/docs/ide).
...
يعرض خادم MOD واجهات API التالية.
