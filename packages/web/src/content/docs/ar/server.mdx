---
title: الخادم
description: تفاعل مع خادم modtools عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `modtools serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل modtools استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `modtools.local` |
...
عيّن `MOD_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `modtools`، أو عيّن `MOD_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `modtools serve` و `modtools web`.
...
عند تشغيل `modtools` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم modtools للتفاعل مع modtools برمجيا.
:::

تتيح هذه البنية لـ modtools دعم عدة عملاء وتمكّنك من التفاعل مع modtools برمجيا.

يمكنك تشغيل `modtools serve` لبدء خادم مستقل. إذا كانت واجهة modtools في terminal (TUI)
قيد التشغيل، فسيبدأ `modtools serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Model-of-Design Tools لـ [IDE](/docs/ide).
...
يعرض خادم modtools واجهات API التالية.
