---
title: الإضافات
description: اكتب إضافاتك الخاصة لتوسيع Model-of-Design Tools.
---

تتيح لك الإضافات توسيع Model-of-Design Tools عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ Model-of-Design Tools.
...
- `.modtools/plugins/` - إضافات على مستوى المشروع
- `~/.config/modtools/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/modtools/node_modules/`.
...
1. إعدادات عامة (`~/.config/modtools/modtools.json`)
2. إعدادات المشروع (`modtools.json`)
3. دليل الإضافات العام (`~/.config/modtools/plugins/`)
4. دليل إضافات المشروع (`.modtools/plugins/`)
...
يشغّل Model-of-Design Tools الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
- `client`: عميل SDK لـ modtools للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع modtools.
...
:::note
إذا كنت تستخدم تطبيق Model-of-Design Tools لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
...
امنع modtools من قراءة ملفات `.env`:
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى modtools:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ modtools استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ modtools إلى جانب الأدوات المضمنة.
...
المستويات: `debug`, `info`, `warn`, `error`. راجع [توثيق SDK](https://modtools.ai/docs/sdk) للتفاصيل.
