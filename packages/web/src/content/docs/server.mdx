---
title: Server
description: Interact with MOD server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `MOD serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an MOD client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `MOD.local` |
...
Set `MODTOOLS_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `MOD`, or set `MODTOOLS_SERVER_USERNAME` to override it. This applies to both `MOD serve` and `MOD web`.
...
When you run `MOD` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the MOD server to interact with MOD programmatically.
:::

This architecture lets MOD support multiple clients and allows you to interact with MOD programmatically.

You can run `MOD serve` to start a standalone server. If you have the
MOD TUI running, `MOD serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Mod [IDE](/docs/ide) plugins.
...
The MOD server exposes the following APIs.
