---
title: Server
description: Interact with modtools server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `modtools serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an modtools client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `modtools.local` |
...
Set `MOD_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `modtools`, or set `MOD_SERVER_USERNAME` to override it. This applies to both `modtools serve` and `modtools web`.
...
When you run `modtools` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the modtools server to interact with modtools programmatically.
:::

This architecture lets modtools support multiple clients and allows you to interact with modtools programmatically.

You can run `modtools serve` to start a standalone server. If you have the
modtools TUI running, `modtools serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Model-of-Design Tools [IDE](/docs/ide) plugins.
...
The modtools server exposes the following APIs.
