...
You can add external tools to MOD using the _Model Context Protocol_, or MCP. MOD supports both local and remote servers.
...
You can define MCP servers in your [MOD Config](https://modtools.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/MOD` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
MOD automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, MOD will:
...
If the server requires authentication, MOD will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `MOD mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, MOD will store the tokens securely in `~/.local/share/MOD/mcp-auth.json`.
...
Your MCPs are available as tools in MOD, alongside built-in tools. So you can manage them through the MOD config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect MOD to your Sentry account.
