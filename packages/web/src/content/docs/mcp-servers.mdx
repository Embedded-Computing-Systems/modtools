...
You can add external tools to Model-of-Design Tools using the _Model Context Protocol_, or MCP. Model-of-Design Tools supports both local and remote servers.
...
You can define MCP servers in your [Model-of-Design Tools Config](https://modtools.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/modtools` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Model-of-Design Tools automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Model-of-Design Tools will:
...
If the server requires authentication, Model-of-Design Tools will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `modtools mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Model-of-Design Tools will store the tokens securely in `~/.local/share/modtools/mcp-auth.json`.
...
Your MCPs are available as tools in Model-of-Design Tools, alongside built-in tools. So you can manage them through the Model-of-Design Tools config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Model-of-Design Tools to your Sentry account.
