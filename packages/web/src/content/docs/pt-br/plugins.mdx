---
title: Plugins
description: Escreva seus próprios plugins para estender o MOD.
---

Plugins permitem que você estenda o MOD conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do MOD.
...
- `~/.config/MOD/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/MOD/node_modules/`.
...
1. Configuração global (`~/.config/MOD/MOD.json`)
2. Configuração do projeto (`MOD.json`)
3. Diretório de plugins global (`~/.config/MOD/plugins/`)
...
O MOD executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do MOD para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o MOD.
...
:::note
Se você estiver usando o aplicativo desktop MOD, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o MOD de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao MOD:
...
O helper `tool` cria uma ferramenta personalizada que o MOD pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o MOD junto com as ferramentas integradas.
