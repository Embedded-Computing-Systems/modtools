---
title: Plugins
description: Escreva seus próprios plugins para estender o modtools.
---

Plugins permitem que você estenda o modtools conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do modtools.
...
- `.modtools/plugins/` - Plugins em nível de projeto
- `~/.config/modtools/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/modtools/node_modules/`.
...
1. Configuração global (`~/.config/modtools/modtools.json`)
2. Configuração do projeto (`modtools.json`)
3. Diretório de plugins global (`~/.config/modtools/plugins/`)
4. Diretório de plugins do projeto (`.modtools/plugins/`)
...
O modtools executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do modtools para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o modtools.
...
:::note
Se você estiver usando o aplicativo desktop modtools, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o modtools de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao modtools:
...
O helper `tool` cria uma ferramenta personalizada que o modtools pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o modtools junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://modtools.ai/docs/sdk) para detalhes.
