...
As habilidades do agente permitem que o modtools descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O modtools pesquisa nesses locais:

- Configuração do projeto: `.modtools/skills/<name>/SKILL.md`
- Configuração global: `~/.config/modtools/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o modtools sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.modtools/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/modtools/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.modtools/skills/git-release/SKILL.md` assim:
...
O modtools lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `modtools.json`:
...
**Para agentes embutidos** (em `modtools.json`):
