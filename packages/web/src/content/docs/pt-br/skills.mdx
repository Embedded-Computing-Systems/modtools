...
As habilidades do agente permitem que o MOD descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O MOD pesquisa nesses locais:

- Configuração do projeto: `.modtools/skills/<name>/SKILL.md`
- Configuração global: `~/.config/MOD/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o MOD sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
...
As definições globais também são carregadas de `~/.config/MOD/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
O MOD lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `MOD.json`:
...
**Para agentes embutidos** (em `MOD.json`):
