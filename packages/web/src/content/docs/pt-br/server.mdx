---
title: Servidor
description: Interaja com o servidor MOD via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `MOD serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente MOD pode usar.
...
```bash
MOD serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `MOD.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
MOD serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `MODTOOLS_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `MOD`, ou defina `MODTOOLS_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `MOD serve` quanto ao `MOD web`.

```bash
MODTOOLS_SERVER_PASSWORD=your-password MOD serve
```
...
Quando você executa `MOD`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor MOD para interagir com o MOD programaticamente.
:::

Essa arquitetura permite que o MOD suporte múltiplos clientes e permite que você interaja com o MOD programaticamente.

Você pode executar `MOD serve` para iniciar um servidor autônomo. Se você tiver o TUI do MOD em execução, `MOD serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do MOD [IDE](/docs/ide).
...
O servidor MOD expõe as seguintes APIs.
