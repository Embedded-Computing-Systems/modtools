---
title: Servidor
description: Interaja com o servidor modtools via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `modtools serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente modtools pode usar.
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `modtools.local` |
...
Defina `MOD_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `modtools`, ou defina `MOD_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `modtools serve` quanto ao `modtools web`.

```bash
MOD_SERVER_PASSWORD=your-password modtools serve
```
...
Quando você executa `modtools`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor modtools para interagir com o modtools programaticamente.
:::

Essa arquitetura permite que o modtools suporte múltiplos clientes e permite que você interaja com o modtools programaticamente.

Você pode executar `modtools serve` para iniciar um servidor autônomo. Se você tiver o TUI do modtools em execução, `modtools serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do modtools [IDE](/docs/ide).
...
O servidor modtools expõe as seguintes APIs.
