...
Você também pode definir agentes usando arquivos markdown em `~/.config/MOD/agents/` ou `.mod/agents/`. [Saiba mais aqui](/docs/agents).
...
Você também pode definir comandos usando arquivos markdown em `~/.config/MOD/commands/` ou `.mod/commands/`. [Saiba mais aqui](/docs/commands).
...
Coloque arquivos de plugin em `.mod/plugins/` ou `~/.config/MOD/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
