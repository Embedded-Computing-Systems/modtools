...
Com o modtools, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o modtools usa nosso próprio tema `modtools`.
...
O modtools vem com vários temas embutidos.
...
- Querem que o modtools corresponda à aparência do seu terminal
...
O modtools suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/modtools/themes/*.json` ou `$XDG_CONFIG_HOME/modtools/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.modtools/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.modtools/themes/*.json`
