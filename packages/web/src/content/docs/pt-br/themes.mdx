...
Com o MOD, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o MOD usa nosso próprio tema `MOD`.
...
O MOD vem com vários temas embutidos.
...
- Querem que o MOD corresponda à aparência do seu terminal
...
O MOD suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/MOD/themes/*.json` ou `$XDG_CONFIG_HOME/MOD/themes/*.json`
