...
description: O modtools integra-se aos seus servidores LSP.
---

O modtools integra-se ao seu Protocolo de Servidor de Linguagem (LSP) para ajudar o LLM a interagir com sua base de código. Ele usa diagnósticos para fornecer feedback ao LLM.

---

## Integrados

O modtools vem com vários servidores LSP integrados para linguagens populares:
...
:::note
Você pode desabilitar os downloads automáticos do servidor LSP definindo a variável de ambiente `MOD_DISABLE_LSP_DOWNLOAD` como `true`.
:::
...
Quando o modtools abre um arquivo, ele:
...
Você pode personalizar os servidores LSP através da seção `lsp` na sua configuração do modtools.
