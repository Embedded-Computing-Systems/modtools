...
Existem dois tipos de agentes no MOD; agentes primários e subagentes.
...
MOD vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Mod vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Mod vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Mod, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `MOD.json`:
...
- Global: `~/.config/MOD/agents/`
- Por projeto: `.modtools/agents/`

```markdown title="~/.config/MOD/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o MOD usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do MOD quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do MOD usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Mod Zen](/docs/zen), você usaria `MOD/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Execute `MOD models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/Embedded-Computing-Systems/modtools).
:::
...
```markdown title="~/.config/MOD/agents/docs-writer.md"
---
...
```markdown title="~/.config/MOD/agents/security-auditor.md"
---
