...
Existem dois tipos de agentes no modtools; agentes primários e subagentes.
...
modtools vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
modtools vem com dois subagentes integrados, **General** e **Explore**. Vamos ver isso abaixo.

---

## Integrados

modtools vem com dois agentes primários integrados e dois subagentes integrados.
...
Configure os agentes em seu arquivo de configuração `modtools.json`:
...
Se nenhuma temperatura for especificada, o modtools usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do modtools quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do modtools usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Model-of-Design Tools Zen](/docs/zen), você usaria `modtools/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Execute `modtools models` para ver uma lista dos modelos disponíveis.
:::
