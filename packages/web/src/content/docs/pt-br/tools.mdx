...
As ferramentas permitem que o LLM execute ações em sua base de código. O MOD vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no MOD.
...
:::note
Esta ferramenta está disponível apenas quando `MODTOOLS_EXPERIMENTAL_LSP_TOOL=true` (ou `MODTOOLS_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor MOD ou quando a variável de ambiente `MODTOOLS_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o MOD:
