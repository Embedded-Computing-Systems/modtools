---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Model-of-Design Tools.
---

import config from "../../../../config.mjs"
export const console = config.console

Model-of-Design Tools bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Model-of-Design Tools-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/modtools/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Model-of-Design Tools-konfigurationen.
...
Model-of-Design Tools Zen er en liste over modeller leveret af Model-of-Design Tools-teamet, som er blevet testet og verificeret til at fungere godt med Model-of-Design Tools. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Model-of-Design Tools Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg modtools og gå til [modtools.ai/auth](https://modtools.ai/auth).
...
Det fungerer som alle andre udbydere i Model-of-Design Tools og er helt valgfrit at bruge.

---

## Model-of-Design Tools Go

Model-of-Design Tools Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Model-of-Design Tools-teamet, som er testet og verificeret til at fungere godt med Model-of-Design Tools.

1. Kør kommandoen `/connect` i TUI, vælg `Model-of-Design Tools Go`, og gå til [modtools.ai/auth](https://modtools.ai/zen).
...
Sådan bruger du Amazon Bedrock med Model-of-Design Tools:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører modtools:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `modtools.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Model-of-Design Tools understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at modtools skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem modtools-konfigurationen.
...
gemme tokenet i modtools auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Model-of-Design Tools bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Model-of-Design Tools
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`modtools.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [modtools-gitlab-auth](https://www.npmjs.com/package/modtools-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `modtools.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Model-of-Design Tools:
...
Sådan bruger du Google Vertex AI med Model-of-Design Tools:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører modtools.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem modtools, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [modtools-helicone-session](https://github.com/H2Shami/modtools-helicone-session) plugin for automatisk at logge hver Model-of-Design Tools-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Model-of-Design Tools-samtale opført som en separat session.
...
Du kan konfigurere modtools til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere modtools til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere modtools til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Model-of-Design Tools. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/modtools) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Model-of-Design Tools:
...
3. Kopier API-nøglen til brug i Model-of-Design Tools.
...
6. **Vigtigt**: Før du bruger skymodeller i Model-of-Design Tools, skal du hente modelinformationen lokalt:
...
Model-of-Design Tools Zen er en liste over testede og verificerede modeller leveret af Model-of-Design Tools-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Model-of-Design Tools Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Model-of-Design Tools Zen**.

   ```txt
   /connect
   ```

3. Indtast Model-of-Design Tools API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem modtools-konfigurationen.
...
5. Du kan også tilpasse dem gennem modtools-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Model-of-Design Tools:
...
Du kan også tilpasse modeller gennem modtools-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med modtools. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `modtools.json`-filen i projektkataloget:
...
`limit`-felterne lader Model-of-Design Tools forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `modtools auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek modtools-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i modtools-konfigurationen.
