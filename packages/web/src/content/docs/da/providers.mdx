---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i MOD.
---

import config from "../../../../config.mjs"
export const console = config.console

MOD bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i MOD-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/MOD/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i MOD-konfigurationen.
...
MOD Zen er en liste over modeller leveret af MOD-teamet, som er blevet testet og verificeret til at fungere godt med MOD. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med MOD Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg MOD og gå til [modtools.ai/auth](https://modtools.ai/auth).
...
Det fungerer som alle andre udbydere i MOD og er helt valgfrit at bruge.

---

## MOD Go

MOD Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af MOD-teamet, som er testet og verificeret til at fungere godt med MOD.

1. Kør kommandoen `/connect` i TUI, vælg `MOD Go`, og gå til [modtools.ai/auth](https://opencode.ai/zen).
...
Sådan bruger du Amazon Bedrock med MOD:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører MOD:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `MOD.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i MOD understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere MOD til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at MOD skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem MOD-konfigurationen.
...
gemme tokenet i MOD auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
MOD bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse MOD
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`MOD.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [MOD-gitlab-auth](https://www.npmjs.com/package/MOD-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `MOD.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med MOD:
...
Sådan bruger du Google Vertex AI med MOD:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem MOD, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [MOD-helicone-session](https://github.com/Embedded-Computing-Systems/modtools-helicone-session) plugin for automatisk at logge hver MOD-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver MOD-samtale opført som en separat session.
...
Du kan konfigurere MOD til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere MOD til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere MOD til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for MOD. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/MOD) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med MOD:
...
3. Kopier API-nøglen til brug i MOD.
...
6. **Vigtigt**: Før du bruger skymodeller i MOD, skal du hente modelinformationen lokalt:
...
MOD Zen er en liste over testede og verificerede modeller leveret af MOD-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>MOD Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **MOD Zen**.

   ```txt
   /connect
   ```

3. Indtast MOD API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem MOD-konfigurationen.
...
5. Du kan også tilpasse dem gennem MOD-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med MOD:
...
Du kan også tilpasse modeller gennem MOD-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med MOD. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `MOD.json`-filen i projektkataloget:
...
`limit`-felterne lader MOD forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `MOD auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek MOD-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i MOD-konfigurationen.
