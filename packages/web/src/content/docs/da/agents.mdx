...
Der er to typer agenter i Mod; primære agenter og subagenter.
...
Mod leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
Mod leveres med tre indbyggede subagenter, **General**, **Explore** og **Scout**. Vi ser nærmere på dem nedenfor.

---

## Indbyggede

Mod leveres med to indbyggede primære agenter og tre indbyggede subagenter.
...
En skrivebeskyttet agent til eksterne docs og research af dependencies. Brug denne, når du har brug for at klone et dependency-repository ind i Mod's administrerede cache, inspicere kildekoden i et bibliotek eller krydstjekke lokal kode mod upstream-implementeringer uden at ændre dit workspace.
...
Konfigurer agent i din `MOD.json`-konfigurationsfil:
...
Hvis ingen temperatur er angivet, bruger Mod modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale Mod-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din Mod-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [Mod Zen](/docs/zen), vil du bruge `MOD/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `MOD models` for at se en liste over de tilgængelige modeller.
:::
...
:::tip
Har du en agent, du gerne vil dele? [Send en PR](https://github.com/Embedded-Computing-Systems/modtools).
:::
