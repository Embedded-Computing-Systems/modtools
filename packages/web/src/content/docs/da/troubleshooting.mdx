...
For at fejlfinde problemer med MOD, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\MOD\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `MOD --log-level DEBUG`.

---

## Lagring

MOD gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/MOD/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\MOD`
...
MOD Desktop kører en lokal MOD-server (`MOD-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `MOD`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/MOD/MOD.jsonc` (eller `~/.config/MOD/MOD.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/MOD/MOD.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\MOD\MOD.jsonc`
...
MOD kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/MOD/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\MOD\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så MOD kan genopbygge den.

1. Afslut MOD Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/MOD`
- **Linux**: slet `~/.cache/MOD` (eller kør `rm -rf ~/.cache/MOD`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\MOD`

3. Start MOD Desktop på ny.

---

### Løs problemer med serverforbindelse

MOD Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `MOD.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `MODTOOLS_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `MODTOOLS_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver MOD Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i MOD.

---

### Meddelelser vises ikke

MOD Desktop viser kun systemvarsler når:

- varsler er aktiveret for MOD i dine OS-indstillinger, og
...
1. Afslut MOD Desktop.
2. Find og slet disse filer (de findes i MOD Desktop-appens datamappe):

- `MOD.settings.dat` (skrivebordsstandardserver URL)
- `MOD.global.dat` og `MOD.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med MOD:
...
3. Sørg for at du har den nyeste version med `MOD upgrade`
...
For at finde ud af hvilke modeller du har adgang til, kør `MOD models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\MOD`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. MOD installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\MOD`

2. Start MOD på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge MOD til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
MOD vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
