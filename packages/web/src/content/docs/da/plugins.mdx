---
title: Plugins
description: Skriv dine egne plugins for at udvide Model-of-Design Tools.
---

Plugins giver dig mulighed for at udvide Model-of-Design Tools ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Model-of-Design Toolss standardadfærd.
...
- `.modtools/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/modtools/node_modules/`.
...
1. Global konfiguration (`~/.config/modtools/modtools.json`)
2. Projektkonfiguration (`modtools.json`)
3. Global plugin-mappe (`~/.config/modtools/plugins/`)
4. Projekt plugin bibliotek (`.modtools/plugins/`)
...
Model-of-Design Tools kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En modtools SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide modtools.
...
:::note
Hvis du bruger Model-of-Design Tools desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå modtools i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til modtools:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som modtools kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for modtools sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://modtools.ai/docs/sdk) for detaljer.
