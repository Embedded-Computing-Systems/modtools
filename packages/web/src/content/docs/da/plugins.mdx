---
title: Plugins
description: Skriv dine egne plugins for at udvide Mod.
---

Plugins giver dig mulighed for at udvide Mod ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Mods standardadfærd.
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/MOD/node_modules/`.
...
1. Global konfiguration (`~/.config/MOD/MOD.json`)
2. Projektkonfiguration (`MOD.json`)
3. Global plugin-mappe (`~/.config/MOD/plugins/`)
...
Mod kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En MOD SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide MOD.
...
:::note
Hvis du bruger Mod desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Plugins kan også tilføje brugerdefinerede værktøjer til MOD:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som MOD kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for MOD sammen med indbyggede værktøjer.
