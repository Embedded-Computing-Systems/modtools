...
Agentferdigheter lar Mod oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Mod søker etter disse stedene:

- Prosjektkonfigurasjon: `.modtools/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/MOD/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Mod opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
...
Globale definisjoner lastes også inn fra `~/.config/MOD/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Mod viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `MOD.json`:
...
**For innebygde agenter** (i `MOD.json`):
