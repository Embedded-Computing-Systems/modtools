...
Agentferdigheter lar Model-of-Design Tools oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Model-of-Design Tools søker etter disse stedene:

- Prosjektkonfigurasjon: `.modtools/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/modtools/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Model-of-Design Tools opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.modtools/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/modtools/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.modtools/skills/git-release/SKILL.md` slik:
...
Model-of-Design Tools viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `modtools.json`:
...
**For innebygde agenter** (i `modtools.json`):
