...
Med Model-of-Design Tools kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Model-of-Design Tools vårt eget `modtools`-tema.
...
Model-of-Design Tools støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/modtools/themes/*.json` eller `$XDG_CONFIG_HOME/modtools/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.modtools/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.modtools/themes/*.json`
