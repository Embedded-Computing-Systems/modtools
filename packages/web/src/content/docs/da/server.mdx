---
title: Server
description: Interager med modtools-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `modtools serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en modtools-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `modtools.local` |
...
Indstil `MOD_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `modtools`, eller indstil `MOD_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `modtools serve` og `modtools web`.
...
Når du kører `modtools` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug modtools-serveren til at interagere med modtools programmatisk.
:::

Denne arkitektur lader modtools understøtte flere klienter og lader dig interagere med modtools programmatisk.

Du kan køre `modtools serve` for at starte en selvstændig server. Hvis du har
modtools TUI kørende, vil `modtools serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Model-of-Design Tools [IDE](/docs/ide) plugins.
...
Model-of-Design Tools-serveren viser følgende API'er.
