---
title: Server
description: Interager med MOD-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `MOD serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en MOD-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `MOD.local` |
...
Indstil `MODTOOLS_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `MOD`, eller indstil `MODTOOLS_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `MOD serve` og `MOD web`.
...
Når du kører `MOD` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug MOD-serveren til at interagere med MOD programmatisk.
:::

Denne arkitektur lader MOD understøtte flere klienter og lader dig interagere med MOD programmatisk.

Du kan køre `MOD serve` for at starte en selvstændig server. Hvis du har
MOD TUI kørende, vil `MOD serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Mod [IDE](/docs/ide) plugins.
...
Mod-serveren viser følgende API'er.
