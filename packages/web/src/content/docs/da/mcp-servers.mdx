...
Du kan tilføje eksterne værktøjer til Model-of-Design Tools ved hjælp af _Model Context Protocol_ eller MCP. Model-of-Design Tools understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [Model-of-Design Tools Config](https://modtools.ai/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Organisationer kan levere standard MCP-servere via deres `.well-known/modtools`-slutpunkt. Disse servere kan være deaktiveret som standard, hvilket giver brugerne mulighed for at tilmelde sig dem, de har brug for.
...
Model-of-Design Tools håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil Model-of-Design Tools:
...
Hvis serveren kræver godkendelse, vil Model-of-Design Tools bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `modtools mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil Model-of-Design Tools opbevare tokens sikkert i `~/.local/share/modtools/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem Model-of-Design Tools-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde Model-of-Design Tools til din Sentry-konto.
