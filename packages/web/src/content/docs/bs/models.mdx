...
Model-of-Design Tools koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM providera**, uključujući lokalne modele.

## Provajderi

Većina popularnih provajdera su unaprijed učitani prema zadanim postavkama. Ako ste dodali vjerodajnice za provajdera putem naredbe `/connect`, oni će biti dostupni kada pokrenete Model-of-Design Tools.
...
Evo nekoliko modela koji dobro rade sa Model-of-Design Tools, bez posebnog redosleda. (Ovo nije potpuna lista niti je nužno ažurirana):
...
Ovdje je puni ID `provider_id/model_id`. Na primjer, ako koristite [Model-of-Design Tools Zen](/docs/zen), koristili biste `modtools/gpt-5.1-codex` za GPT 5.1 Codex.
...
Mnogi modeli podržavaju više varijanti sa različitim konfiguracijama. Model-of-Design Tools se isporučuje sa ugrađenim podrazumevanim varijantama za popularne provajdere.

### Ugrađene varijante

Model-of-Design Tools se isporučuje sa zadanim varijantama za mnoge provajdere:
**Anthropic**:
...
Kada se Model-of-Design Tools pokrene, on provjerava modele u sljedećem prioritetnom redoslijedu:

1. Oznaka komandne linije `--model` ili `-m`. Format je isti kao u konfiguracijskoj datoteci: `provider_id/model_id`.
2. Lista modela u Model-of-Design Tools konfiguraciji.
