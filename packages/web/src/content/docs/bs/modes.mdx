...
ove ili konfigurirajte svoje putem modtools config.
...
modtools dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.modtools/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `modtools.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/modtools/modes/`
- Projekat: `.modtools/modes/`
...
Ako temperatura nije navedena, modtools koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.modtools/modes/` za specifične načine rada ili `~/.config/modtools/modes/` za globalne načine:
