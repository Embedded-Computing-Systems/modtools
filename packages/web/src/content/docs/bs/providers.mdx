---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Model-of-Design Tools.
---

import config from "../../../../config.mjs"
export const console = config.console

Model-of-Design Tools koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Model-of-Design Tools konfiguraciji.
...
u `~/.local/share/modtools/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Model-of-Design Tools
config.
...
Model-of-Design Tools Zen je lista modela koje je obezbedio Model-of-Design Tools tim koji su bili
testirano i potvrđeno da dobro radi sa Model-of-Design Tools. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Model-of-Design Tools Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite modtools i idite na [modtools.ai/auth](https://modtools.ai/auth).
...
Radi kao i svaki drugi provajder u Model-of-Design Tools i potpuno je opcionalan za korištenje.

---

## Model-of-Design Tools Go

Model-of-Design Tools Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Model-of-Design Tools tim i koji su testirani i verificirani da dobro rade s Model-of-Design Tools-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Model-of-Design Tools Go` i idite na [modtools.ai/auth](https://modtools.ai/zen).
...
Da biste koristili Amazon Bedrock s Model-of-Design Tools:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete modtools:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `modtools.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Model-of-Design Tools.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi modtools ispravno radio.
   :::
...
Također možete dodati modele kroz svoju modtools konfiguraciju.
...
3.  Naziv: `Model-of-Design Tools`, opseg: `api`
...
da pohrani token u modtools auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Model-of-Design Tools koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Model-of-Design Tools
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`modtools.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [modtools-gitlab-auth](https://www.npmjs.com/package/modtools-gitlab-auth).

##### Konfiguracija

Prilagodite putem `modtools.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s modtools:
...
Za korištenje Google Vertex AI s Model-of-Design Tools:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće modtools.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem modtoolsa, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [modtools-helicone-session](https://github.com/H2Shami/modtools-helicone-session) da automatski prijavite svaki Model-of-Design Tools razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Model-of-Design Tools razgovor naveden kao zasebna sesija.
...
Možete konfigurirati modtools za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati modtools za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati modtools za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Model-of-Design Tools. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/modtools) za detalje.
:::
...
Da biste koristili Ollama Cloud s Model-of-Design Tools:
...
3. Kopirajte API ključ za korištenje u Model-of-Design Tools.
...
6. **Važno**: Prije upotrebe modela oblaka u Model-of-Design Tools, morate lokalno povući informacije o modelu:
...
Model-of-Design Tools Zen je lista testiranih i verifikovanih modela koju je obezbedio Model-of-Design Tools tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Model-of-Design Tools Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Model-of-Design Tools Zen**.

```txt
   /connect
```

3. Unesite svoj Model-of-Design Tools API ključ.
...
Također možete dodati dodatne modele putem vaše modtools konfiguracije.
...
5. Također ih možete prilagoditi putem vaše modtools konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Modtoolsom:
...
Također možete prilagoditi modele kroz svoju modtools konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s modtools-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `modtools.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Model-of-Design Tools da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `modtools auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Model-of-Design Tools konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj modtools konfiguraciji.
