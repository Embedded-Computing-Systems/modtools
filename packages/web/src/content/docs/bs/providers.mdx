---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u MOD.
---

import config from "../../../../config.mjs"
export const console = config.console

MOD koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj MOD konfiguraciji.
...
u `~/.local/share/MOD/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem MOD
config.
...
MOD Zen je lista modela koje je obezbedio MOD tim koji su bili
testirano i potvrđeno da dobro radi sa MOD. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa MOD Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite MOD i idite na [modtools.ai/auth](https://modtools.ai/auth).
...
Radi kao i svaki drugi provajder u MOD i potpuno je opcionalan za korištenje.

---

## MOD Go

MOD Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža MOD tim i koji su testirani i verificirani da dobro rade s MOD-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `MOD Go` i idite na [modtools.ai/auth](https://opencode.ai/zen).
...
Da biste koristili Amazon Bedrock s MOD:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete MOD:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `MOD.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u MOD.
:::
...
Možete konfigurirati MOD za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi MOD ispravno radio.
   :::
...
Također možete dodati modele kroz svoju MOD konfiguraciju.
...
da pohrani token u MOD auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
MOD koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate MOD
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`MOD.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [MOD-gitlab-auth](https://www.npmjs.com/package/MOD-gitlab-auth).

##### Konfiguracija

Prilagodite putem `MOD.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s MOD:
...
Za korištenje Google Vertex AI s MOD:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće MOD.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem MODa, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [MOD-helicone-session](https://github.com/Embedded-Computing-Systems/modtools-helicone-session) da automatski prijavite svaki MOD razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki MOD razgovor naveden kao zasebna sesija.
...
Možete konfigurirati MOD za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati MOD za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati MOD za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za MOD. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/MOD) za detalje.
:::
...
Da biste koristili Ollama Cloud s MOD:
...
3. Kopirajte API ključ za korištenje u MOD.
...
6. **Važno**: Prije upotrebe modela oblaka u MOD, morate lokalno povući informacije o modelu:
...
MOD Zen je lista testiranih i verifikovanih modela koju je obezbedio MOD tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>MOD Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **MOD Zen**.

```txt
   /connect
```

3. Unesite svoj MOD API ključ.
...
Također možete dodati dodatne modele putem vaše MOD konfiguracije.
...
5. Također ih možete prilagoditi putem vaše MOD konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Modom:
...
Također možete prilagoditi modele kroz svoju MOD konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s MOD-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `MOD.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju MOD da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `MOD auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite MOD konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj MOD konfiguraciji.
