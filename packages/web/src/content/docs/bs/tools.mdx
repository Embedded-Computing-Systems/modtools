...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Mod dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Mod.
...
:::note
Ovaj alat je dostupan samo kada je `MODTOOLS_EXPERIMENTAL_LSP_TOOL=true` (ili `MODTOOLS_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Mod provajdera ili kada je varijabla `MODTOOLS_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Mod:
