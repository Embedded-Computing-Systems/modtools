...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Model-of-Design Tools dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Model-of-Design Tools.
...
:::note
Ovaj alat je dostupan samo kada je `MOD_EXPERIMENTAL_LSP_TOOL=true` (ili `MOD_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Model-of-Design Tools provajdera ili kada je varijabla `MOD_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Model-of-Design Tools:
