...
Agent skills omogucavaju Model-of-Design Tools da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Model-of-Design Tools pretrazuje ove lokacije:

- Konfiguracija projekta: `.modtools/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/modtools/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Model-of-Design Tools ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.modtools/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/modtools/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.modtools/skills/git-release/SKILL.md` ovako:
...
Model-of-Design Tools navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `modtools.json`:
...
**Za ugradene agente** (u `modtools.json`):
