---
title: Konfiguracija
description: Korištenje Model-of-Design Tools JSON konfiguracije.
---

Možete konfigurirati Model-of-Design Tools koristeći JSON konfiguracijski fajl.

---

## Format

Model-of-Design Tools podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/modtools`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/modtools/modtools.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`MOD_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`modtools.json` u projektu) - postavke specifične za projekat
5. **`.modtools` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`MOD_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.modtools` i `~/.config/modtools` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/modtools` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Model-of-Design Tools konfiguraciju u `~/.config/modtools/modtools.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/modtools/tui.json`.
...
Dodajte `modtools.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Model-of-Design Tools pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `MOD_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `MOD_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.modtools` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.modtools`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`modtools.ai/config.json`**](https://modtools.ai/config.json).

TUI konfiguracija koristi [**`modtools.ai/tui.json`**](https://modtools.ai/tui.json).
...
Koristite `MOD_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `modtools.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `modtools serve` i `modtools web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Model-of-Design Tools server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `modtools.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Model-of-Design Tools konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Model-of-Design Tools pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/modtools/agents/` ili `.modtools/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Model-of-Design Tools će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`modtools run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/modtools/commands/` ili `.modtools/commands/`. [Saznajte više ovdje](/docs/commands).
...
Model-of-Design Tools će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Model-of-Design Tools **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Model-of-Design Tools sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.modtools/plugins/` ili `~/.config/modtools/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Model-of-Design Tools da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
