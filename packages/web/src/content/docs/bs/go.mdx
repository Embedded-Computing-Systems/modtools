...
Mod Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u Mod-u. Pretplatite se na Mod Go i
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa Mod-om.
:::
...
Mod Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

Mod Go radi kao bilo koji drugi provajder u Mod-u.

1. Prijavite se na **<a href={console}>Mod Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `Mod Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na Mod Go.
:::
...
Mod Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj Mod konfiguraciji
koristi format `MOD-go/<model-id>`. Na primjer, za Kimi K2.7 Code, koristili biste
`MOD-go/kimi-k2.7-code` u svojoj konfiguraciji.
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz Mod koristite i bilo kojeg drugog provajdera.
