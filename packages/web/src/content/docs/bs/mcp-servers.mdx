...
Možete dodati vanjske alate u Model-of-Design Tools koristeći _Model Context Protocol_, ili MCP. Model-of-Design Tools podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Model-of-Design Tools Config](https://modtools.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/modtools`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Model-of-Design Tools automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Model-of-Design Tools će:
...
Ako server zahtijeva autentifikaciju, Model-of-Design Tools će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `modtools mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Model-of-Design Tools sigurno čuva tokene u `~/.local/share/modtools/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Model-of-Design Tools, zajedno s ugrađenim alatima. Možete njima upravljati kroz Model-of-Design Tools konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Model-of-Design Tools sa vašim Sentry nalogom.
