...
Možete dodati vanjske alate u Mod koristeći _Model Context Protocol_, ili MCP. Mod podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Mod Config](https://modtools.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/MOD`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Mod automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Mod će:
...
Ako server zahtijeva autentifikaciju, Mod će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `MOD mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Mod sigurno čuva tokene u `~/.local/share/MOD/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Mod, zajedno s ugrađenim alatima. Možete njima upravljati kroz Mod konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Mod sa vašim Sentry nalogom.
