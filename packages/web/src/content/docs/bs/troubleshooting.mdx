...
Da biste otklonili probleme s MOD, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\MOD\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `MOD --log-level DEBUG`.

---

## Pohrana

MOD pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/MOD/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\MOD`
...
MOD Desktop pokreće lokalni MOD server (`MOD-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `MOD` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/MOD/MOD.jsonc` (ili `~/.config/MOD/MOD.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/MOD/MOD.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\MOD\MOD.jsonc`
...
MOD također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/MOD/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\MOD\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga MOD mogao ponovo izgraditi.

1. Potpuno zatvorite MOD Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/MOD`
- **Linux**: obrišite `~/.cache/MOD` (ili pokrenite `rm -rf ~/.cache/MOD`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\MOD`

3. Ponovo pokrenite MOD Desktop.

---

### Rješavanje problema sa vezom na serveru

MOD Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `MOD.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `MODTOOLS_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `MODTOOLS_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, MOD Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa MOD karakteristikama.

---

### Obavještenja se ne prikazuju

MOD Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za MOD u postavkama vašeg OS-a, i
...
1. Zatvorite MOD Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka MOD Desktop aplikacije):

- `MOD.settings.dat` (URL zadanog servera za desktop)
- `MOD.global.dat` i `MOD.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
3. Uvjerite se da imate najnoviju verziju sa `MOD upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `MOD models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\MOD`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. MOD dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\MOD`

2. Ponovo pokrenite MOD da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti MOD da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
MOD će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
