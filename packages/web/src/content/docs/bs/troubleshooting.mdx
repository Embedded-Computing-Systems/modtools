...
Da biste otklonili probleme s Model-of-Design Tools, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\modtools\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `modtools --log-level DEBUG`.

---

## Pohrana

Model-of-Design Tools pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/modtools/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\modtools`
...
Model-of-Design Tools Desktop pokreće lokalni Model-of-Design Tools server (`modtools-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Model-of-Design Tools` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/modtools/modtools.jsonc` (ili `~/.config/modtools/modtools.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/modtools/modtools.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\modtools\modtools.jsonc`
...
Model-of-Design Tools također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/modtools/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\modtools\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Model-of-Design Tools mogao ponovo izgraditi.

1. Potpuno zatvorite Model-of-Design Tools Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/modtools`
- **Linux**: obrišite `~/.cache/modtools` (ili pokrenite `rm -rf ~/.cache/modtools`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\modtools`

3. Ponovo pokrenite Model-of-Design Tools Desktop.

---

### Rješavanje problema sa vezom na serveru

Model-of-Design Tools Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `modtools.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `MOD_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `MOD_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Model-of-Design Tools Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Model-of-Design Tools karakteristikama.

---

### Obavještenja se ne prikazuju

Model-of-Design Tools Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Model-of-Design Tools u postavkama vašeg OS-a, i
...
1. Zatvorite Model-of-Design Tools Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Model-of-Design Tools Desktop aplikacije):

- `modtools.settings.dat` (URL zadanog servera za desktop)
- `modtools.global.dat` i `modtools.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s Model-of-Design Tools:
...
Da saznate kojim modelima imate pristup, pokrenite `modtools models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\modtools`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Model-of-Design Tools dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\modtools`

2. Ponovo pokrenite Model-of-Design Tools da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Model-of-Design Tools da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Model-of-Design Tools će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
