---
title: Server
description: Komunicirajte s modtools serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `modtools serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji modtools klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `modtools.local` |
...
Postavite `MOD_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `modtools`, ili postavite `MOD_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `modtools serve` i za `modtools web`.
...
Kada pokrenete `modtools`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite modtools server za programsku interakciju sa modtools-om.
:::

Ova arhitektura omogucava modtools podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `modtools serve` da startate standalone server. Ako je modtools TUI vec pokrenut, `modtools serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Model-of-Design Tools [IDE](/docs/ide) pluginovi.
...
modtools server izlaže sljedece API-je.
