---
title: Server
description: Komunicirajte s MOD serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `MOD serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji MOD klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `MOD.local` |
...
Postavite `MODTOOLS_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `MOD`, ili postavite `MODTOOLS_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `MOD serve` i za `MOD web`.
...
Kada pokrenete `MOD`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite MOD server za programsku interakciju sa MOD-om.
:::

Ova arhitektura omogucava MOD podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `MOD serve` da startate standalone server. Ako je MOD TUI vec pokrenut, `MOD serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste MOD [IDE](/docs/ide) pluginovi.
...
MOD server izlaže sljedece API-je.
