---
title: LSP serveri
description: MOD se integriše sa vašim LSP serverima.
---

MOD se može integrisati sa Language Server Protocol (LSP) serverima kako bi koristio dijagnostiku kao feedback za agenta.

## Ugrađeni

MOD dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `MODTOOLS_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada je LSP omogućen i MOD otvori fajl, on:
...
Možete omogućiti i prilagoditi LSP servere kroz `lsp` odjeljak u vašoj MOD konfiguraciji.
