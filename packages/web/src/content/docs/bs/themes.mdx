...
U Model-of-Design Tools mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Model-of-Design Tools koristi nasu `modtools` temu.
...
Model-of-Design Tools dolazi sa vise ugradenih tema.
...
- Zele da Model-of-Design Tools odgovara izgledu njihovog terminala
...
Model-of-Design Tools podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/modtools/themes/*.json` ili `$XDG_CONFIG_HOME/modtools/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.modtools/themes/*.json`
4. **Trenutni radni direktorij** - `./.modtools/themes/*.json`
