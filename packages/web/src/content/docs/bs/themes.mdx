...
U Mod mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Mod koristi nasu `MOD` temu.
...
Mod dolazi sa vise ugradenih tema.
...
- Zele da Mod odgovara izgledu njihovog terminala
...
Mod podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/MOD/themes/*.json` ili `$XDG_CONFIG_HOME/MOD/themes/*.json`
