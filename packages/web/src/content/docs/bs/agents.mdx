...
Postoje dvije vrste agenata u Mod: primarni agenti i podagenti.
...
Mod dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Mod dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Mod dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u Mod-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `MOD.json`:
...
- Globalno: `~/.config/MOD/agents/`
- Po projektu: `.modtools/agents/`
...
Ako temperatura nije navedena, Mod koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Mod konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Mod konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Mod Zen](/docs/zen), koristili biste `MOD/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `MOD models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/Embedded-Computing-Systems/modtools).
:::
