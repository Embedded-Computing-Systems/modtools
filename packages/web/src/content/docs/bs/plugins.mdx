---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Mod.
---

Dodaci vam omogućavaju da proširite Mod spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Mod.
...
- `~/.config/MOD/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/MOD/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/MOD/MOD.json`)
2. Konfiguracija projekta (`MOD.json`)
3. Globalni direktorij dodataka (`~/.config/MOD/plugins/`)
...
Mod pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Mod SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Mod.
...
Spriječite MOD da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u MOD:
...
Pomoćnik `tool` kreira prilagođeni alat koji MOD može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za MOD zajedno sa ugrađenim alatima.
