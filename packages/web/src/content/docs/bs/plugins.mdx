---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Model-of-Design Tools.
---

Dodaci vam omogućavaju da proširite Model-of-Design Tools spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Model-of-Design Tools.
...
- `.modtools/plugins/` - Dodaci na nivou projekta
- `~/.config/modtools/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/modtools/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/modtools/modtools.json`)
2. Konfiguracija projekta (`modtools.json`)
3. Globalni direktorij dodataka (`~/.config/modtools/plugins/`)
4. Direktorij dodataka projekta (`.modtools/plugins/`)
...
Model-of-Design Tools pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Modtools SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Model-of-Design Tools.
...
Spriječite modtools da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u modtools:
...
Pomoćnik `tool` kreira prilagođeni alat koji modtools može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za modtools zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://modtools.ai/docs/sdk) za detalje.
