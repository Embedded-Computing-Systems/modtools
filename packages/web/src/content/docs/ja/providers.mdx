---
title: プロバイダー
description: Model-of-Design Tools で LLM プロバイダーを使用する。
---

import config from "../../../../config.mjs"
export const console = config.console

Model-of-Design Tools は [AI SDK](https://ai-sdk.dev/) および [Models.dev](https://models.dev) を使用して **75 以上の LLM プロバイダー** をサポートし、ローカルモデルの実行をサポートします。

プロバイダーを追加するには、次のことを行う必要があります。

1. `/connect` コマンドを使用して、プロバイダーの API キーを追加します。
2. Model-of-Design Tools 設定でプロバイダーを構成します。
...
`~/.local/share/modtools/auth.json`で。

---

### 設定

Model-of-Design Tools の `provider` セクションを通じてプロバイダーをカスタマイズできます。
config.
...
Model-of-Design Tools Zen は、Model-of-Design Tools チームが提供するモデルのリストです。
Model-of-Design Tools で適切に動作することがテストおよび検証されています。 [詳細はこちら](/docs/zen)。

:::tip
初めての方は、Model-of-Design Tools Zen から始めることをお勧めします。
:::

1. TUI で `/connect` コマンドを実行し、`Model-of-Design Tools Zen` を選択して、[modtools.ai/zen](https://modtools.ai/zen) にアクセスします。
...
これは Model-of-Design Tools の他のプロバイダーと同様に機能し、使用は完全にオプションです。

---

## Model-of-Design Tools Go

Model-of-Design Tools Go は、Model-of-Design Tools チームによって提供される、人気のあるオープンコーディングモデルへの信頼性の高いアクセスを提供する低コストのサブスクリプションプランです。これらは Model-of-Design Tools でうまく機能することがテストおよび検証されています。

1. TUI で `/connect` コマンドを実行し、`Model-of-Design Tools Go` を選択して、[modtools.ai/zen](https://modtools.ai/zen) にアクセスします。
...
Model-of-Design Tools で Amazon Bedrock を使用するには:
...
2. **次のいずれかの方法を使用して認証を構成します**:
   ...
   ***
   ...
   modtools の実行中に次の環境変数のいずれかを設定します。
   ...
   ***
   ...
   プロジェクト固有の設定または永続的な設定の場合は、`modtools.json` を使用します。
...
:::info
Model-of-Design Tools での Claude Pro/Max サブスクリプションの使用は、[Anthropic](https://anthropic.com) によって正式に許可されていませんが、一般的に機能します。
:::
...
:::note
Model-of-Design Tools が正しく動作するには、デプロイメント名がモデル名と一致する必要があります。
:::
...
Model-of-Design Tools 設定を通じてモデルを追加することもできます。
...
3.  名前: `Model-of-Design Tools`、スコープ: `api`
...
トークンを Model-of-Design Tools 認証ストレージに保存します。
:::

##### 自己ホスト型 GitLab

:::note[コンプライアンスメモ]
Model-of-Design Tools は、セッションタイトルの生成などの一部の AI タスクに小規模なモデルを使用します。
デフォルトでは、Zen によってホストされる gpt-5-nano を使用するように構成されています。 Model-of-Design Tools をロックするには
自分の GitLab でホストされているインスタンスのみを使用するには、次の行を
`modtools.json` ファイル。セッション共有を無効にすることもお勧めします。
...
詳細については、[modtools-gitlab-auth](https://www.npmjs.com/package/modtools-gitlab-auth) ホームページ。

##### 設定

`modtools.json` を通じてカスタマイズします。
...
GitHub Copilot サブスクリプションを Model-of-Design Tools で使用するには:
...
Model-of-Design Tools で Google Vertex AI を使用するには:
...
Model-of-Design Tools を通じて自動的に構成されていない Helicone の機能またはモデルを見つけた場合は、いつでも自分で構成できます。
...
Helicone の [Sessions](https://docs.helicone.ai/features/sessions) 機能を使用すると、関連する LLM リクエストをグループ化できます。 [modtools-helicone-session](https://github.com/H2Shami/modtools-helicone-session) プラグインを使用して、各 Model-of-Design Tools 会話を Helicone のセッションとして自動的に記録します。
...
プラグインは、リクエストに `Helicone-Session-Id` ヘッダーと `Helicone-Session-Name` ヘッダーを挿入します。 Helicone のセッションページでは、Model-of-Design Tools の各会話が個別のセッションとしてリストされています。
...
[llama.cpp の s](https://github.com/ggml-org/llama.cpp) llama-server ユーティリティ] を通じて、ローカルモデルを使用するように modtools を構成できます。
...
LM Studio を通じてローカルモデルを使用するように modtools を構成できます。
...
Ollama を通じてローカルモデルを使用するように modtools を構成できます。

:::tip
Ollama は Model-of-Design Tools 用に自動的に構成できます。詳細については、[Ollama 統合 docs](https://docs.ollama.com/integrations/modtools)」を参照してください。
:::
...
6. **重要**: Model-of-Design Tools でクラウドモデルを使用する前に、モデル情報をローカルに取得する必要があります。
...
Model-of-Design Tools Zen は、Model-of-Design Tools チームによって提供される、テストおよび検証されたモデルのリストです。 [詳細はこちら](/docs/zen)。

1. **<a href={console}>Model-of-Design Tools Zen</a>** にサインインし、**API キーの作成** をクリックします。

2. `/connect` コマンドを実行し、**Model-of-Design Tools Zen** を検索します。

   ```txt
   /connect
   ```

3. Model-of-Design Tools API キーを入力します。
...
Model-of-Design Tools 設定を通じて追加のモデルを追加することもできます。
...
5. Model-of-Design Tools 設定を通じてカスタマイズすることもできます。プロバイダーを指定する例を次に示します。
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) を Modtools で使用するには:
...
Model-of-Design Tools 設定を通じてモデルをカスタマイズすることもできます。プロバイダーのルーティング順序を指定する例を次に示します。
...
:::tip
OpenAI と互換性のある任意のプロバイダーを Model-of-Design Tools で使用できます。最新の AI プロバイダーのほとんどは、OpenAI 互換の API を提供しています。
:::
...
4. プロジェクトディレクトリで `modtools.json` ファイルを作成または更新します。
...
`limit` フィールドを使用すると、Model-of-Design Tools はコンテキストがどのくらい残っているかを理解できます。標準プロバイダーは、これらを models.dev から自動的に取得します。
...
1. **認証設定を確認します**: `modtools auth list` を実行して、資格情報が正しいかどうかを確認します。
   プロバイダー用のものが設定に追加されます。

これは、認証に環境変数に依存する Amazon Bedrock のようなプロバイダーには当てはまりません。

2. カスタムプロバイダーの場合は、Model-of-Design Tools 設定を確認し、次のことを行います。
   - `/connect` コマンドで使用されるプロバイダー ID が、Model-of-Design Tools 設定内の ID と一致することを確認してください。
