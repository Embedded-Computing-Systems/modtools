...
エージェントスキルにより、Model-of-Design Tools はリポジトリまたはホームディレクトリから再利用可能な命令を検出できます。
...
Model-of-Design Tools は次の場所を検索します。

- プロジェクト設定: `.modtools/skills/<name>/SKILL.md`
- グローバル設定: `~/.config/modtools/skills/<name>/SKILL.md`
...
プロジェクトのローカルパスの場合、Model-of-Design Tools は現在の作業ディレクトリから git ワークツリーに到達するまで進みます。
途中で、一致する `skills/*/SKILL.md` を `.modtools/` に読み込み、一致する `.claude/skills/*/SKILL.md` または `.agents/skills/*/SKILL.md` を読み込みます。

グローバル定義は、`~/.config/modtools/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md`、および `~/.agents/skills/*/SKILL.md` からもロードされます。
...
次のように `.modtools/skills/git-release/SKILL.md` を作成します。
...
Model-of-Design Tools では、`skill` ツールの説明に利用可能なスキルがリストされています。
...
`modtools.json` のパターンベースの権限を使用して、エージェントがアクセスできるスキルを制御します。
