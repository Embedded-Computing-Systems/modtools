...
ツールを使用すると、LLM がコードベースでアクションを実行できるようになります。 Model-of-Design Tools には一連の組み込みツールが付属していますが、[カスタムツール](/docs/custom-tools) または [MCP サーバー](/docs/mcp-servers) を使用して拡張できます。
...
Model-of-Design Tools で利用可能なすべての組み込みツールを次に示します。
...
:::note
このツールは、`MOD_EXPERIMENTAL_LSP_TOOL=true` (または `MOD_EXPERIMENTAL=true`) の場合にのみ使用できます。
:::
...
:::note
このツールは、Model-of-Design Tools プロバイダーを使用している場合、または `MOD_ENABLE_EXA` 環境変数が真実の値 (`true` または `1` など) に設定されている場合にのみ使用できます。

Model-of-Design Tools の起動時に有効にするには:
