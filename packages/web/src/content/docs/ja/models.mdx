...
最も一般的なプロバイダーはデフォルトでプリロードされています。 `/connect` コマンドを使用してプロバイダーの資格情報を追加した場合は、Model-of-Design Tools を開始するときにそれらの資格情報を使用できるようになります。
...
ここでは、Model-of-Design Tools で適切に動作するいくつかのモデルを順不同で示します。 (これは完全なリストではなく、必ずしも最新であるとは限りません):
...
これらのいずれかをデフォルトのモデルとして設定するには、`model` キーを Model-of-Design Tools の設定に追加します。
...
ここでの完全な ID は `provider_id/model_id` です。たとえば、[Model-of-Design Tools Zen](/docs/zen) を使用している場合、GPT 5.1 Codex には `modtools/gpt-5.1-codex` を使用します。
...
多くのモデルは、異なる構成の複数のバリアントをサポートしています。 Model-of-Design Tools には、一般的なプロバイダーのデフォルトのバリアントが組み込まれています。

### 組み込みバリアント

Model-of-Design Tools には、多くのプロバイダーのデフォルトのバリアントが付属しています。
...
Model-of-Design Tools が起動すると、次の優先順位でモデルがチェックされます。

1. `--model` または `-m` コマンドラインフラグ。形式は構成ファイルと同じです: `provider_id/model_id`。

2. Model-of-Design Tools 構成内のモデルのリスト。
