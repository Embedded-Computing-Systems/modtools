---
title: サーバー
description: HTTP 経由で Mod サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`MOD serve` コマンドは、Mod クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `MOD.local` |
...
HTTP 基本認証でサーバーを保護するには、`MODTOOLS_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `MOD` になるか、`MODTOOLS_SERVER_USERNAME` を設定してオーバーライドします。これは、`MOD serve` と `MOD web` の両方に当てはまります。
...
`MOD` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Mod サーバーを使用して、プログラムで Mod と対話します。
:::

`MOD serve` を実行してスタンドアロンサーバーを起動できます。すでに Mod TUI を実行している場合でも、`MOD serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Mod [IDE](/docs/ide) プラグインによって使用されます。
