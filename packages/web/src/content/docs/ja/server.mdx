---
title: サーバー
description: HTTP 経由で Model-of-Design Tools サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`modtools serve` コマンドは、Model-of-Design Tools クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `modtools.local` |
...
HTTP 基本認証でサーバーを保護するには、`MOD_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `modtools` になるか、`MOD_SERVER_USERNAME` を設定してオーバーライドします。これは、`modtools serve` と `modtools web` の両方に当てはまります。
...
`modtools` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Model-of-Design Tools サーバーを使用して、プログラムで Model-of-Design Tools と対話します。
:::

`modtools serve` を実行してスタンドアロンサーバーを起動できます。すでに Model-of-Design Tools TUI を実行している場合でも、`modtools serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Model-of-Design Tools [IDE](/docs/ide) プラグインによって使用されます。
...
Model-of-Design Tools サーバーは次の API を公開します。
