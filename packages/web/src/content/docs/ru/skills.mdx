...
Навыки агента позволяют modtools обнаруживать многократно используемые инструкции из вашего репозитория или домашнего каталога.
...
modtools выполняет поиск в следующих местах:

- Конфигурация проекта: `.modtools/skills/<name>/SKILL.md`
- Глобальная конфигурация: `~/.config/modtools/skills/<name>/SKILL.md`.
...
Для локальных путей проекта modtools переходит из вашего текущего рабочего каталога, пока не достигнет рабочего дерева git.
Он загружает все соответствующие `skills/*/SKILL.md` в `.modtools/` и все соответствующие `.claude/skills/*/SKILL.md` или `.agents/skills/*/SKILL.md` по пути.

Глобальные определения также загружаются из `~/.config/modtools/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` и `~/.agents/skills/*/SKILL.md`.
...
Создайте `.modtools/skills/git-release/SKILL.md` следующим образом:
...
modtools перечисляет доступные навыки в описании инструмента `skill`.
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `modtools.json`:
...
**Для встроенных агентов** (в формате `modtools.json`):
