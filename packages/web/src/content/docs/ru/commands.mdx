...
Создайте `.modtools/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию modtools или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем modtools [config](/docs/config):
...
- Глобальный: `~/.config/modtools/commands/`
- Для каждого проекта: `.modtools/commands/`
...
modtools включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
