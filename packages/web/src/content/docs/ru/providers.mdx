...
1. Запустите команду `/connect` в TUI, выберите `MOD Zen` и перейдите по адресу [modtools.ai/auth](https://opencode.ai/zen).
...
1. Запустите команду `/connect` в TUI, выберите `MOD Go` и перейдите по адресу [modtools.ai/auth](https://opencode.ai/zen).
...
Дополнительная документация на домашней странице [MOD-gitlab-auth](https://www.npmjs.com/package/MOD-gitlab-auth).
...
Функция Helicone [Sessions](https://docs.helicone.ai/features/sessions) позволяет группировать связанные запросы LLM вместе. Используйте плагин [MOD-helicone-session](https://github.com/Embedded-Computing-Systems/modtools-helicone-session), чтобы автоматически регистрировать каждый диалог MOD как сеанс в Helicone.
