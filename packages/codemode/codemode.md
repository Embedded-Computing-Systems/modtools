...
This is the living design and status document for `@modtools-ai/codemode` and its existing V2 MOD adapter.
...
`packages/modtools/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
