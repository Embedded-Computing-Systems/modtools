...
Delete `packages/modtools/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/modtools/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/modtools/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/MOD`
- TUI plugin loader tests in `packages/MOD` if runtime plugin API wiring changed
