...
Trim `packages/modtools/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/modtools/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/MOD`.
- `[x]` `bun typecheck` from `packages/MOD`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/modtools/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/MOD`.
- `[x]` `bun dev generate > /tmp/MOD-openapi.json` from `packages/modtools`.
...
- Edit `packages/modtools/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/MOD-openapi.json` from `packages/modtools`.
...
- Check source schemas in files like `packages/modtools/src/session/schema.ts`, `packages/modtools/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/modtools/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/modtools/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
