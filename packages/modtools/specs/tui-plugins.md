...
```json
{
  ...
  "plugin": ["@acme/MOD-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- The value is a semver range checked against the running Mod version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.MOD` is absent, no check is performed (backward compatible).
...
- `MOD plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `MOD plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.MOD` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.MOD`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.MOD`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.MOD` dir or `MODTOOLS_CONFIG_DIR`, Mod installs `@modtools-ai/plugin` into that dir and writes:
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Mod's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Mod registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Mod pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@modtools-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"MOD"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `MOD.default`.
...
- `--pure` / `MODTOOLS_PURE` skips external TUI plugins only.
