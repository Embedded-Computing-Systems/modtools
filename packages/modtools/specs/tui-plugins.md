...
- Author package entrypoint is `@modtools-ai/plugin/tui`.
...
- npm packages can be TUI theme-only via `package.json["mod-themes"]` without a `./tui` entrypoint.
...
```json
{
  "$schema": "https://modtools.ai/tui.json",
  "theme": "smoke-theme",
  "plugin": ["@acme/modtools-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- Import types from `@modtools-ai/plugin/tui`.
- `@modtools-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@modtools-ai/plugin/tui"
...
- If a configured TUI package has no `./tui` entrypoint and no valid `mod-themes`, it is skipped with a warning (not a load failure).
- If a configured TUI package has no `./tui` entrypoint but has valid `mod-themes`, runtime creates a no-op module record and still loads it for theme sync and plugin state.
...
- `tui` target when `mod-themes` exists and resolves to a non-empty set of valid package-relative theme paths.

`mod-themes` rules:

- `mod-themes` is an array of relative paths.
- Absolute paths and `file://` paths are rejected.
- Resolved theme paths must stay inside the package directory.
- Invalid `mod-themes` causes manifest read failure for install.

Example:

```json
{
  "name": "@acme/modtools-plugin",
  "type": "module",
  "main": "./dist/server.js",
  "exports": {
    "./server": {
...
- The value is a semver range checked against the running Model-of-Design Tools version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.modtools` is absent, no check is performed (backward compatible).
...
- `modtools plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `modtools plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.modtools` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.modtools`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.modtools`) is covered by regression tests.
...
- Tuple targets in `mod-plugin` provide default options written into config.
...
When `plugin` entries exist in a writable `.modtools` dir or `MOD_CONFIG_DIR`, Model-of-Design Tools installs `@modtools-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@modtools-ai/plugin/tui`.
...
- `api.theme.install(...)` and `mod-themes` auto-sync share the same installer path.
...
- Local plugins persist installed themes under the local `.modtools/themes` area near the plugin config source.
...
- `api.plugins.add(spec)` can load theme-only packages (`mod-themes` with no `./tui`) as runtime entries.
...
- `--pure` / `MOD_PURE` skips external TUI plugins only.
- External plugin resolution and import are parallel.
- Packages with no `./tui` entrypoint and valid `mod-themes` are loaded as synthetic no-op TUI plugin modules.
- Theme-only packages loaded this way appear in `api.plugins.list()` and plugin manager rows like other external plugins.
- Packages with no `./tui` entrypoint and no valid `mod-themes` are skipped with warning.
- External plugin activation is sequential to keep command, route, and side-effect order deterministic.
- Theme auto-sync from `mod-themes` runs before plugin `tui(...)` execution and only on metadata state `first` or `updated`.
...
- `tui` target detection includes `exports["./tui"]` and valid `mod-themes`.
...
- Local smoke plugin: `.modtools/plugins/tui-smoke.tsx`
- Local vim plugin: `.modtools/plugins/tui-vim.tsx`
- Local smoke config: `.modtools/tui.json`
- Local smoke theme: `.modtools/plugins/smoke-theme.json`
