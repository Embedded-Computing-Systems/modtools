...
Practical reference for a future `packages/server` split after the MOD
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/MOD`.
...
Until enough shared service code lives outside `packages/MOD`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/MOD`

It should not import `packages/MOD` services while `packages/MOD`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on MOD-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/MOD` implementation details.
