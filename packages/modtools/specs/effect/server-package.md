...
Practical reference for extracting a future `packages/server` from the current `packages/modtools` monolith while `packages/core` is still being migrated to Effect.
...
Everything still lives in `packages/modtools`.
...
- the current server host still lives in `packages/modtools`
...
- if `packages/server` imports services from `packages/modtools` while `packages/modtools` imports `packages/server` to host routes, we create a package cycle immediately
...
4. keep `packages/modtools` as the temporary Hono host
...
- `packages/server` must not import from `packages/modtools`

Allowed in phase 1:

- `packages/modtools` imports `packages/server`
...
- `packages/modtools` shrinks or disappears as package responsibilities are fully split
...
- `packages/modtools` keeps generating legacy OpenAPI from Hono routes
...
- expose host adapters needed by `packages/modtools`
...
Keep in `packages/modtools` for now:
...
- host mounting from `packages/modtools`
...
- no imports from `packages/modtools` inside `packages/server`
...
- `packages/server` must still not import from `packages/modtools`
...
### PR 4. Mount `packages/server` question from `packages/modtools`

Scope:

- replace local experimental question route wiring in `packages/modtools`
...
- mount it in parallel from `packages/modtools`
...
- move adapter selection and server startup out of `packages/modtools`

Rules:

- do not start this while `packages/server` still depends on `packages/modtools`
...
- host logic no longer lives in `packages/modtools`
...
3. the route is mounted from the current host in `packages/modtools`
...
- `packages/server` imports services or runtime from `packages/modtools`
- `packages/modtools` imports route definitions or handlers from `packages/server`

Avoid by:

- keeping phase-1 `packages/server` free of `packages/modtools` imports
...
- [ ] mount `question` from `packages/modtools`
...
2. keep `packages/modtools` as the temporary host
...
If a proposed PR would make `packages/server` import from `packages/modtools`, stop and restructure the boundary first.
