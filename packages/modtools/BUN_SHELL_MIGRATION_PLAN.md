...
Replace runtime Bun shell template-tag usage in `packages/modtools/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/modtools/src` is removed except:
