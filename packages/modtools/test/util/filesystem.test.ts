...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "MOD.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "MOD.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "MOD.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "MOD.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["MOD.json", "MOD.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "MOD.json"),
        path.join(project, "MOD.jsonc"),
      ])
    })
  })
