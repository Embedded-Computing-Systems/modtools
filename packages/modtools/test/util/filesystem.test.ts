...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "modtools.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "modtools.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "modtools.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "modtools.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["modtools.json", "modtools.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "modtools.json"),
        path.join(tmp.path, "modtools.jsonc"),
        path.join(project, "modtools.json"),
        path.join(project, "modtools.jsonc"),
      ])
    })
  })
