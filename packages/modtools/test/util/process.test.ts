...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.MOD_TEST ?? "")'), {
      env: {
        MOD_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "MOD_TEST_SHELL"], {
      shell: true,
      env: {
        MOD_TEST_SHELL: "ok",
      },
    ...
    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("MOD_TEST_SHELL=ok")
  })
