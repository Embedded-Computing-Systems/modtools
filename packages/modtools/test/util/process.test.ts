...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.MODTOOLS_TEST ?? "")'), {
      env: {
        MODTOOLS_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "MODTOOLS_TEST_SHELL"], {
      shell: true,
      env: {
        MODTOOLS_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("MODTOOLS_TEST_SHELL=ok")
  })
