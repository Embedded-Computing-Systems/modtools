...
import { SessionV1 } from "@modtools-ai/core/v1/session"
import { FSUtil } from "@modtools-ai/core/fs-util"
import { ModelsDev } from "@modtools-ai/core/models-dev"
import { HttpRecorder } from "@modtools-ai/http-recorder"
import { HttpRecorderInternal } from "@modtools-ai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@modtools-ai/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@modtools-ai/llm/route"
import { Env } from "@/env"
...
const zenURL = (connection: string) => `https://console.modtools.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.MODTOOLS_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["MOD", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("MODTOOLS_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("MODTOOLS_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["MOD", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "MOD-proxy",
    name: "Mod proxy",
    providerID: ProviderV2.ID.MOD,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["MOD", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.MODTOOLS_RECORD_CONSOLE_TOKEN && process.env.MODTOOLS_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.MOD,
        name: "Mod Zen",
        env: ["MODTOOLS_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.MODTOOLS_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.MODTOOLS_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.MODTOOLS_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["MOD", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("MODTOOLS_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("MODTOOLS_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("MODTOOLS_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set MODTOOLS_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the MOD LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      ...
      JSON.stringify({ $schema: "https://modtools.ai/config.json", ...scenario.config(model) }),
    ),
  )
