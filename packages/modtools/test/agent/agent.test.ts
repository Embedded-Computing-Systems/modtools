...
import { LocationServiceMap } from "@modtools-ai/core/location-layer"
...
it.instance("plan agent denies edits except .modtools/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".modtools/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".MOD", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.MODTOOLS_TEST_HOME
      process.env.MODTOOLS_TEST_HOME = test.directory
