...
test("discovers skills from .modtools/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".modtools", "skill", "test-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: test-skill
...
test("returns skill directories from Skill.dirs", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".modtools", "skill", "dir-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: dir-skill
...
`,
  ...
  const home = process.env.MOD_TEST_HOME
  process.env.MOD_TEST_HOME = tmp.path
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const skillDir = path.join(tmp.path, ".modtools", "skill", "dir-skill")
        expect(dirs).toContain(skillDir)
        expect(dirs.length).toBe(1)
      },
    })
  } finally {
    process.env.MOD_TEST_HOME = home
  }
})

test("discovers multiple skills from .modtools/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir1 = path.join(dir, ".modtools", "skill", "skill-one")
      const skillDir2 = path.join(dir, ".modtools", "skill", "skill-two")
      await Bun.write(
...
test("skips skills with missing frontmatter", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".modtools", "skill", "no-frontmatter")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `# No Frontmatter
...
test("discovers global skills from ~/.claude/skills/ directory", async () => {
  ...
  const originalHome = process.env.MOD_TEST_HOME
  process.env.MOD_TEST_HOME = tmp.path
  ...
  try {
    ...
    process.env.MOD_TEST_HOME = originalHome
  }
})
...
test("properly resolves directories that skills live in", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const modtoolsSkillDir = path.join(dir, ".modtools", "skill", "agent-skill")
      const modtoolsSkillsDir = path.join(dir, ".modtools", "skills", "agent-skill")
      const claudeDir = path.join(dir, ".claude", "skills", "claude-skill")
      const agentDir = path.join(dir, ".agents", "skills", "agent-skill")
      await Bun.write(
        path.join(claudeDir, "SKILL.md"),
...
`,
      ...
      await Bun.write(
        path.join(modtoolsSkillDir, "SKILL.md"),
        `---
name: modtools-skill
description: A skill in the .modtools/skill directory.
---

# Model-of-Design Tools Skill
`,
      )
      await Bun.write(
        path.join(modtoolsSkillsDir, "SKILL.md"),
        `---
name: modtools-skill
