...
import type { Event, ModClient } from "@modtools-ai/sdk/v2"
...
type PermissionReplyParams = Parameters<ModClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "mod-acp-permission-"))
