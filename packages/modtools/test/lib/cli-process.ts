// Subprocess test harness for the mod CLI. Spawns the real binary against
...
//
// Configuration flows through mod's built-in test affordances:
//   - MODTOOLS_CONFIG_CONTENT      : provider config inline, no files to find
//   - MODTOOLS_TEST_HOME           : pins os.homedir() → tmpdir
//   - MODTOOLS_DISABLE_PROJECT_CONFIG : skip walking up for MOD.json
//   - MODTOOLS_PURE                : skip external plugin discovery + install
//   - MODTOOLS_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `mod.run` is fully wired. The shape supports adding more
// builders (`mod.serve(opts)`, `mod.acp(opts)`, `mod.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of ModCli.
...
import { AppProcess } from "@modtools-ai/core/process"
...
const modRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(modRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    MODTOOLS_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    MODTOOLS_CONFIG_CONTENT: configJson,
    MODTOOLS_DISABLE_PROJECT_CONFIG: "1",
    MODTOOLS_PURE: "1",
    MODTOOLS_DISABLE_AUTOUPDATE: "1",
    MODTOOLS_DISABLE_AUTOCOMPACT: "1",
    MODTOOLS_DISABLE_MODELS_FETCH: "1",
    MODTOOLS_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `mod run`. New flags should
...
// `mod serve` is a long-lived process — it never exits on its own.
...
// `mod acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type ModCli = {
  ...
  // Spawn `mod serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `mod acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("mod.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          MODTOOLS_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("mod.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("mod.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "mod server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `mod serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("mod.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const mod: ModCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, mod })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `mod.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "mod") {
...
// every call site. Use this for any test that needs the mod CLI fixture.
//
...
// resources (e.g. `mod.serve`) without an extra `Effect.scoped` wrapper —
