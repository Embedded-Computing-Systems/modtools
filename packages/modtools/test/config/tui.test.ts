...
afterEach(async () => {
  delete process.env.MODTOOLS_CONFIG
  delete process.env.MODTOOLS_TUI_CONFIG
...
test("project config takes precedence over MODTOOLS_TUI_CONFIG (matches MODTOOLS_CONFIG)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.MODTOOLS_TUI_CONFIG = custom
    },
  })
...
test("MODTOOLS_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from MODTOOLS_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.MODTOOLS_CONFIG = path.join(customDir, "modtools.json")
    },
  })
  const config = await getTuiConfig(tmp.path)
  expect(config.theme).toBeUndefined()
