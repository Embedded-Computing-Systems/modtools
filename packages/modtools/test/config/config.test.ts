...
const managedConfigDir = process.env.MOD_TEST_MANAGED_CONFIG_DIR!
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["MOD_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { modtools: { options: { apiKey: "{env:MOD_CONSOLE_TOKEN}" } } },
        }),
      ),
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["MOD_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["MOD_CONSOLE_TOKEN"]
    }
  }
})
...
test("does not try to install dependencies in read-only MOD_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.MOD_CONFIG_DIR
  process.env.MOD_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.MOD_CONFIG_DIR
    else process.env.MOD_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable MOD_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
  ...
  const install = spyOn(Npm, "install").mockImplementation(async (dir: string) => {
    const mod = path.join(dir, "node_modules", "@modtools-ai", "plugin")
    await fs.mkdir(mod, { recursive: true })
    await Filesystem.write(
      path.join(mod, "package.json"),
      JSON.stringify({ name: "@modtools-ai/plugin", version: "1.0.0" }),
    )
  })
...
test("dedupes concurrent config dependency installs for the same dir", async () => {
  ...
  const run = spyOn(Npm, "install").mockImplementation(async (d: string) => {
    ...
    const mod = path.join(d, "node_modules", "@modtools-ai", "plugin")
    await fs.mkdir(mod, { recursive: true })
    await Filesystem.write(
      path.join(mod, "package.json"),
      JSON.stringify({ name: "@modtools-ai/plugin", version: "1.0.0" }),
...
test("serializes config dependency installs across dirs", async () => {
  ...
  const run = spyOn(Npm, "install").mockImplementation(async (dir: string) => {
    ...
    const mod = path.join(cwd, "node_modules", "@modtools-ai", "plugin")
    await fs.mkdir(mod, { recursive: true })
    await Filesystem.write(
      path.join(mod, "package.json"),
      JSON.stringify({ name: "@modtools-ai/plugin", version: "1.0.0" }),
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".modtools")
...
// Note: preload.ts sets MOD_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .modtools config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    ...
    expect(await Config.resolvePluginSpec("oh-my-modtools@2.4.3", file)).toBe("oh-my-modtools@2.4.3")
...
describe("MOD_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["MOD_DISABLE_PROJECT_CONFIG"]
    process.env["MOD_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["MOD_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["MOD_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .modtools/ directories when flag is set", async () => {
    const originalEnv = process.env["MOD_DISABLE_PROJECT_CONFIG"]
    process.env["MOD_DISABLE_PROJECT_CONFIG"] = "true"
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["MOD_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["MOD_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["MOD_CONFIG_DIR"]
      process.env["MOD_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["MOD_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["MOD_CONFIG_DIR"]
      } else {
        process.env["MOD_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("MOD_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["MOD_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["MOD_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["MOD_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["MOD_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from MOD_CONFIG_DIR, not project
...
describe("MOD_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in MOD_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["MOD_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["MOD_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://modtools.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["MOD_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["MOD_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in MOD_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["MOD_CONFIG_CONTENT"]
...
test("parseManagedPlist strips MDM metadata keys", async () => {
  const config = await Config.parseManagedPlist(
    JSON.stringify({
      PayloadDisplayName: "Model-of-Design Tools Managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = await Config.parseManagedPlist(
    JSON.stringify({ $schema: "https://modtools.ai/config.json" }),
    "test:mobileconfig",
  )
