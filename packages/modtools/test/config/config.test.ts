...
import { EffectFlock } from "@modtools-ai/shared/util/effect-flock"

import { Instance } from "../../src/project/instance"
import { Auth } from "../../src/auth"
import { Account } from "../../src/account/account"
import { AccessToken, AccountID, OrgID } from "../../src/account/schema"
import { AppFileSystem } from "@modtools-ai/shared/filesystem"
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["MODTOOLS_CONSOLE_TOKEN"]
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["MODTOOLS_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["MODTOOLS_CONSOLE_TOKEN"]
    }
  }
})
...
test("does not try to install dependencies in read-only MODTOOLS_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.MODTOOLS_CONFIG_DIR
  process.env.MODTOOLS_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.MODTOOLS_CONFIG_DIR
    else process.env.MODTOOLS_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable MODTOOLS_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
describe("MODTOOLS_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["MODTOOLS_DISABLE_PROJECT_CONFIG"]
    process.env["MODTOOLS_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["MODTOOLS_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["MODTOOLS_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["MODTOOLS_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["MODTOOLS_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["MODTOOLS_CONFIG_DIR"]
      process.env["MODTOOLS_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["MODTOOLS_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["MODTOOLS_CONFIG_DIR"]
      } else {
        process.env["MODTOOLS_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("MODTOOLS_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["MODTOOLS_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["MODTOOLS_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["MODTOOLS_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["MODTOOLS_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from MODTOOLS_CONFIG_DIR, not project
...
describe("MODTOOLS_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in MODTOOLS_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["MODTOOLS_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["MODTOOLS_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://modtools.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["MODTOOLS_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["MODTOOLS_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in MODTOOLS_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["MODTOOLS_CONFIG_CONTENT"]
