...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.MODTOOLS_WEBSEARCH_PROVIDER

    try {
      process.env.MODTOOLS_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.MODTOOLS_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.MODTOOLS_WEBSEARCH_PROVIDER
      else process.env.MODTOOLS_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is enabled for MOD providers or explicit websearch provider flags", () => {
    expect(webSearchEnabled(ProviderV2.ID.mod, { exa: false, parallel: false })).toBe(true)
    expect(webSearchEnabled(ProviderV2.ID.make("mod-go"), { exa: false, parallel: false })).toBe(true)
