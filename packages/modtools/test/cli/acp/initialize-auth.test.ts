...
describe("mod acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ mod }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ mod }))
        ...
        expect(initialized.agentInfo?.name).toBe("MOD")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ mod }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ mod })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("mod-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "mod-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.MODTOOLS_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ mod }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("mod-login")
