...
// terminal sizes. The default mod tui command is excluded —
// `mod --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `mod --help` prints today;
...
// top-level help on `--help` and exits 1; not a real mod command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("mod CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ mod }) =>
      Effect.gen(function* () {
        const topLevel = yield* mod.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* mod.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`mod ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`mod ${argv.join(" ")} --help`)
        }
