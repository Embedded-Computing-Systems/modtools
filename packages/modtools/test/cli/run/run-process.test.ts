// Subprocess integration tests for `mod run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `mod.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `MODTOOLS_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("mod run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, mod }) =>
      Effect.gen(function* () {
        ...
        const result = yield* mod.run("say hi")
  ...
  cliIt.concurrent(
    ...
    ({ llm, mod }) =>
      Effect.gen(function* () {
        ...
        const result = yield* mod.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, mod }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* mod.run("think", { extraArgs: ["--thinking"] })
        mod.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* mod.run("think again")
        mod.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, mod }) =>
      Effect.gen(function* () {
        ...
        const result = yield* mod.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, mod }) =>
      Effect.gen(function* () {
        ...
        const result = yield* mod.run("say hi", { format: "json" })
        mod.expectExit(result, 0)

        const events = mod.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ mod }) =>
      Effect.gen(function* () {
        const result = yield* mod.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, mod }) =>
      Effect.gen(function* () {
        ...
        const result = yield* mod.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, mod }) =>
      Effect.gen(function* () {
        ...
        const result = yield* mod.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, mod }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* mod.run("request permission", { permission: { bash: "ask" } })
        mod.expectExit(denied, 0)
        ...
        const allowed = yield* mod.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        mod.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* mod.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        mod.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, mod }) =>
      Effect.gen(function* () {
        ...
        const server = yield* mod.serve()

        const result = yield* mod.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, mod }) =>
      Effect.gen(function* () {
        const result = yield* mod.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, mod }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* mod.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
