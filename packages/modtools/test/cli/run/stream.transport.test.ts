...
import { ModClient, type GlobalEvent } from "@modtools-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<ModClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<ModClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<ModClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<ModClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<ModClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://mod.test"),
...
function sdk(
  input: {
    ...
    subscribe?: ModClient["event"]["subscribe"]
    globalEvent?: ModClient["global"]["event"]
    promptAsync?: ModClient["session"]["promptAsync"]
    status?: ModClient["session"]["status"]
    messages?: ModClient["session"]["messages"]
    children?: ModClient["session"]["children"]
    permissions?: ModClient["permission"]["list"]
    questions?: ModClient["question"]["list"]
  } = {},
) {
  const client = new ModClient()

  const subscribe: ModClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: ModClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: ModClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: ModClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: ModClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: ModClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: ModClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: ModClient["question"]["list"] = input.questions ?? (() => ok([]))
