...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ mod }) =>
    Effect.gen(function* () {
      const result = yield* mod.spawn(["--replay-limit", "10"])

      mod.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ mod }) =>
    Effect.gen(function* () {
      const result = yield* mod.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ mod }) =>
    Effect.gen(function* () {
      const result = yield* mod.spawn(["--mini", "--port", "4096"])
