...
describe("MOD read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // MODTOOLS_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ MOD }) =>
      Effect.gen(function* () {
        const r = yield* MOD.spawn(["mcp", "list"])
        MOD.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ MOD }) =>
      Effect.gen(function* () {
        const r = yield* MOD.spawn(["providers", "list"])
        MOD.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ MOD }) =>
      Effect.gen(function* () {
        const r = yield* MOD.spawn(["models"])
        MOD.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ MOD }) =>
      Effect.gen(function* () {
        const r = yield* MOD.spawn(["agent", "list"])
        MOD.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh MODTOOLS_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ MOD }) =>
      Effect.gen(function* () {
        const r = yield* MOD.spawn(["session", "list"])
        MOD.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ MOD }) =>
      Effect.gen(function* () {
        const r = yield* MOD.spawn(["stats"])
        MOD.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under MODTOOLS_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ MOD }) =>
      Effect.gen(function* () {
        const r = yield* MOD.spawn(["db", "path"])
        MOD.expectExit(r, 0, "db path")
