...
import { EventSequenceTable } from "@modtools-ai/core/event/sql"
...
import { WorkspaceV2 } from "@modtools-ai/core/workspace"
import { WorkspaceTable } from "@modtools-ai/core/control-plane/workspace.sql"
...
import { Ripgrep } from "@modtools-ai/core/ripgrep"
...
const originalEnv = {
  MODTOOLS_AUTH_CONTENT: process.env.MODTOOLS_AUTH_CONTENT,
  MODTOOLS_EXPERIMENTAL_WORKSPACES: process.env.MODTOOLS_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.MODTOOLS_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@mod.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.MODTOOLS_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=mod-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.MODTOOLS_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.MODTOOLS_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.MODTOOLS_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=mod-test")
