...
type Sdk = ReturnType<typeof createModClient>
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.MODTOOLS_SERVER_PASSWORD = input?.password
      Flag.MODTOOLS_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".MOD", "skills", "project-rest-skill", "SKILL.md"),
      `---
  ...
  httpapi(
    ...
    withProject("raw", { setup: writeStandardFiles }, ({ directory }) =>
      Effect.gen(function* () {
        ...
        expect(request!.headers.has("x-MOD-directory")).toBe(false)
        expect(request!.headers.has("x-MOD-workspace")).toBe(false)
      }),
    ),
  )
