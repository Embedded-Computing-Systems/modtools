  ...
  test("continues loading plugins when plugin init throws", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "modtools.json"), JSON.stringify({ plugin: [file, ok] }, null, 2))
  ...
  test("continues loading plugins when plugin import fails", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "modtools.json"), JSON.stringify({ plugin: [missing, ok] }, null, 2))
...
`,
  ...
  test("skips external plugins in pure mode", async () => {
    ...
    const pure = process.env.MODTOOLS_PURE
    process.env.MODTOOLS_PURE = "1"
    ...
    try {
      ...
      if (pure === undefined) {
        delete process.env.MODTOOLS_PURE
      } else {
        process.env.MODTOOLS_PURE = pure
      }
    }
  })
