    ...
    JSON.stringify(
      {
        ...
        ...(themes?.length ? { "mod-themes": themes } : {}),
      },
      null,
      2,
    ),
...
describe("plugin.install.task", () => {
  test("writes both server and tui config entries", async () => {
    ...
    const server = await read(path.join(tmp.path, ".modtools", "modtools.jsonc"))
    const tui = await read(path.join(tmp.path, ".modtools", "tui.jsonc"))
  ...
  test("preserves JSONC comments when adding plugins to server and tui config", async () => {
    await using tmp = await tmpdir()
    const target = await plugin(tmp.path, ["server", "tui"])
    const cfg = path.join(tmp.path, ".modtools")
    const server = path.join(cfg, "modtools.jsonc")
...
`,
  ...
  test("preserves JSONC comments when force replacing plugin version", async () => {
    ...
    const cfg = path.join(tmp.path, ".modtools", "modtools.jsonc")
...
`,
  ...
  test("writes to global scope when global flag is set", async () => {
    ...
    expect(await Filesystem.exists(path.join(global, "modtools.jsonc"))).toBe(true)
    expect(await Filesystem.exists(path.join(tmp.path, ".modtools", "modtools.jsonc"))).toBe(false)
  })
  ...
  test("writes local scope under directory when vcs is not git", async () => {
    ...
    expect(ok).toBe(true)
    expect(await Filesystem.exists(path.join(directory, ".modtools", "modtools.jsonc"))).toBe(true)
    expect(await Filesystem.exists(path.join(worktree, ".modtools", "modtools.jsonc"))).toBe(false)
  })
  ...
  test("writes tui local scope under directory when worktree is root slash", async () => {
    ...
    const ok = await run(ctxRoot(directory))
    expect(ok).toBe(true)
    expect(await Filesystem.exists(path.join(directory, ".modtools", "tui.jsonc"))).toBe(true)
  })
  ...
  test("writes only tui config for tui-only plugins", async () => {
    ...
    const ok = await run(ctx(tmp.path))
    expect(ok).toBe(true)
    expect(await Filesystem.exists(path.join(tmp.path, ".modtools", "tui.jsonc"))).toBe(true)
    expect(await Filesystem.exists(path.join(tmp.path, ".modtools", "modtools.jsonc"))).toBe(false)
  })

  test("writes tui config for mod-themes-only packages", async () => {
    await using tmp = await tmpdir()
  ...
  test("returns false for mod-themes outside plugin directory", async () => {
    await using tmp = await tmpdir()
    ...
    expect(await Filesystem.exists(path.join(tmp.path, ".modtools", "tui.jsonc"))).toBe(false)
    expect(await Filesystem.exists(path.join(tmp.path, ".modtools", "modtools.jsonc"))).toBe(false)
  })
  ...
  test("force replaces version in both server and tui configs", async () => {
    await using tmp = await tmpdir()
    const target = await plugin(tmp.path, ["server", "tui"])
    const server = path.join(tmp.path, ".modtools", "modtools.json")
    const tui = path.join(tmp.path, ".modtools", "tui.json")
