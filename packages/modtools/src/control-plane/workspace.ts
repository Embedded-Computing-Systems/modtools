...
import { Slug } from "@modtools-ai/shared/util/slug"
...
export const create = fn(CreateInput, async (input) => {
  ...
  const env = {
    MODTOOLS_AUTH_CONTENT: JSON.stringify(await AppRuntime.runPromise(Auth.Service.use((auth) => auth.all()))),
...
async function startSync(space: Info) {
  if (!Flag.MODTOOLS_EXPERIMENTAL_WORKSPACES) return
