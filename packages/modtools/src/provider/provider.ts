...
export namespace Provider {
  ...
  function e2eURL() {
    const url = Env.get("MOD_E2E_LLM_URL")
    ...
    return {
      ...
      gitlab: Effect.fnUntraced(function* (input: Info) {
        ...
        const aiGatewayHeaders = {
          "User-Agent": `modtools/${Installation.VERSION} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
      ...
      "cloudflare-workers-ai": Effect.fnUntraced(function* (input: Info) {
        ...
        return {
          ...
          options: {
            apiKey,
            headers: {
              "User-Agent": `modtools/${Installation.VERSION} cloudflare-workers-ai (${os.platform()} ${os.release()}; ${os.arch()})`,
            },
          },
          async getModel(sdk: any, modelID: string) {
            return sdk.languageModel(modelID)
      ...
      "cloudflare-ai-gateway": Effect.fnUntraced(function* (input: Info) {
        ...
        const opts = {
          ...
          headers: {
            "User-Agent": `modtools/${Installation.VERSION} cloudflare-ai-gateway (${os.platform()} ${os.release()}; ${os.arch()})`,
          },
        }
      ...
      cerebras: () =>
        Effect.succeed({
          ...
          options: {
            headers: {
              "X-Cerebras-3rd-Party-Integration": "modtools",
            },
          },
        }),
      kilo: () =>
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@modtools/Provider") {}
  ...
  const layer: Layer.Layer<Service, never, Config.Service | Auth.Service | Plugin.Service> = Layer.effect(
    ...
    Effect.gen(function* () {
        ...
        Effect.gen(function* () {
          ...
          for (const [id, provider] of Object.entries(providers)) {
            ...
            for (const [modelID, model] of Object.entries(provider.models)) {
              ...
              if (model.status === "alpha" && !Flag.MOD_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
      ...
      const getSmallModel = Effect.fn("Provider.getSmallModel")(function* (providerID: ProviderID) {
        ...
        if (providerID.startsWith("modtools")) {
          priority = ["gpt-5-nano"]
        }
