...
const FILES = [
  "AGENTS.md",
  ...(Flag.MOD_DISABLE_CLAUDE_CODE_PROMPT ? [] : ["CLAUDE.md"]),
...
function globalFiles() {
  ...
  if (Flag.MOD_CONFIG_DIR) {
    files.push(path.join(Flag.MOD_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.MOD_DISABLE_CLAUDE_CODE_PROMPT) {
...
export namespace Instruction {
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@modtools/Instruction") {}
    ...
    Layer.effect(
      ...
      Effect.gen(function* () {
        ...
        const relative = Effect.fnUntraced(function* (instruction: string) {
          ...
          if (!Flag.MOD_CONFIG_DIR) {
            log.warn(
              `Skipping relative instruction "${instruction}" - no MOD_CONFIG_DIR set while project config is disabled`,
            )
            return []
          }
          return yield* fs
            .globUp(instruction, Flag.MOD_CONFIG_DIR, Flag.MOD_CONFIG_DIR)
            .pipe(Effect.catch(() => Effect.succeed([] as string[])))
        })
