...
import type { LLMClientShape } from "@modtools-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("MOD"))
    return { type: "unsupported", reason: "provider is not openai, MOD, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @modtools-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains MOD-owned. The native runtime only adapts
      // the @modtools-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
