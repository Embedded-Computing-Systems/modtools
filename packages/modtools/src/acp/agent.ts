...
import type { AssistantMessage, Event, ModtoolsClient, SessionMessageResponse, ToolPart } from "@modtools-ai/sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: ModtoolsClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `modtools auth login` in the terminal",
        name: "Login with modtools",
        id: "modtools-login",
      }
      ...
      if (params.clientCapabilities?._meta?.["terminal-auth"] === true) {
        authMethod._meta = {
          "terminal-auth": {
            command: "modtools",
            args: ["auth", "login"],
            label: "Model-of-Design Tools Login",
          },
        }
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // Model-of-Design Tools stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
    ...
    const modtoolsProvider = providers.find((p) => p.id === "modtools")
    if (modtoolsProvider) {
      if (modtoolsProvider.models["big-pickle"]) {
        return { providerID: ProviderID.modtools, modelID: ModelID.make("big-pickle") }
      }
      const [best] = Provider.sort(Object.values(modtoolsProvider.models))
      if (best) {
        return {
