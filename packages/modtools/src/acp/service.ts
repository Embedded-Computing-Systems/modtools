...
import type { AssistantMessage, Message, ModClient, SessionMessageResponse } from "@modtools-ai/sdk/v2"
...
export const AuthMethodID = "mod-login"
...
export class Service extends Context.Service<Service, Interface>()("@mod/ACP/Service") {}
...
function makeDirectoryService(sdk: ModClient) {
...
function makeUsageService(sdk: ModClient) {
...
async function loadDirectorySnapshot(sdk: ModClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, mod provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const modProvider = providers[ProviderV2.ID.make("mod")]
  const modModel = modProvider ? Provider.sort(Object.values(modProvider.models))[0] : undefined
  if (modProvider && modModel) return { providerID: modProvider.id, modelID: modModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "MOD service failure", service })
}
