...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for modtools.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal modtools sessions
...
modtools acp --cwd /path/to/project
```
...
```bash
MOD_ENABLE_QUESTION_TOOL=1 modtools acp
```
...
- **Terminal Integration**: Full terminal support via modtools's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | modtools acp
```
...
### Mapping to Model-of-Design Tools

ACP sessions map cleanly to modtools's internal session model:
