...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for MOD.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal MOD sessions
...
```bash
OPENCODE_ENABLE_QUESTION_TOOL=1 MOD acp
```
...
- **Terminal Integration**: Full terminal support via MOD's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | MOD acp
```
...
ACP sessions map cleanly to MOD's internal session model:
