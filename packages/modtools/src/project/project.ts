...
export namespace Project {
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@modtools/Project") {}
  ...
  > = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const fakeVcs = Info.shape.vcs.parse(Flag.MOD_FAKE_VCS)
      ...
      const readCachedProjectId = Effect.fnUntraced(function* (dir: string) {
        return yield* fs.readFileString(pathSvc.join(dir, "modtools")).pipe(
          Effect.map((x) => x.trim()),
      ...
      const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
        ...
        const data: DiscoveryResult = yield* Effect.gen(function* () {
          ...
          if (!id) {
            ...
            if (id) {
              yield* fs.writeFileString(pathSvc.join(worktree, ".git", "modtools"), id).pipe(Effect.ignore)
            }
          }
        ...
        if (Flag.MOD_EXPERIMENTAL_ICON_DISCOVERY)
