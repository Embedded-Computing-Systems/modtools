...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@modtools-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@modtools-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@modtools-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@modtools-ai/tui/context/theme"
import { Global } from "@modtools-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@modtools-ai/core/util/flock"
import { Flag } from "@modtools-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@modtools-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@modtools-ai/core/v1/config/plugin"
import { createCommandShim } from "@modtools-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@modtools-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".mod"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".mod", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".mod", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.MODTOOLS_PURE ? [] : pluginOrigins
    if (Flag.MODTOOLS_PURE && pluginOrigins.length) {
    }
