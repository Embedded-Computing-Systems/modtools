...
export const DEPRECATED_PLUGIN_PACKAGES = ["MOD-openai-codex-auth", "MOD-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, MODVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(MODVersion) || semver.major(MODVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.MOD
  if (typeof range !== "string") return
  if (!semver.satisfies(MODVersion, range)) {
    throw new Error(`Plugin requires MOD ${range} but running ${MODVersion}`)
  }
}
