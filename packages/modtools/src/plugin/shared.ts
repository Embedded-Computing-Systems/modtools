...
export const DEPRECATED_PLUGIN_PACKAGES = ["modtools-openai-codex-auth", "modtools-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, modtoolsVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(modtoolsVersion) || semver.major(modtoolsVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.modtools
  if (typeof range !== "string") return
  if (!semver.satisfies(modtoolsVersion, range)) {
    throw new Error(`Plugin requires modtools ${range} but running ${modtoolsVersion}`)
  }
}
...
export function readPackageThemes(spec: string, pkg: PluginPackage) {
  const field = pkg.json["mod-themes"]
  if (field === undefined) return []
  if (!Array.isArray(field)) {
    throw new TypeError(`Plugin ${spec} has invalid mod-themes field`)
  }

  const list = field.map((item) => {
    if (typeof item !== "string") {
      throw new TypeError(`Plugin ${spec} has invalid mod-themes entry`)
    }

    const raw = item.trim()
    if (!raw) {
      throw new TypeError(`Plugin ${spec} has empty mod-themes entry`)
    }
    if (raw.startsWith("file://") || isAbsolutePath(raw)) {
      throw new TypeError(`Plugin ${spec} mod-themes entry must be relative: ${item}`)
    }

    return resolvePackageFile(spec, raw, "mod-themes", pkg)
  })
