import type { Hooks, PluginInput, Plugin as PluginInstance, PluginModule } from "@modtools-ai/plugin"
      ...
      const state = yield* InstanceState.make<State>(
        Effect.fn("Plugin.state")(function* (ctx) {
          ...
          const client = createModtoolsClient({
            ...
            headers: Flag.MOD_SERVER_PASSWORD
              ? {
                  Authorization: `Basic ${Buffer.from(`${Flag.MOD_SERVER_USERNAME ?? "modtools"}:${Flag.MOD_SERVER_PASSWORD}`).toString("base64")}`,
                }
              : undefined,
          ...
          const plugins = Flag.MOD_PURE ? [] : (cfg.plugin_origins ?? [])
          if (Flag.MOD_PURE && cfg.plugin_origins?.length) {
