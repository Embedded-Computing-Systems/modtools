...
import { NamedError } from "@modtools-ai/shared/util/error"
...
const InfoSchema = Schema.Struct({
  ...
  ).annotate({ description: "Agent configuration, see https://modtools.ai/docs/agents" }),
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        if (Flag.MODTOOLS_CONFIG) {
          yield* merge(Flag.MODTOOLS_CONFIG, yield* loadFile(Flag.MODTOOLS_CONFIG))
          log.debug("loaded custom config", { path: Flag.MODTOOLS_CONFIG })
        }
        ...
        if (Flag.MODTOOLS_CONFIG_DIR) {
          log.debug("loading config from MODTOOLS_CONFIG_DIR", { path: Flag.MODTOOLS_CONFIG_DIR })
        }
        ...
        if (process.env.MODTOOLS_CONFIG_CONTENT) {
          ...
          log.debug("loaded custom config from MODTOOLS_CONFIG_CONTENT")
        }
