...
import { NamedError } from "@modtools-ai/util/error"
...
export namespace Config {
  ...
  function systemManagedConfigDir(): string {
    switch (process.platform) {
      case "darwin":
        return "/Library/Application Support/modtools"
      case "win32":
        return path.join(process.env.ProgramData || "C:\\ProgramData", "modtools")
      default:
        return "/etc/modtools"
    }
  }
  ...
  export function managedConfigDir() {
    return process.env.MOD_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
  }

  const managedDir = managedConfigDir()

  const MANAGED_PLIST_DOMAIN = "ai.modtools.managed"

  // Keys injected by macOS/MDM into the managed plist that are not Model-of-Design Tools config
  ...
  /**
   * Parse raw JSON (from plutil conversion of a managed plist) into Model-of-Design Tools config.
  ...
  async function loadCommand(dir: string) {
    ...
    for (const item of await Glob.scan("{command,commands}/**/*.md", {
      ...
      const patterns = ["/.modtools/command/", "/.modtools/commands/", "/command/", "/commands/"]
  ...
  export const Server = z
    .object({
      ...
      mdnsDomain: z.string().optional().describe("Custom domain name for mDNS service (default: modtools.local)"),
  ...
  export const Info = z
    .object({
      ...
      server: Server.optional().describe("Server configuration for modtools serve and web commands"),
      command: z
        .record(z.string(), Command)
        .optional()
        .describe("Command configuration, see https://modtools.ai/docs/commands"),
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://modtools.ai/docs/agents"),
      provider: z
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@modtools/Config") {}
      ...
      Effect.gen(function* () {
        ...
        const loadConfig = Effect.fnUntraced(function* (
          ...
          if (parsed.success) {
            if (!parsed.data.$schema && isFile) {
              parsed.data.$schema = "https://modtools.ai/config.json"
        ...
        const loadInstanceState = Effect.fnUntraced(function* (ctx: InstanceContext) {
          ...
          const scope = (source: string): PluginScope => {
            ...
            if (source === "MOD_CONFIG_CONTENT") return "local"
          ...
          if (Flag.MOD_CONFIG) {
            merge(Flag.MOD_CONFIG, yield* loadFile(Flag.MOD_CONFIG))
            log.debug("loaded custom config", { path: Flag.MOD_CONFIG })
          }

          if (!Flag.MOD_DISABLE_PROJECT_CONFIG) {
            for (const file of yield* Effect.promise(() =>
              ConfigPaths.projectFiles("modtools", ctx.directory, ctx.worktree),
            )) {
          ...
          for (const dir of unique(directories)) {
            if (dir.endsWith(".modtools") || dir === Flag.MOD_CONFIG_DIR) {
          ...
          if (process.env.MOD_CONFIG_CONTENT) {
            const source = "MOD_CONFIG_CONTENT"
            const next = yield* loadConfig(process.env.MOD_CONFIG_CONTENT, {
              dir: ctx.directory,
              source,
            })
          ...
          if (active?.active_org_id) {
            yield* Effect.gen(function* () {
              ...
              if (token) {
                process.env["MOD_CONSOLE_TOKEN"] = token
                Env.set("MOD_CONSOLE_TOKEN", token)
              }
