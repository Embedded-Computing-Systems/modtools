...
import { isRecord } from "@modtools-ai/tui/util/record"
...
import { TuiKeybind } from "@modtools-ai/tui/config/keybind"
...
import { TuiConfig } from "@modtools-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@mod/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old MOD.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.mod`
  // folders between cwd and home, and MODTOOLS_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.MODTOOLS_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit MODTOOLS_TUI_CONFIG override, if set.
  if (Flag.MODTOOLS_TUI_CONFIG) {
    const configFile = Flag.MODTOOLS_TUI_CONFIG
  ...
  // 4. `.mod` directories (and MODTOOLS_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".mod") || dir === Flag.MODTOOLS_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".mod") && dir !== Flag.MODTOOLS_CONFIG_DIR) continue
