...
const MANAGED_PLIST_DOMAIN = "ai.mod.managed"

// Keys injected by macOS/MDM into the managed plist that are not config
const PLIST_META = new Set(["PayloadUUID", "PayloadIdentifier"])

function systemManagedConfigDir() {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/mod"
    case "win32":
...
export function managedConfigDir() {
  return process.env.MODTOOLS_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
