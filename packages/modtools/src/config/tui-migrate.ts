...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from MOD.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const mod = await modFiles(input)
...
async function modFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "mod"),
    ...(await Filesystem.findUp(["MOD.json", "MOD.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
