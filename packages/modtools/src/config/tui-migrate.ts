...
const TUI_SCHEMA_URL = "https://modtools.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from modtools.json files
...
async function modtoolsFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.MOD_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("modtools", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "modtools")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "modtools"))
  }
  if (Flag.MOD_CONFIG) files.push(Flag.MOD_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "modtools"))
