...
import { Module } from "@modtools-ai/shared/util/module"
...
export const Vue: Info = {
  ...
  async spawn(root) {
    ...
    const args: string[] = []
    if (!binary) {
      if (Flag.MODTOOLS_DISABLE_LSP_DOWNLOAD) return
...
export const JDTLS: Info = {
  ...
  async spawn(root) {
    ...
    const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "mod-jdtls-data"))
    return {
      process: spawn(
        java,
        [
