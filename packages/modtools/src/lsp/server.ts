...
import { Module } from "@modtools-ai/util/module"
...
export namespace LSPServer {
  ...
  export const Vue: Info = {
    ...
    async spawn(root) {
      ...
      const args: string[] = []
      if (!binary) {
        if (Flag.MOD_DISABLE_LSP_DOWNLOAD) return
  ...
  export const JDTLS: Info = {
    ...
    async spawn(root) {
      ...
      const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "modtools-jdtls-data"))
      return {
        process: spawn(
          java,
          [
