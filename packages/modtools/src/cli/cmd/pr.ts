...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run modtools",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for modtools session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found modtools session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["modtools", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting modtools...")
        UI.println()

        const modtoolsArgs = sessionId ? ["-s", sessionId] : []
        const modtoolsProcess = Process.spawn(["modtools", ...modtoolsArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await modtoolsProcess.exited
        if (code !== 0) throw new Error(`modtools exited with code ${code}`)
      },
    })
  },
})
