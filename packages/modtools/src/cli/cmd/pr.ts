...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run MOD",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for MOD session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/(?:modtools\.ai|opncd\.ai)\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found MOD session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["mod", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting MOD...")
        UI.println()

        const modArgs = sessionId ? ["-s", sessionId] : []
        const modProcess = Process.spawn(["mod", ...modArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await modProcess.exited
        if (code !== 0) throw new Error(`MOD exited with code ${code}`)
      },
    })
  },
})
