...
export const RunCommand = cmd({
  ...
  handler: async (args) => {
    ...
    async function session(sdk: ModClient) {
    ...
    async function share(sdk: ModClient, sessionID: string) {
      ...
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.MODTOOLS_AUTO_SHARE && !args.share) return
    ...
    if (args.attach) {
      ...
      const sdk = createModClient({ baseUrl: args.attach, directory, headers })
