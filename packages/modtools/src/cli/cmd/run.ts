import type { PermissionV1 } from "@modtools-ai/core/v1/permission"
import { FSUtil } from "@modtools-ai/core/fs-util"
// CLI entry point for `mod run` and `mod --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`mod --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`mod --mini --attach`): connects to a running
//      mod server and runs interactive mode against it.
//
...
import { createModClient, type ModClient, type ToolPart } from "@modtools-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<ModClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to MODTOOLS_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to MODTOOLS_SERVER_USERNAME or 'mod')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: ModClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: ModClient, sessionID: string) {
      ...
      async function current(sdk: ModClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: ModClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: ModClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: ModClient) {
        ...
        async function loop(client: ModClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
