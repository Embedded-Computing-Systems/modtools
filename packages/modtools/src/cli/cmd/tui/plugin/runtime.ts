...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".modtools"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".modtools", "themes")
...
async function readThemeFiles(spec: string, pkg?: PluginPackage) {
  ...
  return Promise.resolve()
    ...
    .catch((error) => {
      warn("invalid tui plugin mod-themes", {
        path: spec,
        pkg: pkg.pkg,
        error,
      })
...
async function syncPluginThemes(plugin: PluginEntry) {
  ...
  for (const file of plugin.load.theme_files) {
    await install(file).catch((error) => {
      warn("failed to sync tui plugin mod-themes", { path: plugin.load.spec, id: plugin.id, theme: file, error })
    })
  }
}
...
function defaultPluginOrigin(state: RuntimeState, spec: string): Config.PluginOrigin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".modtools", "tui.json"),
  }
}
...
async function installPluginBySpec(
  ...
  if (!manifest.ok) {
    if (manifest.code === "manifest_no_targets") {
      return {
        ok: false,
        message: `"${spec}" does not expose plugin entrypoints or mod-themes in package.json`,
      }
    }
  ...
  async function load(api: Api) {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const records = Flag.MOD_PURE ? [] : (config.plugin_origins ?? [])
        if (Flag.MOD_PURE && config.plugin_origins?.length) {
