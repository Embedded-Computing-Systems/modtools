...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".mod" || path.basename(source_dir) === ".modtools"
        ? path.join(source_dir, "themes")
        : (await Filesystem.exists(path.join(source_dir, ".mod")))
...
async function load(input: { api: Api; config: TuiConfig.Info }) {
  ...
  try {
    await Instance.provide({
      directory: cwd,
      fn: async () => {
        const records = Flag.MODTOOLS_PURE ? [] : (config.plugin_origins ?? [])
        if (Flag.MODTOOLS_PURE && config.plugin_origins?.length) {
