...
import type { TuiDialogSelectOption, TuiPluginApi, TuiRouteDefinition, TuiSlotProps } from "@modtools-ai/plugin/tui"
...
import { createModtoolsClient, type ModtoolsClient } from "@modtools-ai/sdk/v2"
...
type TuiHostPluginApi = TuiPluginApi & {
  map: Map<string | undefined, ModtoolsClient>
  dispose: () => void
}
...
export function createTuiApi(input: Input): TuiHostPluginApi {
  const map = new Map<string | undefined, ModtoolsClient>()
  const scoped: TuiPluginApi["scopedClient"] = (workspaceID) => {
    const hit = map.get(workspaceID)
    if (hit) return hit

    const next = createModtoolsClient({
