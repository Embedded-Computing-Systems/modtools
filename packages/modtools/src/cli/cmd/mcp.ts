...
async function resolveConfigPath(baseDir: string, global = false) {
  ...
  const candidates = [path.join(baseDir, "modtools.json"), path.join(baseDir, "modtools.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".modtools", "modtools.json"), path.join(baseDir, ".modtools", "modtools.jsonc"))
  }
  ...
  // Default to modtools.json if none exist
  return candidates[0]
}
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "modtools-debug", version: Installation.VERSION },
              },
              id: 1,
            }),
          })
