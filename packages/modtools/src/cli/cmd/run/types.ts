// Shared type vocabulary for the direct interactive mode (`mod --mini`).
//
...
import type { ModClient, PermissionRequest, QuestionRequest, ToolPart } from "@modtools-ai/sdk/v2"
import type { TuiConfig } from "@modtools-ai/tui/config"
...
type PromptModel = Parameters<ModClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<ModClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<ModClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<ModClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<ModClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<ModClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<ModClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<ModClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<ModClient["question"]["reject"]>[0]
