...
export type PlugDeps = {
  ...
  files: (dir: string, name: "modtools" | "tui") => string[]
  global: string
}
  ...
  return async (ctx: PlugCtx) => {
    ...
    if (!manifest.ok) {
      ...
      if (manifest.code === "manifest_no_targets") {
        ...
        dep.log.info(
          'Expected one of: exports["./tui"], exports["./server"], package.json main for server, or package.json["mod-themes"] for tui themes.',
        )
        return false
      }
