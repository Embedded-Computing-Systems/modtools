...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall modtools and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Model-of-Design Tools")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g modtools-ai",
      pnpm: "pnpm uninstall -g modtools-ai",
      bun: "bun remove -g modtools-ai",
      yarn: "yarn global remove modtools-ai",
      brew: "brew uninstall modtools",
      choco: "choco uninstall modtools",
      scoop: "scoop uninstall modtools",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "modtools", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Model-of-Design Tools!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# modtools") || content.includes(".modtools/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".modtools/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".modtools/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".modtools"))
    ) {
      continue
    }
