...
const sandbox = await fs.mkdtemp(path.join(os.tmpdir(), "modtools-e2e-"))
const keepSandbox = process.env.MOD_E2E_KEEP_SANDBOX === "1"

const serverEnv = {
  ...process.env,
  MOD_DISABLE_SHARE: process.env.MOD_DISABLE_SHARE ?? "true",
  ...
  MOD_E2E_PROJECT_DIR: repoDir,
  MOD_E2E_SESSION_TITLE: "E2E Session",
  MOD_E2E_MESSAGE: "Seeded for UI e2e",
  MOD_E2E_MODEL: process.env.MOD_E2E_MODEL ?? "modtools/gpt-5-nano",
  MOD_CLIENT: "app",
...
const runnerEnv = {
  ...
  VITE_MOD_SERVER_HOST: "127.0.0.1",
  VITE_MOD_SERVER_PORT: String(serverPort),
...
try {
  ...
  if (seedExit !== 0) {
    ...
    const log = await import("../../modtools/src/util/log")
    const install = await import("../../modtools/src/installation")
    await log.Log.init({
      print: true,
    ...
    const servermod = await import("../../modtools/src/server/server")
    inst = await import("../../modtools/src/project/instance")
    server = servermod.Server.listen({ port: serverPort, hostname: "127.0.0.1" })
    console.log(`modtools server listening on http://127.0.0.1:${serverPort}`)
