...
import { type Session } from "@modtools-ai/sdk/v2/client"
import {
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("modtools://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("modtools://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("modtools://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("modtools://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("modtools://open-project")).toBeUndefined()
    expect(parseDeepLink("modtools://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "modtools://open-project?directory=/a",
      "modtools://other?directory=/b",
      "modtools://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("modtools://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("modtools://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("modtools://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("modtools://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "modtools://new-session?directory=/a",
      "modtools://open-project?directory=/b",
      "modtools://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __MOD__: {
        deepLinks: ["modtools://open-project?directory=/a"],
      },
    } as unknown as Window & { __MOD__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["modtools://open-project?directory=/a"])
