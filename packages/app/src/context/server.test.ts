  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://MOD.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://MOD.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://MOD.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          ...
          lastProject: { local: "/local", "https://MOD.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://MOD.example.com"),
      ),
    ).toEqual({
