...
import { enterWslModStep } from "./settings-model"

type WslServerStep = "wsl" | "distro" | "MOD"

const STEPS: WslServerStep[] = ["wsl", "distro", "MOD"]
...
export function DialogAddWslServer(props: DialogWslServerProps = {}) {
  ...
  const MODCheck = createMemo(() => {
    const distro = selectedDistro()
    if (!distro) return null
    return current()?.MODChecks[distro] ?? null
  })
  ...
  const MODReady = createMemo(() => {
    const check = MODCheck()
  ...
  const installingMod = createMemo(() => {
    const job = current()?.job
    return job?.kind === "install-MOD" && job.distro === selectedDistro()
  })
  const allReady = createMemo(() => wslReady() && distroReady() && MODReady())
  const addDisabled = createMemo(() => {
    const job = current()?.job
    if (!job) return store.adding
    return store.adding || job.kind !== "probe-MOD"
  })
  ...
  const autoProbe = createMemo(() => {
    ...
    if (!state.MODChecks[distro]) {
      return { key: `probe-MOD:${distro}`, run: () => api.probeMod(distro) }
    }
    return null
  })
  ...
  const MODMessage = createMemo(() => {
    const state = current()
    if (!state) return language.t("wsl.onboarding.checkingMod")
    const distro = selectedDistro()
    if (state.job?.kind === "install-MOD") {
      return distro
        ? language.t("wsl.onboarding.updatingModIn", { distro })
        : language.t("wsl.onboarding.updatingMod")
    }
    if (state.job?.kind === "probe-MOD") {
      return distro
        ? language.t("wsl.onboarding.checkingModIn", { distro })
        : language.t("wsl.onboarding.checkingMod")
    }
    if (MODCheck()?.error) return MODCheck()!.error
    if (MODCheck()?.matchesDesktop === false) {
      return distro
        ? language.t("wsl.onboarding.updateModIn", { distro })
        : language.t("wsl.onboarding.updateMod")
    }
    if (MODReady()) {
      return distro
        ? language.t("wsl.onboarding.MODReadyIn", { distro })
        : language.t("wsl.onboarding.MODReady")
    }
    return distro
      ? language.t("wsl.onboarding.installModIn", { distro })
  ...
  const openModStep = () => {
    const distro = selectedDistro()
    if (!distro) return
    void run(() => enterWslModStep(distro, api.probeMod, (step) => setStore("step", step)))
  }
        ...
        title:
          step === "wsl"
            ...
            : step === "distro"
              ...
              : language.t("wsl.onboarding.step.MOD"),
        state:
          active === step
            ? "current"
            : step === "wsl"
      ...
      >
        ...
        >
          ...
          <Switch>
            ...
            <Match when={activeStep() === "MOD"}>
              <div class="rounded-md bg-surface-base p-4 flex flex-col gap-3">
                <div class="flex items-center justify-between gap-3">
                  <div class="text-14-medium text-text-strong">{language.t("wsl.onboarding.step.MOD")}</div>
                  <div class="flex items-center gap-2">
                    <Show when={selectedDistro()}>
                      <Button
                        variant="ghost"
                        size="large"
                        disabled={busy()}
                        onClick={() => runSelectedDistro((distro) => api.probeMod(distro))}
                      >
                        {language.t("wsl.onboarding.refresh")}
                      </Button>
                    </Show>
                    <Show when={!MODReady() || MODCheck()?.matchesDesktop === false}>
                      <Button
                        variant="secondary"
                        size="large"
                        disabled={busy()}
                        onClick={() => runSelectedDistro((distro) => api.installMod(distro))}
                      >
                        <Show when={installingMod()}>
                          <Spinner class="size-4 shrink-0" />
                        </Show>
                        {MODCheck()?.resolvedPath
                          ? language.t("wsl.onboarding.updateMod")
                          : language.t("wsl.onboarding.installMod")}
                      </Button>
                    </Show>
                  </div>
                </div>
                <div class="text-12-regular text-text-weak whitespace-pre-wrap break-words">{MODMessage()}</div>
                <Show when={MODCheck()?.matchesDesktop === false ? MODCheck() : null}>
                  {(check) => (
                    <div class="rounded-md border border-border-weak-base px-3 py-3 flex flex-col gap-1">
                      <div class="text-12-regular text-text-weak">
          ...
          <Show when={activeStep() === "MOD" && allReady() && selectedDistro()}>
