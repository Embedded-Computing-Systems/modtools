...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when MOD needs attention", () => {
    ...
    ).toBe("wsl.onboarding.installMod")
    expect(
      wslModAction({
        distro: "Debian",
  ...
  test("does not report ready when MOD is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        modChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.mod/bin/mod",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installMod" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-mod")
  })
