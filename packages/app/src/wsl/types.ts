...
export type WslJob =
  ...
  | { kind: "probe-MOD"; distro: string; startedAt: number }
  | { kind: "install-MOD"; distro: string; startedAt: number }
...
export type WslServersPlatform = {
  ...
  probeMod(name: string): Promise<void>
  installMod(name: string): Promise<void>
