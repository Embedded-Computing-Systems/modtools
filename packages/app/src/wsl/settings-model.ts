...
export function wslModAction(check?: WslModCheck) {
  if (!check) return
  if (!check.resolvedPath) return "wsl.onboarding.installMod"
  if (check.matchesDesktop === false) return "wsl.onboarding.updateMod"
}
...
export function addServerViewModel(input: {
  ...
  const modCheck = selectedDistro ? (state?.modChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.modChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateMod" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.modMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installMod" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  ...
  const installingMod =
    input.state?.job?.kind === "install-mod" && input.state.job.distro === input.selectedDistro
  if (!ready || probingMod) {
    return {
      variant: "contrast",
  ...
  if (!addServerModReady(input.modCheck)) {
    const update = !!input.modCheck?.resolvedPath && input.modCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingMod
        ? { key: "wsl.onboarding.updatingMod" }
        : update
          ? { key: "wsl.onboarding.updateMod" }
          : { key: "wsl.onboarding.installMod" },
      disabled: !!input.state?.job || input.adding,
      action: "install-mod",
      loading: installingMod,
...
function addServerModReady(check: WslModCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.modChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.modChecks[item.name]) return [`mod:${item.name}`]
    return []
  })
