...
import { Dialog } from "@modtools-ai/ui/v2/dialog-v2"
...
import { wslModAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.MODChecks[item.config.distro]
          const MODAction = () => wslModAction(check())
          const busy = () => wsl.data?.job?.kind === "install-MOD" && wsl.data.job.distro === item.config.distro
          return (
                ...
                <Show when={MODAction()}>
                  {(label) => (
                    <ButtonV2
                      ...
                      onClick={() => api && request.mutate(() => api.installMod(item.config.distro))}
                    >
