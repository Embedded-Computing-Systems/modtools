...
import { List, type ListRef } from "@modtools-ai/ui/list"
...
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@modtools-ai/ui/v2/dialog-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "mod-go") return language.t("dialog.provider.modGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "mod"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.mod.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["mod", "mod-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "mod" || provider.id === "mod-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "mod"
                                ? "dialog.provider.mod.tagline"
                                : "dialog.provider.modGo.tagline",
                            )}
                          </span>
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "mod"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.modZen.line1")}</div>
              <div>{language.t("provider.connect.modZen.line2")}</div>
              <div>
                {language.t("provider.connect.modZen.visit.prefix")}
                <Link
                  href="https://opencode.ai/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.modZen.visit.link")}
                </Link>
                {language.t("provider.connect.modZen.visit.suffix")}
              </div>
            </div>
          </Show>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "mod"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.modZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.modZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.modZen.visit.prefix")}
