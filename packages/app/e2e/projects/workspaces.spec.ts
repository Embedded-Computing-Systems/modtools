...
import { base64Decode } from "@modtools-ai/util/encode"
...
test("non-git projects keep workspace mode disabled", async ({ page, project }) => {
  ...
  const nonGit = await fs.mkdtemp(path.join(os.tmpdir(), "modtools-e2e-project-nongit-"))
  ...
  try {
    ...
    expect(path.basename(activeDir)).toContain("modtools-e2e-project-nongit-")
