...
import type { Item, Reply, Usage } from "../../modtools/test/lib/llm-server"
import { TestLLMServer } from "../../modtools/test/lib/llm-server"
...
const seedModel = (() => {
  const [providerID = "modtools", modelID = "big-pickle"] = (
    process.env.MOD_E2E_MODEL ?? "modtools/big-pickle"
  ).split("/")
  return {
    providerID: providerID || "modtools",
...
async function promptSend(page: Page) {
  return page
    .evaluate(() => {
      ...
      const sent = win.__modtools_e2e?.prompt?.sent
      return {
  ...
  const send = async (text: string, input: { noReply: boolean; shell: boolean }) => {
    if (input.noReply) {
      ...
      const state = await page.evaluate(() => {
        const model = (window as E2EWindow).__modtools_e2e?.model?.current
...
async function seedStorage(
  ...
  await page.addInitScript(
    (args: {
      ...
      const key = "modtools.global.dat:server"
      ...
      localStorage.setItem("modtools.settings.dat:defaultServerUrl", args.serverUrl)
      ...
      localStorage.setItem("modtools.global.dat:model", JSON.stringify({ recent: [args.model], user: [], variant: {} }))
    },
