...
test("creates a session in a new project, connects MOD Go, and selects its model", async ({ page }) => {
  ...
  await mockModServer(page, {
    ...
    provider: () => ({
      ...
      connected: connectedGo ? ["mod", "mod-go"] : ["mod"],
      default: { providerID: "mod", modelID: "free-model" },
    }),
    integrationMethods: { "mod-go": [{ type: "api", label: "API key" }] },
    onConnectKey: (input) => {
      connections.push(input)
      if (input.integrationID === "mod-go") pendingGo = true
    },
    onInstanceDispose: () => {
  ...
  await page.addInitScript(() => {
    ...
    localStorage.setItem("mod.global.dat:server", JSON.stringify({ projects: { local: [] } }))
  })
  ...
  await expect(page.locator('[data-section="free-models"]')).toContainText("Free models provided by MOD")

  await page.locator('[data-provider-id="mod-go"]').click()
  ...
  expect(connections).toEqual([{ integrationID: "mod-go", body: { type: "api", key: "mock-go-api-key" } }])

  await expect(modelControl).toHaveAttribute("data-control-type", "popover")
  await modelControl.click()
  const goModel = page.locator('[data-option-key="mod-go:go-model-1"]')
