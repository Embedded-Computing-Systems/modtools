...
test("navigates to a subagent child session missing from the session list", async ({ page }) => {
  ...
  const titlebarRight = page.locator("#mod-titlebar-right")
  ...
  return [
    {
      info: {
        ...
        agent: "build",
        model: { providerID: "mod", modelID: "claude-opus-4-6" },
      },
      parts: [
        {
...
async function configurePage(page: Page) {
  ...
  await page.addInitScript(
    ({ directory, server, sessionId }) => {
      ...
      localStorage.setItem("mod.window.browser.dat:tabs", JSON.stringify([{ type: "session", server, sessionId }]))
    },
