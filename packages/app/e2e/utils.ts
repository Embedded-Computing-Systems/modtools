...
import { base64Encode, checksum } from "@modtools-ai/util/encode"
...
export function createSdk(directory?: string, baseUrl = serverUrl) {
  return createModtoolsClient({ baseUrl, directory, throwOnError: true })
}
...
export function workspacePersistKey(directory: string, key: string) {
  ...
  return `modtools.workspace.${head}.${sum}.dat:workspace:${key}`
}
