...
async function todoDock(page: any, sessionID: string) {
  ...
  const write = async (driver: ComposerDriverState | undefined) => {
    await page.evaluate(
      (input: { event: string; sessionID: string; driver: ComposerDriverState | undefined }) => {
        const win = window as ComposerWindow
        const composer = win.__modtools_e2e?.composer
  ...
  const read = () =>
    page.evaluate((sessionID: string) => {
      const win = window as ComposerWindow
      return win.__modtools_e2e?.composer?.sessions?.[sessionID]?.probe ?? null
    ...
    async (session) => {
      ...
      await withMockPermission(
        page,
        {
          ...
          sessionID: session.id,
          permission: "bash",
          patterns: ["/tmp/modtools-e2e-perm-once"],
    ...
    async (session) => {
      ...
      await withMockPermission(
        page,
        {
          ...
          sessionID: session.id,
          permission: "bash",
          patterns: ["/tmp/modtools-e2e-perm-reject"],
        },
        undefined,
        async (state) => {
          await page.goto(page.url())
    ...
    async (session) => {
      ...
      await withMockPermission(
        page,
        {
          ...
          sessionID: session.id,
          permission: "bash",
          patterns: ["/tmp/modtools-e2e-perm-always"],
          metadata: { description: "Need permission for command" },
        },
        undefined,
        async (state) => {
      ...
      try {
        await withMockPermission(
          page,
          {
            ...
            sessionID: child.id,
            permission: "bash",
            patterns: ["/tmp/modtools-e2e-perm-child"],
