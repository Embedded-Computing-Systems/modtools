...
import type { IntegrationOAuthMethodRegistration } from "@modtools-ai/plugin/v2/effect/integration"
import { define } from "@modtools-ai/plugin/v2/effect/plugin"
import type { CredentialValue } from "@modtools-ai/sdk/v2/types"
...
const defaultServer = "https://modtools.ai/console"
const clientID = "mod-cli"
...
export const ModPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "mod",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("ModPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("mod")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load MOD provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("mod", (integration) => {
        integration.name = "MOD"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "mod", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("mod")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("mod")
      ...
      const item = catalog.provider.get(ProviderV2.ID.mod)
      if (!item) return
      const hasKey = Boolean(process.env.MODTOOLS_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("mod")),
