...
// test/preload.ts pins MODTOOLS_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.MODTOOLS_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.MODTOOLS_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.MODTOOLS_MODELS_PATH = undefined
  Flag.MODTOOLS_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.MODTOOLS_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.MODTOOLS_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
  ...
  it.live("get() recovers from a corrupted cache file by fetching a fresh catalog", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.acquireUseRelease(
        Effect.sync(() => {
          Flag.MODTOOLS_DISABLE_MODELS_FETCH = false
        }),
        () =>
          provided(
            state,
