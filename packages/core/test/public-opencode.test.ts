...
import { AbsolutePath, Location, Model, Mod, Session, Tool } from "@modtools-ai/core/public"
import { tmpdir } from "./fixture/tmpdir"
import { testEffect } from "./lib/effect"

const it = testEffect(Mod.layer)

describe("public native Mod API", () => {
  it.effect("exposes only the intentional Session capabilities", () =>
    Effect.gen(function* () {
      const MOD = yield* Mod.Service

      expect(Object.keys(MOD).sort()).toEqual(["sessions", "tools"])

      expect(Object.keys(MOD.sessions).sort()).toEqual([
        "context",
        "create",
        "events",
        "get",
      ...
      expect(yield* MOD.sessions.list()).toBeArray()
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          expect((yield* MOD.sessions.get(sessionID)).model).toEqual(model)
        }),
      ),
    ),
  )
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap(([available, disabled]) =>
        Effect.gen(function* () {
          ...
          yield* MOD.sessions.switchModel({ sessionID: availableID, model: ref({ variant: "default" }) })
          const disabledError = yield* MOD.sessions
            .switchModel({ sessionID: disabledID, model: ref() })
            .pipe(Effect.flip)
          const missingError = yield* MOD.sessions
          ...
          expect((yield* MOD.sessions.get(availableID)).model).toEqual(ref({ variant: "default" }))
          expect((yield* MOD.sessions.get(disabledID)).model).toBeUndefined()
        }),
      ),
    ),
  )
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          yield* MOD.sessions.switchModel({ sessionID, model: selected })

          const error = yield* MOD.sessions
            .switchModel({ sessionID, model: ref({ variant: "unknown" }) })
            .pipe(Effect.flip)

          expect(error).toBeInstanceOf(Session.VariantUnavailableError)
          expect((yield* MOD.sessions.get(sessionID)).model).toEqual(selected)
        }),
      ),
    ),
  )
