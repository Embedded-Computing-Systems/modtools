import { FileDiff, Message, Model, Part, Session, SessionStatus, UserMessage } from "@modtools-ai/sdk/v2"
import { SessionTurn } from "@modtools-ai/ui/session-turn"
import { SessionReview } from "@modtools-ai/ui/session-review"
import { DataProvider } from "@modtools-ai/ui/context"
...
import { iife } from "@modtools-ai/util/iife"
import { Binary } from "@modtools-ai/util/binary"
...
import { FileSSR } from "@modtools-ai/ui/file-ssr"
...
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@modtools-ai/ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/modtools-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | Model-of-Design Tools</Title>
              </Show>
              <Meta name="description" content="modtools - The AI coding agent built for the terminal." />
