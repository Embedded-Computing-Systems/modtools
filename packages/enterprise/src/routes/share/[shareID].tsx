import { Message, Model, Part, Session, SessionStatus, SnapshotFileDiff, UserMessage } from "@modtools-ai/sdk/v2"
import { SessionTurn } from "@modtools-ai/session-ui/session-turn"
import { SessionReview } from "@modtools-ai/session-ui/session-review"
import { DataProvider } from "@modtools-ai/session-ui/context"
import { FileComponentProvider } from "@modtools-ai/ui/context/file"
import { WorkerPoolProvider } from "@modtools-ai/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createMemo, createSignal, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@modtools-ai/ui/logo"
import { IconButton } from "@modtools-ai/ui/icon-button"
import { ProviderIcon } from "@modtools-ai/ui/provider-icon"
import { iife } from "@modtools-ai/core/util/iife"
import { Binary } from "@modtools-ai/core/util/binary"
import { NamedError } from "@modtools-ai/core/util/error"
import { DateTime } from "luxon"
import { createStore } from "solid-js/store"
import NotFound from "../[...404]"
import { Tabs } from "@modtools-ai/ui/tabs"
import { MessageNav } from "@modtools-ai/session-ui/message-nav"
import { FileSSR } from "@modtools-ai/session-ui/file-ssr"
...
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@modtools-ai/session-ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/mod-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | MOD</Title>
              </Show>
              <Meta name="description" content="mod - The AI coding agent built for the terminal." />
                        ...
                        <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                          <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                            ...
                            <div class="flex gap-3 items-center">
                              <IconButton
                                as={"a"}
                                href="https://github.com/Embedded-Computing-Systems/modtools"
                                target="_blank"
                                icon="github"
                                variant="ghost"
                              />
                              <IconButton
                                as={"a"}
                                href="https://modtools.ai/discord"
                                target="_blank"
                                icon="discord"
                                variant="ghost"
                              />
