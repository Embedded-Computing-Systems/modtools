...
test("embedded client uses the real router and handlers", async () => {
  const directory = await mkdtemp(join(tmpdir(), "mod-embedded-"))
  const database = Flag.MODTOOLS_DB
  Flag.MODTOOLS_DB = join(directory, "mod.sqlite")
  ...
  try {
    const program = Effect.gen(function* () {
      const mod = yield* OpenCode.create()
      yield* mod.tools.register({
      ...
      const created = yield* mod.sessions.create({
        id: sessionID,
        agent: Agent.ID.make("build"),
        location: Location.Ref.make({ directory: AbsolutePath.make(directory) }),
      })
      yield* mod.sessions.switchModel({ sessionID, model })
      const selected = yield* mod.sessions.get({ sessionID })
      const page = yield* mod.sessions.list({ directory: AbsolutePath.make(directory) })
      const active = yield* mod.sessions.active()
      const admitted = yield* mod.sessions.prompt({
        sessionID,
        prompt: Prompt.make({ text: "Do not run" }),
        resume: false,
      })
      const context = yield* mod.sessions.context({ sessionID })
      const wake = yield* mod.sessions.prompt({
        sessionID,
        prompt: Prompt.make({ text: "Promote this input" }),
      })
      const prompted = yield* mod.sessions.events({ sessionID }).pipe(
        Stream.filter((event) => event.type === "session.next.prompted" && event.data.messageID === wake.id),
        Stream.runHead,
        Effect.timeout("10 seconds"),
        Effect.map(Option.getOrThrow),
      )
      const wakeContext = yield* mod.sessions.context({ sessionID })
      const event = yield* mod.sessions
        .events({ sessionID })
        .pipe(Stream.take(1), Stream.runHead, Effect.map(Option.getOrUndefined))
      const modelMessage = Option.fromNullishOr(context.find((message) => message.type === "model-switched")).pipe(
        Option.getOrThrow,
      )
      const message = yield* mod.sessions.message({ sessionID, messageID: modelMessage.id })
      yield* mod.sessions.interrupt({ sessionID })
      const other = yield* mod.sessions.create({
        location: Location.Ref.make({ directory: AbsolutePath.make(directory) }),
      })
      const missingSessionID = Session.ID.make(`ses_missing_${crypto.randomUUID()}`)
      const missing = yield* Effect.all(
        [
          mod.sessions.events({ sessionID: missingSessionID }).pipe(Stream.runHead, Effect.flip),
          mod.sessions.interrupt({ sessionID: missingSessionID }).pipe(Effect.flip),
          mod.sessions.message({ sessionID: missingSessionID, messageID: modelMessage.id }).pipe(Effect.flip),
        ],
        { concurrency: "unbounded" },
      )
      const missingMessage = yield* Effect.flip(
        mod.sessions.message({
          sessionID: other.id,
    ...
    Flag.MODTOOLS_DB = database
    await rm(directory, { recursive: true, force: true })
  }
})

test("Location-owned runner events reach the ready global client", async () => {
  const directory = await mkdtemp(join(tmpdir(), "mod-embedded-events-"))
  const database = Flag.MODTOOLS_DB
  Flag.MODTOOLS_DB = join(directory, "mod.sqlite")
  ...
  try {
    const program = Effect.gen(function* () {
      ...
      yield* mod.events.subscribe().pipe(
      ...
      yield* mod.sessions.create({
        id: sessionID,
        location: Location.Ref.make({ directory: AbsolutePath.make(directory) }),
      })
      yield* mod.sessions.prompt({ sessionID, prompt: Prompt.make({ text: "Observe this input" }) })
...
test("independent embedded hosts do not share live notifications", async () => {
  const directory = await mkdtemp(join(tmpdir(), "mod-embedded-hosts-"))
  const database = Flag.MODTOOLS_DB
  Flag.MODTOOLS_DB = join(directory, "mod.sqlite")
...
test("embedded client is available as a Layer service", async () => {
  const directory = await mkdtemp(join(tmpdir(), "mod-embedded-layer-"))
  const database = Flag.MODTOOLS_DB
  Flag.MODTOOLS_DB = join(directory, "mod.sqlite")
  ...
  try {
    const created = await Effect.runPromise(
      Effect.gen(function* () {
        const mod = yield* OpenCode.Service
        return yield* mod.sessions.create({
          id: sessionID,
          location: Location.Ref.make({ directory: AbsolutePath.make(directory) }),
        })
