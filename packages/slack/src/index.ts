...
import { createModtools, type ToolPart } from "@modtools-ai/sdk"
...
console.log("🚀 Starting modtools server...")
const modtools = await createModtools({
  port: 0,
})
console.log("✅ Modtools server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
;(async () => {
  const events = await modtools.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new modtools session...")
    const { client, server } = modtools
    ...
    console.log("✅ Created modtools session:", createResult.data.id)
  ...
  console.log("📝 Sending to modtools:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Modtools response:", JSON.stringify(result, null, 2))
