...
import { createMod, type ToolPart } from "@modtools-ai/sdk"
...
console.log("🚀 Starting MOD server...")
const mod = await createMod({
  port: 0,
})
console.log("✅ MOD server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await mod.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new MOD session...")
    const { client, server } = mod
    ...
    console.log("✅ Created MOD session:", createResult.data.id)
  ...
  console.log("📝 Sending to MOD:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 MOD response:", JSON.stringify(result, null, 2))
