...
const reg = process.env.REGISTRY ?? "ghcr.io/Embedded-Computing-Systems"
...
const setup = async () => {
  ...
  if (list.includes("modtools")) {
    await $`docker buildx use modtools`
    return
  }
  await $`docker buildx create --name modtools --use`
}
