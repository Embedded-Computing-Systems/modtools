...
import { getModModeStack, MODTOOLS_BASE_MODE, ModKeymapProvider, registerModKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerModKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when mod mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getModModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getModModeStack(keymap).push("autocomplete")
