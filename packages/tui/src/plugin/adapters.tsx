import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@modtools-ai/plugin/tui"
...
import type { useModKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useModKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getModModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getModModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
