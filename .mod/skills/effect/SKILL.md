...
1. If `.mod/references/effect-smol` is missing, clone `https://github.com/Effect-TS/effect-smol` there. Do this in the project, not in the skill folder.
2. Search `.mod/references/effect-smol` for exact APIs, examples, tests, and naming patterns before answering or implementing Effect-specific code.
...
- Do not answer from memory. Verify against `.mod/references/effect-smol` or nearby code first.
...
- Run tests from package directories such as `packages/modtools`; never run package tests from the repo root.
