...
cd "$PROJECT_ROOT"
  ...
  # Channel must be set before `bun run build`, not just before packaging: prebuild
  # resolves it too, and on the dev channel (the default) v1.18.8+ tries to npm-install
  # upstream's next-generation CLI (`mod2`), which does not exist for a local MOD build.
  # On prod that download step is skipped.
...
# ============================================
# STEP 7.5: Ask MOD App Intents extension
# ============================================
# The Electron binary is not ours to link Swift into, so the intent ships as an
# ExtensionKit appex built from .packages/ask-mod and embedded post-install. Skipped
# quietly when the toolchain or the stash is absent (e.g. building on Linux).
if [ "$INSTALL_GUI" = true ] && [ "$PLATFORM" = "darwin" ] && [ -d "/Applications/MOD.app" ]; then
  HELPER_BUILDER="$HOME/git/modtools/.packages/ask-mod/build-helper.sh"
  if [ -x "$HELPER_BUILDER" ]; then
    echo ""
    echo -e "${YELLOW}Step 7.5: Installing the Ask MOD helper app...${NC}"
    if "$HELPER_BUILDER" > /tmp/mod-askhelper.log 2>&1; then
      echo -e "  ${GREEN}Ask MOD.app installed and registered${NC}"
    else
      echo -e "  ${RED}helper build failed — see /tmp/mod-askhelper.log${NC}"
    fi
  fi

  IMAGE_BUILDER="$HOME/git/modtools/.packages/scripts/build-install-image.sh"
  if [ -x "$IMAGE_BUILDER" ] && [ "${MOD_NO_IMAGE:-}" != "1" ]; then
    echo ""
    echo -e "${YELLOW}Step 7.6: Building the install image (DMG for other Macs)...${NC}"
    if "$IMAGE_BUILDER" --worktree "$PROJECT_ROOT" > /tmp/mod-image.log 2>&1; then
      echo -e "  ${GREEN}$(grep -o 'Image ready: .*' /tmp/mod-image.log | head -1)${NC}"
    else
      echo -e "  ${RED}image build failed — see /tmp/mod-image.log${NC}"
    fi
  fi
fi
