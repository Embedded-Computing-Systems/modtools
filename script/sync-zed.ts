...
const FORK_REPO = "Embedded-Computing-Systems/zed-extensions"
const UPSTREAM_REPO = "zed-industries/extensions"
const EXTENSION_NAME = "modtools"
...
async function main() {
  ...
  // Delete any existing branches for modtools updates
  ...
  const prResult =
    await $`gh pr create --repo ${UPSTREAM_REPO} --base main --head ${FORK_REPO.split("/")[0]}:${branchName} --title "Update ${EXTENSION_NAME} to v${cleanVersion}" --body "Updating Model-of-Design Tools extension to v${cleanVersion}"`
