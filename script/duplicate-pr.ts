...
import { createMod } from "@modtools-ai/sdk"
...
`)
  ...
  const MOD = await createMod({ port: 0 })
  ...
  try {
    ...
    const session = await MOD.client.session.create()
    const result = await MOD.client.session
      .prompt({
