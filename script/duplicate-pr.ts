...
import { createModtools } from "@modtools-ai/sdk"
...
`)
  ...
  const modtools = await createModtools({ port: 0 })
  ...
  try {
    ...
    const session = await modtools.client.session.create()
    const result = await modtools.client.session
      .prompt({
