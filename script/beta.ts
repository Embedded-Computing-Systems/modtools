...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with mod...`)
  ...
  try {
    await $`mod run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  mod failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with mod")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with mod...")
