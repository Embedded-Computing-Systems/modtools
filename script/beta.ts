...
const model = "modtools/gpt-5.3-codex"
...
async function typecheck() {
  ...
  try {
    await $`bun typecheck`.cwd("packages/modtools")
    return true
  } catch (err) {
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/modtools")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with modtools...`)
  ...
  const prompt = [
    ...
    "After resolving the conflicts, run `bun typecheck` in `packages/modtools`.",
  ...
  try {
    await $`modtools run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  modtools failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with modtools")
  return true
}

async function smoke(prs: PR[], applied: number[]) {
  console.log("\nRunning final smoke check with modtools...")

  const done = lines(prs.filter((x) => applied.includes(x.number)))
  const prompt = [
    "The beta merge batch is complete.",
    `Merged PRs on HEAD:\n${done}`,
    "Run `bun typecheck` in `packages/modtools`.",
    "Run `./script/build.ts --single` in `packages/modtools`.",
