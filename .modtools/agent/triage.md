...
- `modtools web` command
- The desktop app itself

**Only** add if it's specifically about the Desktop application or `modtools web` view. **Do not** add for terminal, TUI, or general modtools issues.
...
**Only** add if the issue mentions "zen" or "modtools zen" or "modtools black".

If the issue doesn't have "zen" or "modtools black" in it then don't add zen label

#### core

Use for core server issues in `packages/modtools/`, excluding `packages/modtools/src/cli/cmd/tui/`.
...
TUI (`packages/modtools/src/cli/cmd/tui/...`):
...
Core (`packages/modtools/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for modtools server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
