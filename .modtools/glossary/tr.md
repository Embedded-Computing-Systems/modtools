...
- `Mod` (preserve casing in prose, docs, and UI copy)
- Keep lowercase `MOD` in commands, package names, paths, URLs, and other exact identifiers
...
- Keep product naming consistent: `Mod` in prose, `MOD` only for exact technical identifiers
...
- Avoid changing `MOD` to `Mod` inside commands, URLs, package names, or code literals
